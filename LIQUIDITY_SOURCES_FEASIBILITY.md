...
| **Algebra forks** (Camelot/QuickSwap V3, Ramses V2) | tractable-now | callback (router) | dispatched as UniV3 | yes (needs `globalState()` reader + dynamic-fee source) | yes (V3 oracle + fee source) | M |
...
**4. Algebra dynamic-fee forks (M).** Camelot/QuickSwap V3, Ramses V2 already map to `poolType=UniV3` and
execute fine, but read `globalState()` (not `slot0()`) and carry a **dynamic fee**. **Integration**: a
state-reader variant in the lens for `globalState()` + a fee source; the V3 oracle is otherwise reused
unchanged. The fee is read once at quote time and treated as fixed over the trade (same snapshot assumption
the recipe already makes for V3 tiers), so it stays wei-exact against that snapshot.
