...
function walkStatements(statements, locals, declMap, kinds, ctx) {
    let dynamic = false;
    for (const stmt of statements) {
        if (walkStatement(stmt, locals, declMap, kinds, ctx))
            dynamic = true;
    }
    return dynamic;
}
...
function walkStatement(stmt, locals, declMap, kinds, ctx) {
    switch (stmt.type) {
        case 'BlockStatement':
            return walkStatements(stmt.body, locals, declMap, kinds, ctx);
        case 'IfStatement': {
            const ifStmt = stmt;
        ...
        case 'ForStatement':
            return walkBody(stmt.body, locals, declMap, kinds, ctx);
        case 'WhileStatement':
            return walkBody(stmt.body, locals, declMap, kinds, ctx);
        case 'VariableDeclaration': {
            for (const declarator of stmt.declarations) {
                if (declarator.id.type === 'Identifier' && declarator.init) {
                    const kind = kindOfExpr(declarator.init, locals, declMap, kinds);
                    locals.set(declarator.id.name, kind);
                    continue;
                }
        ...
        case 'ExpressionStatement': {
            ...
            if (expr.type === 'AssignmentExpression') {
                ...
                if (assign.operator === '=' && assign.left.type === 'Identifier') {
                    const kind = kindOfExpr(assign.right, locals, declMap, kinds);
                    ...
                    if (kind === 'dynamic')
                        locals.set(assign.left.name, 'dynamic');
                }
            }
            return false;
        }
        case 'ReturnStatement': {
            const ret = stmt;
            return ret.argument ? kindOfExpr(ret.argument, locals, declMap, kinds) === 'dynamic' : false;
        }
        default:
            return false;
    }
...
function walkBody(body, locals, declMap, kinds, ctx) {
...
function kindOfExpr(expr, locals, declMap, kinds) {
