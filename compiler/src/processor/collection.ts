...
const assertNoNestedArrayLiteral = (elements: Expression[], ctx: CompilerContext): void => {
  if (!ctx.isV12) return;

  if (elements.some((el) => el.type === 'ArrayExpression')) {
    throw new Error(
      `nested array literals (e.g. [[1, 2], [3, 4]]) are not supported on target '${ctx.target}' — the ` +
...
const assertNoDynamicVariableObjectField = (
  properties: { key: string; value: Expression }[],
  processed: SaucerLike[],
): void => {
  properties.forEach((p, i) => {
    ...
    if (bytes.length === 2 && bytes[0] === OPS.READ_HEAP) {
      throw new Error(
        `object literal field '${p.key}' reads an existing dynamic-kind variable (an array, a new Array(n) ` +
