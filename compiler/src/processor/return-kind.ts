...
import type { ContractInfo, MethodInfo } from '../contracts.js';
...
// A `let pool = Contract.at(addr);` binding tracked by THIS pass's own walk — see
// `kindOfExpr`'s own doc comment for why `ctx.lookupBoundContract` can never resolve one of
// these here, and why this can't just reuse `ctx.boundContracts` directly.
type LocalBoundContract = { contract: ContractInfo; callTypeOverride?: 'static' | 'delegate' };
...
function functionReturnsDynamic(
  ...
  const localBoundContracts = new Map<string, LocalBoundContract>();
...
function walkStatements(
  ...
  kinds: Map<string, VariableKind>,
  ctx: CompilerContext,
  localBoundContracts: Map<string, LocalBoundContract>,
): boolean {
  let dynamic = false;

  for (const stmt of statements) {
    if (walkStatement(stmt, locals, declMap, kinds, ctx, localBoundContracts)) dynamic = true;
  }
...
function walkStatement(
  ...
  switch (stmt.type) {
    case 'BlockStatement':
      return walkStatements((stmt as BlockStatement).body, locals, declMap, kinds, ctx, localBoundContracts);
    ...
    case 'ForStatement':
      return walkBody((stmt as ForStatement).body, locals, declMap, kinds, ctx, localBoundContracts);

    case 'WhileStatement':
      return walkBody((stmt as WhileStatement).body, locals, declMap, kinds, ctx, localBoundContracts);
    ...
    case 'ExpressionStatement': {
      ...
      if (expr.type === 'AssignmentExpression') {
        ...
        if (assign.operator === '=' && assign.left.type === 'Identifier') {
          ...
          const kind = kindOfExpr(assign.right as Expression, locals, declMap, kinds, ctx, localBoundContracts);
          ...
          if (kind === 'dynamic') locals.set(name, 'dynamic');
          ...
          const standalone = matchStandaloneBindingShape(assign.right as Expression);
          const contract = standalone ? ctx.lookupContract(standalone.contractName) : undefined;
    ...
    case 'ReturnStatement': {
      ...
      return ret.argument
        ? kindOfExpr(ret.argument as Expression, locals, declMap, kinds, ctx, localBoundContracts) === 'dynamic'
        : false;
    }
...
function kindOfExpr(
  ...
  if (expr.type === 'CallExpression') {
    ...
    if (!resolveCatchChain(call)) {
      ...
      if (outputs && outputs.length === 1 && abiOutputKind(outputs[0]) === 'dynamic') return 'dynamic';
    }
  }
...
function resolveMethodTarget(
  ...
  const bound = localBoundContracts.get((member.object as { name: string }).name);
