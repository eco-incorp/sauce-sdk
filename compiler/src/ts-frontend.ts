 ...
 */
function containsShadowedIdentifier(node: ts.Node, shadowed: ReadonlySet<string>): boolean {
  if (shadowed.size === 0) return false;

  if (ts.isIdentifier(node)) return shadowed.has(node.text);

  let found = false;

  ts.forEachChild(node, (child) => {
    if (!found && containsShadowedIdentifier(child, shadowed)) found = true;
  });
...
function foldExpression(
  ...
  if (containsShadowedIdentifier(node, shadowed)) return { success: false };
 ...
 */
function collidesWithSurroundingDeclarations(
      ...
      if (ts.isIfStatement(node)) {
        ...
        if (evaluated.success) {
          ...
          if (ts.isBlock(taken)) {
            ...
            if (!collidesWithSurroundingDeclarations(result, shadowed)) return result;
          } else {
        ...
        if (node.body) collectLexicalNamesInScope(node.body, names);
        ...
        if (node.body) collectLexicalNamesInScope(node.body, names); // see isPlainFunctionScope's note above
...
/** The union of `isPlainFunctionScope`'s 3 shapes and `isMethodLikeScope`'s 3 — every node kind
 * that introduces its OWN function-like variable scope in this language surface. Shared by the
 * scope-shadow helpers below and by the local (per-function) constant-propagation pass further
 * down, which walks exactly this same family of scopes independently, one at a time. */
...
function isFunctionLikeScope(node: ts.Node): node is FunctionLikeNode {
 ...
 */
function collectLexicalNamesInScope(node: ts.Node, out: Set<string>): void {
  if (ts.isFunctionDeclaration(node) && node.name) out.add(node.name.text); // record before treating as a nested scope below

  if (isFunctionLikeScope(node) || ts.isClassLike(node)) return; // a separate scope — don't recurse into its own body

  if (ts.isVariableStatement(node)) {
    for (const decl of node.declarationList.declarations) collectBindingNames(decl.name, out);
  } else if (ts.isForStatement(node) && node.initializer && ts.isVariableDeclarationList(node.initializer)) {
    for (const decl of node.initializer.declarations) collectBindingNames(decl.name, out);
  } else if (
...
function substituteConstReads(
  ...
  if (ts.isForInStatement(node) || ts.isForOfStatement(node)) {
    const names = new Set(shadowed);
    let visitedInit: ts.ForInitializer;

    if (ts.isVariableDeclarationList(node.initializer)) {
      for (const decl of node.initializer.declarations) collectBindingNames(decl.name, names);

      visitedInit = visit(node.initializer, names) as ts.ForInitializer;
    } else {
 ...
 */
function collectAssignmentTargetIdentifiers(node: ts.Expression, report: (name: string) => void): void {
  if (ts.isParenthesizedExpression(node)) {
  ...
  if (ts.isObjectLiteralExpression(node)) {
    for (const prop of node.properties) {
      ...
      else if (ts.isPropertyAssignment(prop)) collectAssignmentTargetIdentifiers(prop.initializer, report);
      else if (ts.isSpreadAssignment(prop)) collectAssignmentTargetIdentifiers(prop.expression, report);
    }
 ...
 */
function collectWriteCounts(root: ts.Node, candidateNames: ReadonlySet<string>): Map<string, number> {
  ...
  const reportIfUnshadowed = (shadowed: ReadonlySet<string>) => (name: string) => {
    if (!shadowed.has(name)) bump(name);
  };

  const visit = (node: ts.Node, shadowed: ReadonlySet<string>): void => {
    if (isOutOfScopeForPropagation(node)) return;
    ...
    if (ts.isBlock(node)) {
      ...
      for (const s of node.statements) visit(s, names);
    ...
    if (isPlainFunctionScope(node)) {
      ...
      ts.forEachChild(node, (child) => {
        if (child !== node.name) visit(child, names);
      });
    ...
    if (ts.isForStatement(node)) {
      ...
      if (node.initializer) visit(node.initializer, names);

      if (node.condition) visit(node.condition, names);

      if (node.incrementor) visit(node.incrementor, names);
...
function collectTopLevelMutableNames(sourceFile: ts.SourceFile): Set<string> {
  const names = new Set<string>();
...
/**
 * Real top-level `const`s AND effectively-const top-level `let`/`var`s (both shapes above),
 * resolved together in ONE top-to-bottom pass so a later declaration may reference an earlier
 * one regardless of which kind either is — exactly how a real `const` chain already works.
 *
 * Deliberately returns NAMES only, never node references: this analysis runs against its OWN
 * `ts.createSourceFile` parse (see `tsPartialEval`), a completely separate tree instance from
 * the one `ts.transpileModule` parses internally and actually hands to the transformers below —
 * two independent parses of the same text never share node identity. `consts`/`primaryNames`
 * are plain data (a name is a name in either tree), so this was always safe for real `const`s;
 * the STRETCH pair shape must be the same way — `deadConstEliminationTransformer` RE-DERIVES
 * which specific statements a pair name maps to, structurally, fresh, from whichever tree it is
 * actually handed (`findPairStatements` below) — never by carrying a node reference across.
 */
interface TopLevelConstAnalysis {
  consts: Map<string, bigint>;
  /** PRIMARY: a top-level `let`/`var` with an inline initializer, confirmed never written
   * again — eliminable exactly like a real top-level `const` (single-declarator removal). */
  primaryNames: Set<string>;
  /** STRETCH: a top-level `let`/`var` predeclared with no initializer, then assigned exactly
   * once by a later, unconditional, DIRECT top-level statement — eliminable as a PAIR. */
  pairNames: Set<string>;
}
...
function simpleAssignmentTargetName(stmt: ts.Statement): string | undefined {
...
function analyzeTopLevelConsts(sourceFile: ts.SourceFile, functions: TopLevelFunctions): TopLevelConstAnalysis {
  ...
  const pendingPredecl = new Set<string>();
  ...
  for (const stmt of sourceFile.statements) {
    ...
    if (pendingPredecl.size > 0) {
      for (const name of [...pendingPredecl]) {
        if (name !== resolvingName && referencesIdentifier(stmt, name)) pendingPredecl.delete(name);
      }
    }
    ...
    if (ts.isVariableStatement(stmt)) {
      ...
      for (const decl of stmt.declarationList.declarations) {
        ...
        if (isConst) {
          ...
          if (value !== undefined) consts.set(name, value);

          continue;
        }

        if (decl.initializer) {
          if ((writeCounts.get(name) ?? 0) > 0) continue; // written again somewhere — not effectively const
    ...
    if (!pendingPredecl.has(name)) continue;

    pendingPredecl.delete(name); // this IS the name's one recognized write, resolvable or not

    if (writeCounts.get(name) !== 1) continue; // a write exists elsewhere too — not sound to fold
 ...
 */
function findPairStatements(
  sourceFile: ts.SourceFile,
  name: string,
):
  | {
      declStatement: ts.VariableStatement;
      declName: ts.Identifier;
      assignStatement: ts.ExpressionStatement;
      assignTargetName: ts.Identifier;
    }
  | undefined {
  let declStatement: ts.VariableStatement | undefined;
  let declName: ts.Identifier | undefined;
  let assignStatement: ts.ExpressionStatement | undefined;
  let assignTargetName: ts.Identifier | undefined;
  ...
  if (!declStatement || !declName || !assignStatement || !assignTargetName) return undefined;
 ...
 */
function countIdentifierRefs(root: ts.Node, name: string, exclude: ReadonlySet<ts.Node>): number {
  let count = 0;
...
// ── Array/object lookup-table folding ──
//
// A top-level `const NAME = [<foldable>, ...]` / `const NAME = { key: <foldable>, ... }`
// whose elements/properties are ALL themselves foldable to a bigint (via the SAME
// `tsEvalConst`/`consts` machinery the scalar consts above already use) is a candidate
// "lookup table". If — and only if — a whole-file scan proves every remaining use of NAME is
// a plain constant-indexed read (`NAME[k]` with a compile-time-constant, in-bounds, non-negative
// integer key, or `NAME.prop`/`NAME["prop"]`), each resolvable access is folded to its literal
// element/property value, same as a scalar const's read. This is a strictly NARROWER, additive
// capability layered on top of everything above — it never changes what a scalar `consts` read
// resolves to, and a table that fails ANY part of the soundness proof is simply never tracked,
// leaving it and every access to it completely untouched (fail closed, not "fold what we can").
//
// SOUNDNESS is entirely the file-wide scan in `isSafeTableRead`/`checkTableSafety`: before a
// literal is treated as foldable AT ALL, every occurrence of its name anywhere in the file
// (regardless of whether that code path is reachable — an unreachable `if (false) { ARR.push() }`
// still disqualifies) must be a direct, unshadowed `NAME.prop`/`NAME[k]` access that is itself
// neither a call/new callee (rule out `NAME.anything(...)` — ANY method name, no whitelist), an
// assignment/update/delete target (rule out `NAME[i] = ...`, `NAME.x = ...`, `NAME[i]++`, `delete
// NAME.x`, and — climbing through the object/array-literal "pattern" node shapes TypeScript
// reuses for destructuring assignment — `({ k: NAME.x } = obj)` / `[NAME.x] = arr` too), nor
// anything else at all (a bare `NAME` used as a call argument, aliased to another variable,
// returned, spread, reassigned, or any other shape not immediately recognized as a safe
// property/element-access base) — this is a strict ALLOWLIST (only the specific safe shapes
// pass), not a blocklist of known-bad shapes, so an unanticipated escape shape fails closed by
// construction rather than by enumeration. A single disqualifying occurrence anywhere rules out
// the WHOLE table; it is never partially tracked.
//
...
interface ObjectTable {
  ...
  readonly props: ReadonlyMap<string, bigint>;
}

type TableConst = ArrayTable | ObjectTable;

/** Every array element must itself resolve via `tsEvalConst` — no spreads, no nested tables. */
function foldArrayLiteralElements(
  ...
  for (const el of lit.elements) {
    if (ts.isSpreadElement(el) || ts.isArrayLiteralExpression(el) || ts.isObjectLiteralExpression(el)) return undefined;
 ...
 */
function foldObjectLiteralProps(
  ...
  for (const prop of lit.properties) {
    if (!ts.isPropertyAssignment(prop) || ts.isComputedPropertyName(prop.name)) return undefined;
 ...
 */
function outermostParenWrapper(node: ts.Node): ts.Node {
  let current = node;
 ...
 */
function isTableAccessUnsafeUsage(access: ts.PropertyAccessExpression | ts.ElementAccessExpression): boolean {
  const wrapped = outermostParenWrapper(access);
  const immediateParent = wrapped.parent;

  if (immediateParent && ts.isCallExpression(immediateParent) && immediateParent.expression === wrapped) return true;

  if (immediateParent && ts.isNewExpression(immediateParent) && immediateParent.expression === wrapped) return true;

  if (immediateParent && ts.isDeleteExpression(immediateParent) && immediateParent.expression === wrapped) return true;
  ...
  for (;;) {
    ...
    if ((ts.isForOfStatement(parent) || ts.isForInStatement(parent)) && parent.initializer === w) return true;
 ...
 */
function isSafeTableRead(id: ts.Identifier): boolean {
  const parent = id.parent;

  if (ts.isPropertyAccessExpression(parent) && parent.expression === id) return !isTableAccessUnsafeUsage(parent);

  if (ts.isElementAccessExpression(parent) && parent.expression === id) return !isTableAccessUnsafeUsage(parent);
 ...
 */
function isForOfIterableExpression(id: ts.Identifier): boolean {
  const parent = id.parent;
 ...
 */
function checkTableSafety(
  node: ts.Node,
  name: string,
  kind: TableConst['kind'],
  shadowed: ReadonlySet<string>,
  exclude: ts.Node,
): boolean {
  ...
  if (isOutOfScopeForPropagation(node)) return true;
  ...
  if (isPlainFunctionScope(node)) {
    ...
    ts.forEachChild(node, (child) => {
      ...
      if (!checkTableSafety(child, name, kind, names, exclude)) ok = false;
    });
  ...
  if (ts.isForStatement(node)) {
    ...
    if (node.initializer && !checkTableSafety(node.initializer, name, kind, names, exclude)) ok = false;

    if (node.condition && !checkTableSafety(node.condition, name, kind, names, exclude)) ok = false;

    if (node.incrementor && !checkTableSafety(node.incrementor, name, kind, names, exclude)) ok = false;

    if (!checkTableSafety(node.statement, name, kind, names, exclude)) ok = false;
  ...
  if (ts.isForInStatement(node) || ts.isForOfStatement(node)) {
    ...
    if (!checkTableSafety(node.initializer, name, kind, names, exclude)) ok = false;
  ...
  ts.forEachChild(node, (child) => {
    if (!checkTableSafety(child, name, kind, shadowed, exclude)) ok = false;
  });
 ...
 * feature existed.
 */
function collectTopLevelTables(
  sourceFile: ts.SourceFile,
  consts: ReadonlyMap<string, bigint>,
  functions: TopLevelFunctions,
): Map<string, TableConst> {
  const tables = new Map<string, TableConst>();
  ...
  for (const stmt of sourceFile.statements) {
    ...
    for (const decl of stmt.declarationList.declarations) {
      ...
      let entry: TableConst | undefined;

      if (ts.isArrayLiteralExpression(init)) {
        const elements = foldArrayLiteralElements(init, consts, functions);

        if (elements) entry = { kind: 'array', elements };
      } else if (ts.isObjectLiteralExpression(init)) {
        const props = foldObjectLiteralProps(init, consts, functions);

        if (props) entry = { kind: 'object', props };
      }

      if (!entry) continue; // not a fully-foldable literal — never a candidate, left completely alone

      if (checkTableSafety(sourceFile, name, entry.kind, new Set(), decl.name)) tables.set(name, entry);
    }
  }
...
function foldTableAccess(
  ...
  if (ts.isPropertyAccessExpression(node)) {
    if (table.kind !== 'object') return undefined;
  ...
  if (table.kind === 'array') {
    ...
    if (index === undefined || index < 0n || index >= BigInt(table.elements.length)) return undefined;
 ...
 */
function tryUnrollForOfArrayTable(
  node: ts.ForOfStatement,
  tables: ReadonlyMap<string, TableConst>,
  shadowed: ReadonlySet<string>,
  context: ts.TransformationContext,
  reprocess: (statements: readonly ts.Statement[], shadowed: ReadonlySet<string>) => ts.Statement[],
): ts.Statement[] | undefined {
  if (node.awaitModifier) return undefined; // for-await — out of scope

  if (!ts.isIdentifier(node.expression) || shadowed.has(node.expression.text)) return undefined;

  const table = tables.get(node.expression.text);

  if (!table || table.kind !== 'array') return undefined; // only an ARRAY table has an iteration order

  if (!ts.isVariableDeclarationList(node.initializer) || node.initializer.declarations.length !== 1) return undefined;

  const decl = node.initializer.declarations[0];

  if (!ts.isIdentifier(decl.name)) return undefined; // a destructured loop variable — out of scope, decline

  const loopVar = decl.name.text;

  if (table.elements.length > MAX_UNROLL_ITERATIONS) return undefined;

  if (bodyBlocksUnrolling(node.statement, loopVar)) return undefined;
...
function visitStatementList(
  statements: readonly ts.Statement[],
  visit: (n: ts.Node, shadowed: ReadonlySet<string>) => ts.Node,
  shadowed: ReadonlySet<string>,
  tables: ReadonlyMap<string, TableConst>,
  context: ts.TransformationContext,
): ts.Statement[] {
  const reprocess = (stmts: readonly ts.Statement[], s: ReadonlySet<string>): ts.Statement[] =>
    visitStatementList(stmts, visit, s, tables, context);
  ...
  return statements.flatMap((stmt) => {
    ...
    return [visit(stmt, shadowed) as ts.Statement];
  });
}

function tableFoldTransformer(
  tables: ReadonlyMap<string, TableConst>,
  consts: ReadonlyMap<string, bigint>,
  functions: TopLevelFunctions,
) {
  return (context: ts.TransformationContext): ts.Transformer<ts.SourceFile> => {
    const visit = (node: ts.Node, shadowed: ReadonlySet<string>): ts.Node => {
      ...
      if (ts.isForInStatement(node) || ts.isForOfStatement(node)) {
        ...
        const visitedInit = visit(node.initializer, names) as ts.ForInitializer;
        const visitedStmt = visit(node.statement, names) as ts.Statement;
        const visitedExpr = visit(node.expression, shadowed) as ts.Expression; // outer scope
    ...
    return (sourceFile) =>
      ts.factory.updateSourceFile(
        sourceFile,
        visitStatementList(sourceFile.statements, visit, new Set(), tables, context),
      );
  };
}
 ...
 */
function deadConstEliminationTransformer(
  consts: ReadonlyMap<string, bigint>,
  primaryNames: ReadonlySet<string>,
  pairNames: ReadonlySet<string>,
  tables: ReadonlyMap<string, TableConst>,
) {
  return (_context: ts.TransformationContext): ts.Transformer<ts.SourceFile> => {
    return (sourceFile) => {
      ...
      for (const stmt of sourceFile.statements) {
        ...
        for (const decl of stmt.declarationList.declarations) {
          if (!ts.isIdentifier(decl.name) || declNameNodes.has(decl.name.text)) continue;

          const name = decl.name.text;
          const tracked = isConst ? consts.has(name) || tables.has(name) : primaryNames.has(name);

          if (tracked) declNameNodes.set(name, decl.name);
        }
      }
      ...
      const pairInfo = new Map<string, NonNullable<ReturnType<typeof findPairStatements>>>();

      for (const name of pairNames) {
        const found = findPairStatements(sourceFile, name);

        if (found) pairInfo.set(name, found);
      }

      if (declNameNodes.size === 0 && pairInfo.size === 0) return sourceFile;

      const eliminable = new Set<string>();

      for (const [name, nameNode] of declNameNodes) {
        if (countIdentifierRefs(sourceFile, name, new Set([nameNode])) === 0) eliminable.add(name);
      }

      for (const [name, info] of pairInfo) {
        const exclude = new Set<ts.Node>([info.declName, info.assignTargetName]);

        if (countIdentifierRefs(sourceFile, name, exclude) === 0) eliminable.add(name);
      }
      ...
      const statements = sourceFile.statements.flatMap((stmt) => {
        ...
        for (const [name, info] of pairInfo) {
          if (eliminable.has(name) && info.assignStatement === stmt) return [];
        }
        ...
        const kept = stmt.declarationList.declarations.filter((decl) => {
          ...
          if (isConst) return !((consts.has(name) || tables.has(name)) && eliminable.has(name));

          if (primaryNames.has(name)) return !eliminable.has(name);

          const pair = pairInfo.get(name);

          if (pair && pair.declStatement === stmt) return !eliminable.has(name);
...
type LocalValue = bigint | typeof NAC;
type LocalEnv = Map<string, LocalValue>;

/** The same-file top-level `consts`/`functions` this pass only ever READS — bundled purely so
 * its many helpers don't have to thread two separate parameters everywhere a snapshot of "what
 * does the top level know" is needed. */
interface TopLevelContext {
  readonly consts: ReadonlyMap<string, bigint>;
  readonly functions: TopLevelFunctions;
}

/**
 * Bundles the exact 3 extra arguments every real `tsEvalConst`/`tsEvalUnary`/`tsEvalBinary`
 * call below needs, besides the expression node itself — a snapshot of this pass's OWN
 * evolving per-function `env` at one program point, built by `resolveLocalEnv` just below.
 * `consts` is `top.consts` OVERLAID with every name this scope currently knows a concrete
 * bigint value for, so an identifier read naturally resolves to whichever is correct (the
 * local value if tracked, else the top-level const — exactly matching `tsEvalConst`'s own
 * single flat lookup, unchanged). `shadowed` is every name this scope tracks WITHOUT (yet, or
 * ever) knowing a concrete value — deliberately NOT every locally-bound name, since a
 * known-value local must NOT be in `shadowed` (`tsEvalConst`'s Identifier case checks
 * `shadowed` BEFORE ever consulting `consts`, so including a known-value name there would
 * incorrectly block it from ever resolving). `shadowed` still does its normal job for
 * `tsEvalCall`'s OWN callee check, though: a name this scope tracks at all — NAC or not —
 * that happens to collide with a same-named top-level function (most plausibly a nested
 * function declaration shadowing an unrelated top-level one of the same name, since
 * `collectLexicalNamesInScope` — reused for this pre-seed — records a nested function
 * declaration's name too) is seeded NAC in `env` from the moment this pass first sees the
 * function's body (see `transformFunctionLikeScopeBody`), so it always ends up in `shadowed`
 * here and can never be wrongly resolved as a call to the OUTER, unrelated top-level function.
 */
...
function resolveLocalEnv(env: ReadonlyMap<string, LocalValue>, top: TopLevelContext): LocalResolution {
...
function markNamesNac(name: ts.BindingName, env: LocalEnv): void {
  const names = new Set<string>();

  collectBindingNames(name, names);

  for (const n of names) env.set(n, NAC);
}
 ...
 */
function containsDisallowedConstruct(node: ts.Node): boolean {
  if (isOutOfScopeForPropagation(node) || isFunctionLikeScope(node)) return false;
  ...
  ts.forEachChild(node, (child) => {
    if (!found && containsDisallowedConstruct(child)) found = true;
  });
 ...
 */
function collectFreeIdentifierNames(node: ts.Node, bound: ReadonlySet<string>, out: Set<string>): void {
  if (isOutOfScopeForPropagation(node)) return;

  if (ts.isIdentifier(node)) {
    if (!bound.has(node.text)) out.add(node.text);
  ...
  if (isFunctionLikeScope(node)) {
    ...
    if (isPlainFunctionScope(node) && node.name) inner.add(node.name.text); // a named function expression can reference itself

    if (node.body && ts.isBlock(node.body)) collectLexicalNamesInScope(node.body, inner);
    ...
    if (node.body) collectFreeIdentifierNames(node.body, inner, out);
 ...
 * names bails `fn`.
 */
function bailsForOuterClosure(fn: FunctionLikeNode): boolean {
  if (!fn.body) return false;

  const localNames = new Set<string>();

  for (const p of fn.parameters) collectBindingNames(p.name, localNames);

  if (ts.isBlock(fn.body)) collectLexicalNamesInScope(fn.body, localNames);
...
function functionBails(fn: FunctionLikeNode): boolean {
  if (!fn.body) return false;
...
function visitLocalAssignmentTarget(
  node: ts.Expression,
  resolution: LocalResolution,
  context: ts.TransformationContext,
): ts.Expression {
  const read = (n: ts.Expression): ts.Expression => substituteLocalReads(n, resolution, context) as ts.Expression;
  const target = (n: ts.Expression): ts.Expression => visitLocalAssignmentTarget(n, resolution, context);
...
function substituteLocalReads(node: ts.Node, resolution: LocalResolution, context: ts.TransformationContext): ts.Node {
  const visit = (n: ts.Node): ts.Node => substituteLocalReads(n, resolution, context);
  ...
  if (isFunctionLikeScope(node)) return node;

  if (ts.isBinaryExpression(node) && ASSIGNMENT_OPERATOR_TOKENS.has(node.operatorToken.kind)) {
    return ts.factory.updateBinaryExpression(
      node,
      visitLocalAssignmentTarget(node.left, resolution, context),
      node.operatorToken,
      visit(node.right) as ts.Expression,
    );
  }
  ...
  if (ts.isVariableDeclaration(node)) {
    return ts.factory.updateVariableDeclaration(
      node,
      node.name, // a binding name is never a read
      node.exclamationToken,
      node.type,
      node.initializer ? (visit(node.initializer) as ts.Expression) : undefined,
    );
  ...
  if (ts.isParameter(node) || ts.isCatchClause(node)) return node; // defensive no-op; not a shape this pass reaches in practice
 ...
 */
function containsIdentifier(node: ts.Node): boolean {
...
function foldOrSubstitute(
  ...
  if (value !== undefined) {
    ...
    if (literal) return { expr: literal, value };
  }

  return { expr: substituteLocalReads(expr, resolution, context) as ts.Expression, value: undefined };
}
...
function walkAssignmentExpr(
  ...
  if (ts.isIdentifier(expr.left)) {
    ...
    for (const name of written) env.set(name, NAC);
  }
...
function walkUpdateExpr(
  ...
  if (ts.isIdentifier(expr.operand)) {
    ...
    const next =
      current === undefined ? undefined : expr.operator === ts.SyntaxKind.PlusPlusToken ? current + 1n : current - 1n;
...
function walkExpressionStatementExpr(
  expr: ts.Expression,
  env: LocalEnv,
  top: TopLevelContext,
  context: ts.TransformationContext,
  ...
  if (ts.isParenthesizedExpression(expr)) {
    return ts.factory.updateParenthesizedExpression(
      expr,
      walkExpressionStatementExpr(expr.expression, env, top, context),
    );
  }
  ...
  return substituteLocalReads(expr, resolution, context) as ts.Expression;
}
...
function walkVariableDeclarator(
  ...
  if (!ts.isIdentifier(decl.name)) {
    ...
    return ts.factory.updateVariableDeclaration(
      ...
      decl.type,
      substituteLocalReads(decl.initializer, resolution, context) as ts.Expression,
    );
  }
...
function collectWrittenNames(node: ts.Node, out: Set<string>): void {
...
function collectAssignmentTargetNames(node: ts.Expression, out: Set<string>): void {
  if (ts.isParenthesizedExpression(node)) {
  ...
  if (ts.isObjectLiteralExpression(node)) {
    for (const prop of node.properties) {
      ...
      else if (ts.isPropertyAssignment(prop)) collectAssignmentTargetNames(prop.initializer, out);
...
function walkForInitializer(
  ...
  if (ts.isVariableDeclarationList(init)) {
    return ts.factory.updateVariableDeclarationList(
      init,
      init.declarations.map((d) => substituteLocalReads(d, resolution, context) as ts.VariableDeclaration),
    );
  }

  return substituteLocalReads(init, resolution, context) as ts.Expression;
}
...
function walkLoopStatement(
  ...
  if (ts.isForStatement(stmt)) {
    ...
    const newCondition = stmt.condition
      ? (substituteLocalReads(stmt.condition, resolution, context) as ts.Expression)
      : undefined;
    const newIncrementor = stmt.incrementor
      ? (substituteLocalReads(stmt.incrementor, resolution, context) as ts.Expression)
      : undefined;
    const newBody = substituteLocalReads(stmt.statement, resolution, context) as ts.Statement;

    return ts.factory.updateForStatement(stmt, newInit, newCondition, newIncrementor, newBody);
  }

  if (ts.isWhileStatement(stmt)) {
    const newCondition = substituteLocalReads(stmt.expression, resolution, context) as ts.Expression;
    const newBody = substituteLocalReads(stmt.statement, resolution, context) as ts.Statement;
...
function walkStatement(
  ...
  if (ts.isVariableStatement(stmt)) {
    return ts.factory.updateVariableStatement(
      ...
      ts.factory.updateVariableDeclarationList(
        ...
        stmt.declarationList.declarations.map((decl) => walkVariableDeclarator(decl, env, top, context)),
      ),
    );
  }
  ...
  if (ts.isIfStatement(stmt)) return walkIfStatement(stmt, env, top, context);
  ...
  if (ts.isReturnStatement(stmt)) {
    if (!stmt.expression) return stmt;
...
function walkStatementList(
  ...
  for (const name of declaredHere) env.set(name, NAC);

  return statements.map((stmt) => walkStatement(stmt, env, top, context));
}

function updateFunctionLikeBody(node: FunctionLikeNode, body: ts.Block): ts.Node {
...
function transformFunctionLikeScopeBody(
  ...
  for (const p of node.parameters) markNamesNac(p.name, env);
  ...
  for (const name of lexicalNames) env.set(name, NAC);
 ...
 */
function localConstPropagationTransformer(top: TopLevelContext) {
  return (context: ts.TransformationContext): ts.Transformer<ts.SourceFile> => {
    const visit = (node: ts.Node): ts.Node => {
      if (isOutOfScopeForPropagation(node)) return node;

      if (isFunctionLikeScope(node)) {
        if (functionBails(node)) return node;
