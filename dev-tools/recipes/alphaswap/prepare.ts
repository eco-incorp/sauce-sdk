...
import { BASE_TOKENS } from "../shared/constants";
import type {
  AlphaSwapConfig,
  AlphaSwapPrepared,
...
export async function prepareAlphaSwap(
  ...
  const filteredMultiHop = multiHopRoutes
    .filter((r) => {
      const effLiq =
        r.hop1Pool.liquidity < r.hop2Pool.liquidity
    ...
    .sort((a, b) => {
      const effA =
        a.hop1Pool.liquidity < a.hop2Pool.liquidity
          ? a.hop1Pool.liquidity
          : a.hop2Pool.liquidity;
      const effB =
        b.hop1Pool.liquidity < b.hop2Pool.liquidity
  ...
  const effLiq = (r: DiscoveredMultiHopRoute) =>
    r.hop1Pool.liquidity < r.hop2Pool.liquidity
      ? r.hop1Pool.liquidity
      : r.hop2Pool.liquidity;
  ...
  const finalMultiHop = filteredMultiHop.filter(
    (r) => effLiq(r) >= relativeThreshold,
  );
