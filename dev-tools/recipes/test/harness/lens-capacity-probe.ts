 ...
 *   npx tsx recipes/test/harness/lens-capacity-probe.ts
 *   ECO_ENGINE=v1 npx tsx recipes/test/harness/lens-capacity-probe.ts
 */
  ...
  if (!existsSync(blob) || !existsSync(manifestPath)) {
    console.error(
      `cached allpools state for engine ${engine} not found at ${blob}\n` +
        `capture it first: RECAPTURE_ANVIL_STATE=1 npx tsx --test recipes/test/ecoswap.allpools.prodmirror.evm.test.ts`,
    );
    process.exit(1);
  }
