...
function stepReal(sqrtReal, stepRatio, zeroForOne) {
...
function v3Slices(p, out) {
    ...
    if (ts <= 0 || stepRatio <= 0n || spot <= 0n || p.spotTickShifted === undefined)
        return;
    ...
    const net = new Map();
    for (const r of p.netRows ?? [])
        net.set(r.shiftedTick, r.rawNet);
    let L = liveL;
    let nearReal = spot;
 ...
 */
export function estimateExpectedOutput(prepared, amountIn) {
    if (amountIn <= 0n)
        return 0n;
    const slices = [];
    ...
    slices.sort((a, b) => {
        if (a.adjNear !== b.adjNear)
            return a.adjNear < b.adjNear ? 1 : -1;
        if (a.adjFar !== b.adjFar)
            return a.adjFar < b.adjFar ? 1 : -1;
        return 0;
    });
    let cum = 0n;
    let out = 0n;
...
//# sourceMappingURL=expected-output.js.map