 ...
 */
import { createPublicClient, http, defineChain, parseAbi, keccak256, encodeAbiParameters, encodeFunctionData, decodeFunctionResult, } from "viem";
import { createRequire } from "module";
import { join, dirname } from "path";
import { fileURLToPath } from "url";
import { readFileSync } from "fs";
import ts from "typescript";
import { prepareEcoSwap } from "./prepare.js";
import { MULTICALL3, BASE_CHAIN_POOL_CONFIG, SwapPoolType, } from "../shared/constants.js";
 ...
 */
function buildPoolTuple(p, netStart, netCount) {
    return [
        BigInt(p.poolType),
        BigInt(p.address),
        BigInt(p.fee),
 ...
 */
function buildPoolsAndNetCache(pools) {
    const netCache = [];
    const poolTuples = [];
    for (const p of pools) {
        ...
        for (const r of rows)
            netCache.push([r.shiftedTick, r.rawNet]);
    }
 ...
 */
export function buildUniverseRoutingAndQlv(prepared) {
    const directCount = prepared.pools.length;
    const universe = [...prepared.pools];
    ...
    prepared.routes.forEach((route, routeIdx) => {
        ...
        route.legs.forEach((leg, legIdx) => {
            ...
            const inter = legIdx < route.intermediateTokens.length
    ...
    for (let v = directQlvCount; v < qlvRows.length; v += 1) {
        ...
        if (ri < prevRoute || (ri === prevRoute && li < prevLeg)) {
            throw new Error(`qlv ordering violated: leg row ${v} (route ${ri}, leg ${li}) after (route ${prevRoute}, leg ${prevLeg})`);
        }
        prevRoute = ri;
        prevLeg = li;
    }
    ...
    if (directQlvCount + coveredLegRows !== qlvRows.length) {
        throw new Error(`qlv ordering violated: ${qlvRows.length - directQlvCount} leg rows built but routing covers ${coveredLegRows}`);
    }
 ...
 */
function fluidResolverAddr(prepared) {
    const first = prepared.fluidPools?.[0];
    if (first)
        return BigInt(first.resolver);
    const leg = legQlVenues(prepared).find((v) => v.family === "fluid");
 ...
 */
function mentoBrokerAddr(prepared) {
    const first = prepared.mentoPools?.[0];
    if (first)
        return BigInt(first.broker);
    const leg = legQlVenues(prepared).find((v) => v.family === "mento");
 ...
 */
function balancerV3RouterAddr(prepared) {
    const first = prepared.balancerV3Pools?.[0];
    if (first)
        return BigInt(first.router);
    const leg = legQlVenues(prepared).find((v) => v.family === "balancerV3");
 ...
 */
function balancerV3VaultAddr(prepared) {
    const first = prepared.balancerV3Pools?.[0];
    if (first)
        return BigInt(first.vault);
    const leg = legQlVenues(prepared).find((v) => v.family === "balancerV3");
 ...
 */
function balancerV2VaultAddr(prepared) {
    const first = prepared.balancerStables?.[0];
    if (first)
        return BigInt(first.vault);
    const leg = legQlVenues(prepared).find((v) => v.family === "balancerV2");
    return leg ? BigInt(leg.desc.vault) : 0n;
}
...
const pad12 = (r) => r.length >= 12 ? r : [...r, ...new Array(12 - r.length).fill(0n)];
/**
...
function qlRowFor(v, refIdx) {
    switch (v.family) {
        ...
        case "balancerV2": {
            ...
            const packedReg = BigInt(rp[0] ?? 0) | (BigInt(rp[1] ?? 0) << 8n) | (BigInt(rp[2] ?? 0) << 16n);
            return [
                BigInt(b.address),
                BigInt(b.i),
                BigInt(b.j),
 ...
 */
function buildQLVenues(prepared) {
 ...
 */
export function protocolDefines(prepared) {
    ...
    const HAS_V3 = allPools.some((p) => !p.isV2 && p.poolType !== SwapPoolType.UniV4 && p.poolType !== SwapPoolType.PancakeInfinityCL);
    ...
    const hasLegFam = (family) => legQl.some((v) => v.family === family);
    ...
    const HAS_SOLIDLY_STABLE = (prepared.solidlyStables?.length ?? 0) > 0 || hasLegFam("solidlyStable");
    ...
    const HAS_PANCAKE_STABLE = (prepared.pancakeStablePools?.length ?? 0) > 0 || hasLegFam("pancakeStable");
 ...
 * dereferenced).
 */
function infinityVaultAddr(prepared) {
 ...
 */
export function buildSolverArgs(tokenIn, tokenOut, amountIn, caller, prepared) {
    const { poolTuples, netCache, routing, directCount, qlvRows, directQlvCount } = buildUniverseRoutingAndQlv(prepared);
    return [
        [
            BigInt(tokenIn),
            BigInt(tokenOut),
 ...
 */
export async function ecoSwap(config, rpcUrl, cookEntry, caller, poolConfig, opts, target = "v1") {
    ...
    const prepared = await prepareEcoSwap(config, client, cookEntry, poolConfig ?? BASE_CHAIN_POOL_CONFIG, { ...(opts ?? {}), lensTarget: opts?.lensTarget ?? target, caller });
...
export const OZ_ERC20_SLOTS = { balanceSlot: 0n, allowanceSlot: 1n };
...
function nestedMappingSlot(a, b, slot) {
    const inner = mappingSlot(a, slot);
    return keccak256(encodeAbiParameters([{ type: "address" }, { type: "bytes32" }], [b, inner]));
}
...
const OVERRIDE_AMOUNT = ("0x" + "0".repeat(32) + "f".repeat(32));
/**
 ...
 */
export async function quoteEcoSwap(config, rpcUrl, cookEntry, caller, poolConfig, opts) {
    ...
    const client = createPublicClient({ chain, transport: http(rpcUrl, { timeout: 120_000 }) });
    const prepared = await prepareEcoSwap(config, client, lensRouter, poolConfig ?? BASE_CHAIN_POOL_CONFIG, { ...(opts ?? {}), lensTarget: opts?.lensTarget ?? target, caller });
    ...
    const clearCache = (p) => p.isV2 ? p : { ...p, netRows: [], windowTopShifted: 0n, windowBotShifted: 0n };
    const usePrepared = opts?.noBrackets
        ? {
            ...prepared,
    ...
    const { poolTuples, netCache, routing, directCount, qlvRows, directQlvCount } = buildUniverseRoutingAndQlv(usePrepared);
    const result = compile(jsSource, {
        baseDirs: [REPO_ROOT, __dirname],
        target,
    ...
    const amountOut = decodeCookUint(ret, target);
 ...
 */
function decodeCookUint(ret, target) {
    if (!ret || ret === "0x")
        return 0n;
    if (target === "v1") {
