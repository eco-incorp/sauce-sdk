...
import { parseAbi, decodeAbiParameters, encodeFunctionData, keccak256, encodeAbiParameters, } from "viem";
import { FactoryType, V2_DEFAULT_FEE_PPM, SLIPSTREAM_TICK_SPACINGS, feeToTickSpacing, } from "../shared/constants.js";
...
const DEFAULT_LENS_ACCOUNT = "0x0000000000000000000000000000000000000001";
/**
 ...
 */
export function buildLensCook(poolConfig, params) {
    ...
    const v3Factories = poolConfig.factories.filter((f) => (f.factoryType === FactoryType.V3Standard ||
    ...
    const v2Factories = poolConfig.factories.filter((f) => f.factoryType === FactoryType.V2Standard);
    const v4Factories = poolConfig.factories.filter((f) => f.factoryType === FactoryType.UniswapV4 && !!f.stateView);
    ...
    const infinityFactories = poolConfig.factories.filter((f) => f.factoryType === FactoryType.PancakeInfinityCL);
    ...
    const v3FeeSet = new Set();
    for (const f of v3Factories) {
        if (f.factoryType === FactoryType.AlgebraV3)
            continue;
        if (f.factoryType === FactoryType.SlipstreamCL) {
            ...
            for (const ts of f.slipstreamTickSpacings ?? [...SLIPSTREAM_TICK_SPACINGS])
                v3FeeSet.add(ts);
            continue;
        }
        for (const fee of f.feeTiers ?? poolConfig.feeTiers)
            v3FeeSet.add(fee);
    }
    ...
    const v4SpecSet = new Set();
    for (const f of v4Factories)
        for (const fee of f.feeTiers ?? poolConfig.feeTiers)
            v4SpecSet.add(fee);
    ...
    const singletonFactories = [
        ...v4Factories.map((f) => ({ poolManager: f.address, stateView: f.stateView, isInfinity: false, cfg: f })),
    ...
    for (const fac of singletonFactories) {
        for (const spec of v4Specs) {
            if (fac.isInfinity !== spec.isInfinity) {
                ...
                v4PoolIds.push(computeInfinityPoolId(currency0, currency1, ZERO_HOOKS, fac.poolManager, spec.fee, encodeInfinityCLParameters(spec.tickSpacing)));
            }
            else {
 ...
 */
export async function runLens(client, cookEntry, poolConfig, params) {
    ...
    if (target === "v12") {
        const { data } = await client.call({
            account,
            to: cookEntry,
            data: encodeFunctionData({ abi: cookAbi, functionName: "cook", args: [bytecodes] }),
            gas: 2000000000n,
        });
        programOut = (data ?? "0x");
    }
    ...
    const [poolBlob, tickBlob] = decodeAbiParameters([{ type: "bytes" }, { type: "bytes" }], programOut);
 ...
 */
export async function measureLensGas(client, cookEntry, poolConfig, params) {
...
export function decodeLens(poolBlob, tickBlob) {
    ...
    for (let i = 0; i < nPools; i++) {
        ...
        const poolType = Number(pw[o]);
        const addr = ("0x" + pw[o + 1].toString(16).padStart(40, "0"));
        ...
        pools.push({
            ...
            hooks: ("0x" + pw[o + 4].toString(16).padStart(40, "0")),
            sqrtPriceX96: pw[o + 5],
            liquidity: pw[o + 6],
            tick: Number(BigInt.asIntN(24, pw[o + 7])),
            inIsToken0: pw[o + 8] === 1n,
            stateView: ("0x" + stateViewWord.toString(16).padStart(40, "0")),
            poolId: ("0x" + poolIdWord.toString(16).padStart(64, "0")),
            net: new Map(),
    ...
    for (let r = 0; r < nRows; r++) {
        ...
        if (net !== 0n)
            pools[poolIdx].net.set(tickIdx, net);
    }
