 ...
 */
import { runLens, LENS_MAX_TICKS, LENS_BAND_TICKS } from "./lens.js";
import { INFINITY_DEFAULT_CL_PRESETS, computeInfinityPoolId, encodeInfinityCLParameters, } from "../shared/infinity-math.js";
import { discoverKyberClassicPools, discoverAlgebraPoolAddresses, discoverInfinityCLPoolsTyped, discoverSolidlyVolatilePoolsTyped, discoverCurvePoolsTyped, discoverTraderJoeLBPoolsTyped, discoverDodoV2PoolsTyped, discoverSolidlyStablePoolsTyped, discoverWombatPoolsTyped, discoverBalancerStablePoolsTyped, discoverEulerSwapPoolsTyped, discoverMaverickV2PoolsTyped, discoverCryptoSwapPoolsTyped, discoverWooFiPoolsTyped, discoverFermiPoolsTyped, discoverFluidPoolsTyped, discoverTesseraPoolsTyped, discoverElfomoPoolsTyped, discoverMetricPoolsTyped, discoverLiquidCorePoolsTyped, discoverSizePoolsTyped, discoverPancakeStablePoolsTyped, discoverEkuboPoolsTyped, discoverMentoPoolsTyped, discoverBalancerV3PoolsTyped, } from "../shared/pool-discovery.js";
import { buildCurveSegments } from "../shared/curve-math.js";
import { buildCryptoSwapSegments } from "../shared/cryptoswap-math.js";
import { buildLbSegments, lbFeeToPpm } from "../shared/lb-math.js";
import { buildDodoSegments } from "../shared/dodo-math.js";
import { buildSolidlyStableSegments } from "../shared/solidly-stable-math.js";
import { buildWombatSegments } from "../shared/wombat-math.js";
import { buildWooFiSegments } from "../shared/woofi-math.js";
import { buildFermiSegments } from "../shared/fermi-math.js";
import { buildMentoSegments } from "../shared/mento-math.js";
import { buildBalancerV3QLLadder } from "../shared/balancer-v3-math.js";
import { buildEulerSwapSegments } from "../shared/eulerswap-math.js";
import { buildMaverickWalkLadder } from "../shared/maverick-math.js";
import { buildBalancerStableQLLadder } from "../shared/balancer-stable-math.js";
import { estimateExpectedOutput } from "./expected-output.js";
import { MIN_SQRT_RATIO, MAX_SQRT_RATIO, SwapPoolType, FactoryType, BASE_CHAIN_POOL_CONFIG, kyberFeeToPpm, } from "../shared/constants.js";
import { EcoBracketKind, } from "../shared/types.js";
// ── Tunables ─────────────────────────────────────────────────
const Q96 = 1n << 96n;
const Q192 = 1n << 192n;
const FEE_DENOM = 1000000n; // ppm
/**
...
function getSqrtRatioAtTick(tick) {
    ...
    if (absTick & 0x2n)
        mul(0xfff97272373d413259a46990580e213an);
    if (absTick & 0x4n)
        mul(0xfff2e50f5f656932ef12357cf3c7fdccn);
    if (absTick & 0x8n)
        mul(0xffe5caca7e10e4e61c3624eaa0941cd0n);
    if (absTick & 0x10n)
        mul(0xffcb9843d60f6159c9db58835c926644n);
    if (absTick & 0x20n)
        mul(0xff973b41fa98c081472e6896dfb254c0n);
    if (absTick & 0x40n)
        mul(0xff2ea16466c96a3843ec78b326b52861n);
    if (absTick & 0x80n)
        mul(0xfe5dee046a99a2a811c461f1969c3053n);
    if (absTick & 0x100n)
        mul(0xfcbe86c7900a88aedcffc83b479aa3a4n);
    if (absTick & 0x200n)
        mul(0xf987a7253ac413176f2b074cf7815e54n);
    if (absTick & 0x400n)
        mul(0xf3392b0822b70005940c7a398e4b70f3n);
    if (absTick & 0x800n)
        mul(0xe7159475a2c29b7443b29c7fa6e889d9n);
    if (absTick & 0x1000n)
        mul(0xd097f3bdfd2022b8845ad8f792aa5825n);
    if (absTick & 0x2000n)
        mul(0xa9f746462d870fdf8a65dc1f90e061e5n);
    if (absTick & 0x4000n)
        mul(0x70d869a156d2a1b890bb3df62baf32f7n);
    if (absTick & 0x8000n)
        mul(0x31be135f97d08fd981231505542fcfa6n);
    if (absTick & 0x10000n)
        mul(0x9aa508b5b7a84e1c677de54f3e99bc9n);
    if (absTick & 0x20000n)
        mul(0x5d6af8dedb81196699c329225ee604n);
    if (absTick & 0x40000n)
        mul(0x2216e584f5fa1ea926041bedfe98n);
    if (absTick & 0x80000n)
        mul(0x48a170391f7dc42444e8fa2n);
    if (tick > 0)
...
function toOutIn(sqrtReal, zeroForOne) {
 ...
 */
function stepRealTs(sqrtReal, stepRatio, zeroForOne) {
 ...
 */
function stampPoolCache(r, zeroForOne, seed) {
    ...
    if (scanned > 0) {
        ...
        for (const row of rows) {
            if (row.shiftedTick < wLo || row.shiftedTick > wHi) {
                throw new Error(`EcoSwap stampPoolCache: net row shiftedTick ${row.shiftedTick} outside cache window ` +
 ...
 */
function infinityKeyFeeByPoolId(hopIn, hopOut, poolConfig) {
    const map = new Map();
    const factories = poolConfig.factories.filter((f) => f.factoryType === FactoryType.PancakeInfinityCL);
    if (factories.length === 0)
        return map;
    const [c0, c1] = BigInt(hopIn) < BigInt(hopOut) ? [hopIn, hopOut] : [hopOut, hopIn];
    for (const f of factories) {
        for (const p of f.infinityPresets ?? INFINITY_DEFAULT_CL_PRESETS) {
            const id = computeInfinityPoolId(c0, c1, ZERO_ADDRESS, f.address, p.fee, encodeInfinityCLParameters(p.tickSpacing));
            map.set(id.toLowerCase(), p.fee);
        }
    }
    return map;
}
function lensToEcoPool(p, zHop, sourceLabel, isDirect, isAlgebra = false, infinityKeyFee) {
 ...
 */
function buildCurveBrackets(pool, refIdx, amountIn) {
 ...
 */
function buildCryptoSwapBrackets(pool, refIdx, amountIn) {
...
function curveFeeToPpm(feePpm10) {
    return Number((feePpm10 * 1000000n + 5000000000n) / 10000000000n);
}
/**
 ...
 */
function buildLbBrackets(pool, refIdx, amountIn) {
 ...
 */
function buildDodoBrackets(pool, refIdx, amountIn) {
 ...
 */
function buildSolidlyStableBrackets(pool, refIdx, amountIn) {
 ...
 */
function buildWombatBrackets(pool, refIdx, amountIn) {
 ...
 */
function buildWooFiBrackets(pool, refIdx, amountIn) {
 ...
 */
function buildFermiBrackets(pool, refIdx, amountIn) {
 ...
 */
function buildMentoBrackets(pool, refIdx, amountIn) {
 ...
 */
function buildEulerSwapBrackets(pool, refIdx, amountIn) {
...
function balancerFeeToPpm(swapFeeWad) {
    return Number((swapFeeWad * 1000000n + 5n * 10n ** 17n) / 10n ** 18n);
}
/**
 ...
 */
async function discoverQlVenuesForPair(pairIn, pairOut, probeAmount, client, poolConfig, dodoCaller) {
    const out = [];
    ...
    const curveRegistries = poolConfig.factories.filter((f) => f.factoryType === FactoryType.CurveRegistry);
    ...
    const solidlyFactories = poolConfig.factories.filter((f) => f.factoryType === FactoryType.SolidlyV2);
    ...
    const pancakeStableConfigs = poolConfig.factories.filter((f) => f.factoryType === FactoryType.PancakeStableSwap);
 ...
 */
function mentoClaimKey(exchangeProvider, exchangeId) {
    return `${exchangeProvider.toLowerCase()}|${exchangeId.toLowerCase()}`;
}
function ekuboClaimKey(poolId) {
    return `ekubo|${poolId.toLowerCase()}`;
}
function qlVenueClaimKey(v) {
    if (v.family === "mento") {
...
export async function prepareEcoSwap(config, client, 
...
lensCookEntry, poolConfig = BASE_CHAIN_POOL_CONFIG, opts = {}) {
    ...
    if (droppedByLens > 0) {
        console.log(`  EcoSwap lens dropped ${droppedByLens} shallow pool(s) (< ${minRelBps}bps of Σ in-range ` +
            `capacity, floor=${lensResult.capacityFloor} of Σ${lensResult.totalInRangeCapacity})`);
    }
    if (survivors.length > MAX_DIRECT_POOLS) {
        console.log(`  EcoSwap capped to deepest ${MAX_DIRECT_POOLS} of ${survivors.length} survivor pools (ECO_MAX_POOLS)`);
    }
    ...
    const algebraFactories = poolConfig.factories.filter((f) => f.factoryType === FactoryType.AlgebraV3);
    const algebraAddrs = algebraFactories.length > 0
        ? await discoverAlgebraPoolAddresses(tokenIn, tokenOut, client, algebraFactories)
        : new Set();
    const isAlgebraPool = (p) => algebraAddrs.size > 0 && algebraAddrs.has(p.address.toLowerCase());
    ...
    for (const p of v3Raw)
        pools.push(lensToEcoPool(p, zeroForOne, "lens V3", true, isAlgebraPool(p)));
    for (const p of v4Raw)
        pools.push(lensToEcoPool(p, zeroForOne, "lens V4", true));
    for (const p of infRaw) {
        pools.push(lensToEcoPool(p, zeroForOne, "lens InfinityCL", true, false, infKeyFees?.get(p.poolId.toLowerCase())));
    }
    for (const p of v2Raw)
        pools.push(lensToEcoPool(p, p.inIsToken0, "lens V2", true));
    ...
    const kyberFactories = poolConfig.factories.filter((f) => f.factoryType === FactoryType.KyberClassic);
    if (kyberFactories.length > 0) {
        ...
        for (const k of kyberRaw) {
            if (k.vReserveIn <= 0n || k.vReserveOut <= 0n)
                continue;
    ...
    const solidlyVolFactories = poolConfig.factories.filter((f) => f.factoryType === FactoryType.SolidlyV2);
    if (solidlyVolFactories.length > 0) {
        ...
        for (const v of volRaw) {
            if (v.reserveIn <= 0n || v.reserveOut <= 0n)
                continue;
    ...
    const infinityFactoriesCfg = poolConfig.factories.filter((f) => f.factoryType === FactoryType.PancakeInfinityCL);
    const hasInfinityTierB = infinityFactoriesCfg.some((f) => (f.infinityHookedPools?.length ?? 0) > 0 && (f.infinityHookAllowlist?.length ?? 0) > 0);
    if (hasInfinityTierB) {
        ...
        for (const ip of infTyped) {
            ...
            if (hooks.toLowerCase() === ZERO_ADDRESS.toLowerCase())
                continue; // Tier A rides the lens
    ...
    const brackets = [];
    const directQl = await discoverQlVenuesForPair(tokenIn, tokenOut, amountIn, client, poolConfig, caller ?? ZERO_ADDRESS);
    const qlFamilyDescs = (family) => directQl.filter((e) => e.venue.family === family).map((e) => e.venue.desc);
    const curves = qlFamilyDescs("curve");
    const cryptoSwaps = qlFamilyDescs("cryptoSwap");
    const solidlyStables = qlFamilyDescs("solidlyStable");
    const wooFiPools = qlFamilyDescs("wooFi");
    const lbs = qlFamilyDescs("lb");
    const mentoPools = qlFamilyDescs("mento");
    const dodos = qlFamilyDescs("dodo");
    const wombats = qlFamilyDescs("wombat");
    const fermiPools = qlFamilyDescs("fermi");
    const eulerSwaps = qlFamilyDescs("euler");
    const balancerV3Pools = qlFamilyDescs("balancerV3");
    const balancerStables = qlFamilyDescs("balancerV2");
    const maverickPools = qlFamilyDescs("maverick");
    ...
    const fluidPools = qlFamilyDescs("fluid");
    ...
    const tesseraPools = qlFamilyDescs("tessera");
    const elfomoPools = qlFamilyDescs("elfomo");
    ...
    const metricPools = qlFamilyDescs("metric");
    ...
    const liquidCorePools = qlFamilyDescs("liquidcore");
    const sizePools = qlFamilyDescs("size");
    ...
    const pancakeStablePools = qlFamilyDescs("pancakeStable");
    ...
    const ekuboPools = qlFamilyDescs("ekubo");
    ...
    // once.
    const edgeKey = (a, b) => `${a.toLowerCase()}|${b.toLowerCase()}`;
    ...
    const algebraEdgeCache = new Map();
    const readEdgeAlgebra = (hopIn, hopOut) => {
        if (algebraFactories.length === 0)
    ...
    const qlEdgeCache = new Map();
    const readEdgeQl = (hopIn, hopOut, estIn) => {
        const key = edgeKey(hopIn, hopOut);
    ...
    const foldEstIn = (est, headOI) => (((est * headOI) / Q96) * headOI) / Q96;
    /**
     ...
     */
    const buildLeg = async (hopIn, hopOut, estIn) => {
        ...
        for (const p of lens.pools) {
            if (p.poolType === SwapPoolType.UniV2) {
                ...
                legPools.push(lensToEcoPool(p, zHop, "lens route leg V3", false, legAlgebra.has(p.address.toLowerCase())));
            }
        ...
        if (legPools.length + edgeQl.length === 0)
            return null;
        ...
        if (legPools.length === 0) {
            for (const e of edgeQl)
                if (e.headOI > headOI)
                    headOI = e.headOI;
        }
        const leg = { hopIn, hopOut, zeroForOne: zHop, pools: legPools };
        if (edgeQl.length > 0)
            leg.qlVenues = edgeQl.map((e) => e.venue);
        return { leg, headOI };
    };
    ...
    const dfs = async (token, legsSoFar, interSoFar, visited, estIn) => {
        ...
        for (const next of interiorTokens) {
            ...
            if (visited.has(nl))
                continue; // no cycles — each token at most once per path
            const built = await buildLeg(token, next, estIn);
            if (!built)
                continue; // dead edge — prune
            visited.add(nl);
            await dfs(next, [...legsSoFar, built.leg], [...interSoFar, next], visited, foldEstIn(estIn, built.headOI));
            visited.delete(nl);
        }
    };
    ...
    if (MAX_ROUTES > 0) {
        await dfs(tokenIn, [], [], new Set([inLower]), amountIn);
    }
    if (routesTruncated > 0) {
        console.log(`  EcoSwap capped routes to ${MAX_ROUTES} (ECO_MAX_ROUTES); pruned exploration at ` +
    ...
    for (const p of pools)
        claimed.add(p.address.toLowerCase());
    ...
    for (const c of curves)
        claimed.add(c.address.toLowerCase());
    for (const cp of cryptoSwaps)
        claimed.add(cp.address.toLowerCase());
    for (const sp of solidlyStables)
        claimed.add(sp.address.toLowerCase());
    for (const wp of wooFiPools)
        claimed.add(wp.address.toLowerCase());
    for (const lb of lbs)
        claimed.add(lb.address.toLowerCase());
    for (const mp of mentoPools) {
        claimed.add(mentoClaimKey(mp.exchangeProvider, mp.exchangeId));
    }
    for (const d of dodos)
        claimed.add(d.address.toLowerCase());
    for (const wp of wombats)
        claimed.add(wp.address.toLowerCase());
    for (const fp of fermiPools)
        claimed.add(fp.address.toLowerCase());
    for (const ep of eulerSwaps)
        claimed.add(ep.address.toLowerCase());
    for (const bp of balancerV3Pools)
        claimed.add(bp.address.toLowerCase());
    for (const bp of balancerStables)
        claimed.add(bp.address.toLowerCase());
    for (const mp of maverickPools)
        claimed.add(mp.address.toLowerCase());
    for (const fp of fluidPools)
        claimed.add(fp.address.toLowerCase());
    ...
    // twice).
    for (const tp of tesseraPools)
        claimed.add(tp.address.toLowerCase());
    for (const ep of elfomoPools)
        claimed.add(ep.address.toLowerCase());
    // Metric claims by POOL address (per-pair inventory contracts — the qlVenueClaimKey default).
    for (const mp of metricPools)
        claimed.add(mp.address.toLowerCase());
    ...
    for (const lp of liquidCorePools)
        claimed.add(lp.address.toLowerCase());
    for (const sp of sizePools)
        claimed.add(sp.address.toLowerCase());
    ...
    for (const pp of pancakeStablePools)
        claimed.add(pp.address.toLowerCase());
    ...
    for (const ev of ekuboPools)
        claimed.add(ekuboClaimKey(ev.poolId));
    ...
    for (const route of orderedRoutes) {
        ...
        for (const leg of route.legs) {
            ...
            const keepVenues = legVenues.filter((v) => {
                ...
                if (claimed.has(k) || routeClaims.has(k) || legSeen.has(k))
                    return false;
                legSeen.add(k);
                return true;
            });
            ...
            for (const lp of keepPools)
                routeClaims.add(lp.address.toLowerCase());
            for (const v of keepVenues)
                routeClaims.add(qlVenueClaimKey(v));
            const survLeg = { ...leg, pools: keepPools };
            if (keepVenues.length > 0)
                survLeg.qlVenues = keepVenues;
            else
                delete survLeg.qlVenues; // a venue-free leg carries NO qlVenues key (shape-stable)
        ...
        for (const k of routeClaims)
            claimed.add(k);
        disjointRoutes.push({ ...route, legs: survivingLegs });
    }
    if (droppedRouteCount > 0 || droppedLegPoolCount > 0 || droppedLegVenueCount > 0) {
        console.log(`  EcoSwap disjoint-route filter: dropped ${droppedRouteCount} route(s) + ${droppedLegPoolCount} ` +
    ...
    const legQlvCount = routes.reduce((s, r) => s + r.legs.reduce((t, l) => t + (l.qlVenues?.length ?? 0), 0), 0);
    ...
    console.log(`  EcoSwap prepared: ${nV3} V3, ${nV4} V4, ${nInf} InfinityCL, ${nV2} V2 direct, ${nKyber} Kyber, ` +
    ...
    const infinityVaultCfg = poolConfig.factories.find((f) => f.factoryType === FactoryType.PancakeInfinityCL && !!f.infinityVault)?.infinityVault;
    if (infinityVaultCfg)
        prepared.infinityVault = infinityVaultCfg;
    if (opts.minOut !== undefined) {
        ...
        prepared.minOut = (expectedTotalOut * BigInt(10_000 - slippageBps)) / 10000n;
    }
    return prepared;
}
