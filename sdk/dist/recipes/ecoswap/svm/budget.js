...
export function familyCuCoefficients(slug) {
 ...
 */
export function planLadders(slots, cuBudget = CU_ADMISSION_BUDGET) {
    if (slots.length === 0)
        throw new Error('planLadders needs at least one slot');
    const warnings = [];
    let admitted = slots.length;
    let rungs = slots.map((slot) => slot.rungs ?? defaultRungsFor(slot.slug));
    const estimate = () => estimateShapeCu(slots.slice(0, admitted).map((slot, i) => ({ slug: slot.slug, rungs: rungs[i] })));
    for (;;) {
        if (estimate() <= cuBudget)
            break;
        ...
        const degrade = (kind) => {
            ...
            for (let i = 0; i < admitted; i++) {
                if (familyCuCoefficients(slots[i].slug).kind !== kind || rungs[i] <= MIN_RUNGS)
                    continue;
                if (pick === -1 || rungs[i] >= rungs[pick])
                    pick = i;
            }
            if (pick === -1)
                return false;
            rungs[pick] -= 1;
            warnings.push(`ecoSwapSvm CU budget: degraded slot ${pick} (${slots[pick].slug}) to ${rungs[pick]} rungs (modeled ${estimate()} CU, budget ${cuBudget})`);
            return true;
        };
        if (degrade('stable') || degrade('cp'))
            continue;
        // Nothing left to degrade — drop the tail slot.
        if (admitted > 1) {
            admitted -= 1;
            rungs = rungs.slice(0, admitted);
            warnings.push(`ecoSwapSvm CU budget: dropped slot ${admitted} (${slots[admitted].slug}) — modeled cost exceeds the ${cuBudget} CU budget`);
            continue;
        }
        throw new Error(`ecoSwapSvm CU budget: a single ${slots[0].slug} slot at ${MIN_RUNGS} rungs models ${estimate()} CU, over the ${cuBudget} CU budget` +
...
export function estimateRouteCu(leg0, leg1) {
 ...
 */
export function planRouteLadders(leg0, leg1, cuBudget = CU_ADMISSION_BUDGET) {
    if (leg0.length === 0 || leg1.length === 0)
        throw new Error('planRouteLadders needs at least one slot per leg');
    const warnings = [];
    ...
    const estimate = () => estimateRouteCu(leg0.slice(0, leg0Admitted).map((slot, i) => ({ slug: slot.slug, rungs: leg0Rungs[i] })), leg1.slice(0, leg1Admitted).map((slot, i) => ({ slug: slot.slug, rungs: leg1Rungs[i] })));
    ...
    const degrade = (kind) => {
        ...
        let pickIndex = -1;
        let pickCurrent = -1;
        const consider = (rungsArr, slots, admitted) => {
            for (let i = 0; i < admitted; i++) {
                if (familyCuCoefficients(slots[i].slug).kind !== kind || rungsArr[i] <= MIN_RUNGS)
                    continue;
        ...
        if (pickRungs === undefined)
            return false;
        pickRungs[pickIndex] -= 1;
        const leg = pickRungs === leg0Rungs ? 0 : 1;
        warnings.push(`ecoSwapSvm route CU budget: degraded leg-${leg} slot ${pickIndex} to ${pickRungs[pickIndex]} rungs (modeled ${estimate()} CU, budget ${cuBudget})`);
        return true;
    };
    ...
    for (;;) {
        ...
        if (drop())
            continue;
        throw new Error(`ecoSwapSvm route CU budget: a minimal 1-slot-per-leg route (${leg0[0].slug} >> ${leg1[0].slug}) at ${MIN_RUNGS} rungs models ${estimate()} CU,` +
