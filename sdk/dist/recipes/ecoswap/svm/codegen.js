...
export const hexLiteral = (bytes) => '0x' + Array.from(bytes, (b) => b.toString(16).padStart(2, '0')).join('');
export const progRef = (slot) => `s${slot}:prog`;
...
export function accountEntry(account) {
    const flags = [
...
export function bindAddress(addressByRef, ref, address) {
...
export function resolveSlotRungs(slot) {
 ...
 */
export function encodeEcoSwapSvmTrade(slots, amountIn, minOut) {
    const words = [amountIn, minOut];
    ...
    words.forEach((word, i) => {
        if (word < 0n || word > U64_MAX)
            throw new Error(`ecoSwapSvm trade word ${i} out of u64 range: ${word}`);
        view.setBigUint64(i * 8, word, true);
    });
    return `0x${Buffer.from(bytes).toString('hex')}`;
}
/** Collects and dedupes the slots' helper functions; one name = one source. */
export function collectHelpers(slots) {
...
export function quoteMode(adapter) {
    if (adapter.emitQuoteCall !== undefined)
        return 'expression';
    if (adapter.emitLadderQuote !== undefined && adapter.emitFinalQuote !== undefined)
        return 'statements';
    throw new Error(`ecoSwapSvm adapter '${adapter.slug}' must implement emitQuoteCall or (emitLadderQuote + emitFinalQuote)`);
}
function generateSource(input) {
    ...
    const lines = [LE8_HELPER, ...collectHelpers(slots)];
    ...
    slots.forEach(({ adapter, cfg }, i) => {
        ...
        for (let j = 1; j <= r; j++) {
            ...
            if (j < r)
                lines.push(`    const ${g} = amountIn >> ${r - j};`);
            if (mode === 'expression') {
    ...
    lines.push('  let remaining = amountIn;', `  for (let it = 0; it < ${totalRungs} && remaining > 0; it++) {`, `    let best = ${k};`, `    for (let s = 0; s < ${k}; s++) {`, '      if (ptr[s] < rl[s]) {', `        if (best === ${k}) { best = s }`, '        if (best !== s) {', '          const c = rb[s] + ptr[s];', '          const b = rb[best] + ptr[best];', '          if (dout[c] * din[b] > dout[b] * din[c]) { best = s }', '        }', '      }', '    }', `    if (best === ${k}) { throw "fill" }`, '    const r = rb[best] + ptr[best];', '    let take = din[r];', '    if (take > remaining) { take = remaining }', '    fill[best] += take;', '    remaining -= take;', '    if (take === din[r]) { ptr[best] = ptr[best] + 1 }', '  }', '  if (remaining > 0) { throw "fill" }');
    ...
    slots.forEach(({ adapter, cfg, swapOverride }, i) => {
        ...
        lines.push(`  if (fill[${i}] > 0 && p${i} > 0) {`, `    const s${i}pfx = Uint8Array.from([${Array.from(template.prefix).join(', ')}]);`, ...(template.suffix.length > 0
            ? [`    const s${i}sfx = Uint8Array.from([${Array.from(template.suffix).join(', ')}]);`]
            : []), `    const s${i}amt = abi.encode(le8(${patched}));`, `    const s${i}cd = ${parts[0]}.concat(${parts.slice(1).join(', ')});`, `    contract.call(${target}, s${i}cd, [${accounts}]);`, '  }');
    });
    lines.push(`  const after = accountUint(${outAta}, 64, 8);`, '  const realized = after - before;', '  if (realized < minOut) { throw "out" }');
 ...
 */
export function ecoSwapSvmShapeKey(slots) {
    return slots
        .map((slot) => {
...
export function generateEcoSwapSvm(input) {
    ...
    for (const key of ['outAta', 'inAta', 'owner']) {
        if (user[key].length === 0)
            throw new Error(`ecoSwapSvm user.${key} ref must not be empty`);
    }
    ...
    if (!argsLayout)
        throw new Error('staged svm compile produced no args layout');
    ...
    slots.forEach(({ adapter, cfg, swapOverride }, i) => {
        for (const account of adapter.quoteRefs(cfg, i))
