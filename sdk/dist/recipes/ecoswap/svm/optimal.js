...
export function solveOptimal(venues, amountIn) {
    if (venues.length === 0)
        throw new Error('solveOptimal needs at least one venue');
    if (amountIn <= 0n)
        throw new Error(`solveOptimal amountIn must be positive, got ${amountIn}`);
 ...
 */
export function efficiencyLoss(optimalOut, quantizedOut) {
    if (optimalOut <= 0n)
        throw new Error('efficiencyLoss needs a positive optimal output');
