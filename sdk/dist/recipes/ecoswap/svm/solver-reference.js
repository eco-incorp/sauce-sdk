...
export function ladderGrid(amountIn, rungs = QL_S) {
    const grid = [];
    for (let j = 1; j <= rungs; j++)
        grid.push(amountIn >> BigInt(rungs - j));
    return grid;
}
/**
 ...
 */
export function buildLadder(quote, amountIn, rungs = QL_S, ladderQuotes) {
 ...
 */
export function solveReference(slots, amountIn) {
    const k = slots.length;
    if (k === 0)
        throw new Error('solveReference needs at least one slot');
    if (amountIn <= 0n)
        throw new Error(`solveReference amountIn must be positive, got ${amountIn}`);
    ...
    const din = new Array(total).fill(0n);
    const dout = new Array(total).fill(0n);
    const ptr = new Array(k).fill(0);
    const fill = new Array(k).fill(0n);
        ...
        for (let s = 0; s < k; s++) {
            if (ptr[s] < rungs[s]) {
                ...
                if (best !== s) {
                    ...
                    if (dout[c] * din[b] > dout[b] * din[c])
                        best = s;
                }
            }
        }
        if (best === k)
            throw new Error('solveReference: no enabled capacity left (mirrors the on-chain "fill" revert)');
    ...
    if (remaining > 0n)
        throw new Error('solveReference: amountIn not fully assigned (mirrors the on-chain "fill" revert)');
...
export function routeReference(leg0, leg1, amountIn) {
    if (leg0.length === 0)
        throw new Error('routeReference needs at least one leg-0 slot');
    if (leg1.length === 0)
        throw new Error('routeReference needs at least one leg-1 slot');
...
//# sourceMappingURL=solver-reference.js.map