...
export const AMP_PRECISION = 1000n;
...
export function calculateInvariant(amp, balances) {
    ...
    for (let it = 0; it < 255; it++) {
        ...
        if (invariant > prev) {
            if (invariant - prev <= 1n)
                break;
        }
        else {
            if (prev - invariant <= 1n)
                break;
        }
    }
    return invariant;
 ...
 */
export function getTokenBalanceGivenInvariant(amp, balances, invariant, tokenIndex) {
    const n = BigInt(balances.length);
    const ampTimesTotal = amp * n;
    let sum = balances[0];
    ...
    for (let it = 0; it < 255; it++) {
        ...
        if (tokenBalance > prev) {
            if (tokenBalance - prev <= 1n)
                break;
        }
        else {
            if (prev - tokenBalance <= 1n)
                break;
        }
    }
    return tokenBalance;
 ...
 */
function calcOutGivenInUpscaled(amp, balances, i, j, amountInUp, invariant) {
 ...
 * backward-merge).
 */
export function buildBalancerStableSegments(pool, amountIn, samples = BALANCER_STABLE_SAMPLES) {
    if (amountIn <= 0n)
        return [];
 ...
 */
export function buildBalancerStableQLLadder(pool, amountIn) {
    return buildQLLadder((dx) => getDy(pool, dx), amountIn);
}
//# sourceMappingURL=balancer-stable-math.js.map