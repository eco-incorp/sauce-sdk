 ...
 */
export function balancerV3SampleInputs(amountIn, samples = BALANCER_V3_SAMPLES) {
    if (amountIn <= 0n)
        return [];
 ...
 */
export function buildBalancerV3Segments(pool, amountIn, _samples = BALANCER_V3_SAMPLES) {
    if (amountIn <= 0n)
        return [];
    if (!pool.cumIn || !pool.cumOut)
        return [];
 ...
 */
export function computeRateRoundUp(rate) {
 ...
 */
export function balancerV3StableOut2(amp, feeWad, bIn, bOut, dxScaled) {
    if (dxScaled <= 0n || bIn <= 0n || bOut <= 0n)
        return 0n;
 ...
 */
export function balancerV3StableGetDy(pool, dxRaw) {
    if (dxRaw <= 0n)
        return 0n;
 ...
 */
export function buildBalancerV3QLLadder(pool, amountIn) {
    return buildQLLadder((dx) => balancerV3StableGetDy(pool, dx), amountIn);
}
//# sourceMappingURL=balancer-v3-math.js.map