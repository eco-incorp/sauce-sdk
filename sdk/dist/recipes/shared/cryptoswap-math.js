...
export const FEE_DENOMINATOR_CRYPTO = 10000000000n;
 ...
 * on-chain).
 */
export function newtonD(A, _gamma, x) {
    if (x[0] <= 0n || x[1] <= 0n)
        return 0n;
    const S = x[0] + x[1];
    if (S === 0n)
 ...
 */
export function cryptoFee(pool, xp) {
    const S = xp[0] + xp[1];
    if (S === 0n)
        return pool.midFee;
 ...
 */
export function getDyCrypto(pool, dx) {
    if (dx <= 0n)
        return 0n;
    ...
    if (y + 1n >= xp[pool.j])
        return 0n; // view: assert y < xp[j] (revert ⇒ unusable quote)
 ...
 * backward-merge).
 */
export function buildCryptoSwapSegments(pool, amountIn, samples = CRYPTOSWAP_SAMPLES) {
    if (amountIn <= 0n)
        return [];
 ...
 */
export function buildCryptoSwapQLLadder(pool, amountIn) {
    return buildQLLadder((dx) => getDyCrypto(pool, dx), amountIn);
}
//# sourceMappingURL=cryptoswap-math.js.map