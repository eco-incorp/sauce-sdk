...
export const FEE_DENOMINATOR_CURVE = 10000000000n;
...
function xpFromBalances(balances, rates) {
    const xp = new Array(balances.length);
 ...
 */
export function getD(xp, amp, aPrecision) {
 ...
 */
export function getY(i, j, x, xp, amp, aPrecision) {
    const N = BigInt(xp.length);
 ...
 */
export function dynamicFee(xpi, xpj, fee, offpeg) {
    if (offpeg <= FEE_DENOMINATOR_CURVE)
        return fee;
...
export function getDy(pool, dx) {
    ...
    const feeRate = pool.offpegFeeMultiplier !== undefined
        ? dynamicFee((xp[pool.i] + x) / 2n, (xp[pool.j] + y) / 2n, pool.feePpm10, pool.offpegFeeMultiplier)
        : pool.feePpm10;
 ...
 */
export function qlSliceHead(sliceOut, capacity) {
    if (capacity <= 0n || sliceOut <= 0n)
        return 0n;
 ...
 */
export function qlLadderInputs(amountIn, seedFloor = 0n) {
    if (amountIn <= 0n)
        return [];
...
export function buildQLLadder(getDy, amountIn, seedFloor = 0n, flatLadder = false) {
    if (amountIn <= 0n)
        return [];
    let seed = amountIn / QL_SEED_DIV;
    if (seed <= 0n)
 ...
 */
export function buildCurveQLLadder(pool, amountIn) {
 ...
 */
export function buildCurveSegments(pool, amountIn, samples = CURVE_SAMPLES) {
    if (amountIn <= 0n)
        return [];
