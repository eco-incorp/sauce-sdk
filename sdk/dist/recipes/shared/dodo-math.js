...
function mulFloor(a, b) {
    return (a * b) / DODO_ONE;
}
/** Plain integer ceil-divide ceil(a/b). */
function divCeilRaw(a, b) {
 ...
 */
export function generalIntegrate(V0, V1, V2, i, k) {
    if (V0 <= 0n)
        return 0n;
    const fairAmount = i * (V1 - V2);
    if (k === 0n)
        return fairAmount / DODO_ONE;
 ...
 */
export function solveQuadraticForTrade(V0, V1, delta, i, k) {
    if (V0 <= 0n)
        return 0n;
    if (delta === 0n)
        return 0n;
    ...
    if (bSig) {
        ...
        if (numerator === 0n)
            return 0n; // the contract reverts ("should not be zero")
    }
    else {
 ...
 */
function rOneSellBase(i, K, Q0, payBase) {
 ...
 */
function rAboveSellBase(i, K, B0, B, payBase) {
...
function rBelowSellQuote(i, K, Q0, Q, payQuote) {
 ...
 */
function sellBaseToken(s, payBase) {
    if (s.R === 0 /* RState.ONE */) {
        return rOneSellBase(s.i, s.K, s.Q0, payBase);
    }
    if (s.R === 1 /* RState.ABOVE_ONE */) {
        ...
        if (payBase < backToOnePayBase) {
            ...
            if (recv > backToOneReceiveQuote)
                recv = backToOneReceiveQuote;
            return recv;
        }
 ...
 */
function sellQuoteToken(s, payQuote) {
    if (s.R === 0 /* RState.ONE */) {
        return rOneSellQuote(s.i, s.K, s.B0, payQuote);
    }
    if (s.R === 2 /* RState.BELOW_ONE */) {
        ...
        if (payQuote < backToOnePayQuote) {
            ...
            if (recv > backToOneReceiveBase)
                recv = backToOneReceiveBase;
            return recv;
        }
 ...
 */
export function getDy(pool, payAmount) {
    if (payAmount <= 0n)
        return 0n;
 ...
 */
export function buildDodoSegments(pool, amountIn, samples = DODO_SAMPLES) {
    if (amountIn <= 0n)
        return [];
 ...
 */
export function buildDodoQLLadder(pool, amountIn) {
...
export function dodoFeeToPpm(lpFeeRate, mtFeeRate) {
    const fee = lpFeeRate + mtFeeRate; // 1e18-scaled
    return Number((fee * 1000000n + DODO_ONE / 2n) / DODO_ONE);
}
//# sourceMappingURL=dodo-math.js.map