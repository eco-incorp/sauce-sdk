 ...
 */
export function eulerFeeToPpm(feeWad) {
    return Number((feeWad * 1000000n + EULER_ONE / 2n) / EULER_ONE);
}
 ...
 *   out = y0 + ceil(v / py)
 */
function f(x, px, py, x0, y0, c) {
    if (x >= x0)
        return y0; // at/past equilibrium on this side ⇒ the curve's y floor (f domain is x <= x0)
 ...
 */
function fInverse(y, px, py, x0, y0, cx) {
 ...
 */
function findCurvePointIn(pool, dxNet) {
 ...
 */
export function computeQuote(pool, dx) {
    if (dx <= 0n)
        return 0n;
 ...
 * backward-merge).
 */
export function buildEulerSwapSegments(pool, amountIn, samples = EULERSWAP_SAMPLES) {
    if (amountIn <= 0n)
        return [];
 ...
 */
export function buildEulerSwapQLLadder(pool, amountIn) {
    ...
    return buildQLLadder((dx) => {
        if (inCap > 0n && dx > inCap)
            return 0n;
        const out = computeQuote(pool, dx);
        if (outCap > 0n && out > outCap)
            return 0n;
        return out;
    }, amountIn);
}
//# sourceMappingURL=eulerswap-math.js.map