...
export declare const FLUID_FEE_SCALE: bigint;
/**
 * One Fluid DEX venue DESCRIPTOR (a FluidDexT1 pool + a direct tokenIn→tokenOut leg), oriented for the
 * swap. This is ALL prepare ships (the QL family contract: descriptor-only, zero sampled values) — the
 * on-chain solver builds the ladder LIVE from the resolver quote at cook.
 */
export interface FluidVenue {
    /** DexT1 pool address — the swapIn / approve target AND the resolver `dex_` arg. */
    address: `0x${string}`;
    /** DexReservesResolver address — the estimateSwapIn quote target (a CALL — see the module header). */
    resolver: `0x${string}`;
    ...
    tokenOut: `0x${string}`;
    /**
     * Effective per-pool fee in ppm, DERIVED from the liveness probe for price-ordering / diagnostics only
...
/**
 * The ORACLE/REFERENCE model of one Fluid venue: the descriptor plus a `getDy` QUOTE model — the
 * cumulative `estimateSwapIn(dex, swap0to1, dx, 0)` out for total input `dx`. Fluid has NO off-chain
 * closed form (the real pool prices off the shared Liquidity layer), so the model is caller-supplied:
 *   - the local EVM tests replay the FluidDexPool.sol fixture math bit-for-bit (like the WOOFi/Mento
 *     fixture models), and
 *   - the prod-mirror prefetches the REAL etched resolver's quotes at the DETERMINISTIC direct-venue QL
 *     grid (`fluidQLGridInputs`) and answers by exact-point lookup.
 * `getDy` must return 0 at/past the tradeable cap (mirroring the graceful resolver), so the ladder
 * self-truncates in lockstep with the on-chain build.
 */
export interface FluidPool extends FluidVenue {
    /** Cumulative quote model: out for TOTAL input dx (post-fee + post-cap; 0 ⇒ not fillable). */
 ...
 */
export declare function buildFluidQLLadder(pool: FluidPool, amountIn: bigint): MergeSegment[];
//# sourceMappingURL=fluid-math.d.ts.map