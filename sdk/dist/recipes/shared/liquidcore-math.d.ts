...
export declare const LIQUIDCORE_FEE_SCALE: bigint;
/** The HyperEVM BBO read-precompile address LiquidCore pools price off (input-keyed mock target in tests). */
export declare const HYPEREVM_BBO_PRECOMPILE: Hex;
/**
 * One LiquidCore venue DESCRIPTOR (a per-pair pool, oriented for the swap). This is ALL prepare
 * ships (the QL family contract: descriptor-only, zero sampled values) — the on-chain solver builds
 * the ladder LIVE from pool.estimateSwap at cook and executes approve-POOL + pool.swap.
 */
export interface LiquidCoreVenue {
    /** Pool address — the per-pair inventory proxy (the estimateSwap/swap/approve target + the claim key). */
    address: Hex;
    /** The router that enumerated this pool (getPoolForPair) — diagnostics only; the hot path never calls it. */
    router: Hex;
    /** The venue's tokenIn (the edge from-token; the estimateSwap/swap `tokenIn` arg). */
    tokenIn: Hex;
    /** The venue's tokenOut (the edge to-token; the estimateSwap/swap `tokenOut` arg). */
    tokenOut: Hex;
    /**
     * Diagnostic fee/spread in ppm derived at discovery from the probe quote vs the pool's spot
     * (price-ordering hint only — the quote folds the spread + the adaptive imbalance fee in; there
     * is no flat fee getter). 0 when unknown.
     */
    feePpm: number;
...
/**
 * The ORACLE/REFERENCE model of one LiquidCore venue: the descriptor plus a `getDy` QUOTE model —
 * the cumulative out that `pool.estimateSwap(tokenIn, tokenOut, dx)` returns for total input `dx`.
 * LiquidCore pool implementations are unverified bytecode, so the model is caller-supplied:
 *   - the local EVM tests replay the LiquidCorePool.sol fixture (BBO-mock-priced) bit-for-bit, and
 *   - the prod-mirror prefetches the REAL etched pool's quotes at the DETERMINISTIC QL grid
 *     (`qlLadderInputs`) and answers by exact-point lookup.
 * `getDy` must return 0 where the real view reverts (zero amount / unsupported pair) or returns 0
 * (drained output inventory) — the ladder then self-truncates in lockstep with the on-chain
 * probe-then-decode build. An OVERSIZED dx must return the real view's CAPPED asymptotic quote
 * (the ladder's non-descending-head guard then truncates where the marginal collapses).
 */
export interface LiquidCorePool extends LiquidCoreVenue {
 ...
 */
export declare function buildLiquidCoreQLLadder(pool: LiquidCorePool, amountIn: bigint): MergeSegment[];
//# sourceMappingURL=liquidcore-math.d.ts.map