...
export const MAV_ONE = 1000000000000000000n;
...
function mulDown(x, y) {
    return (x * y) / MAV_ONE;
}
function divDown(x, y) {
    return (x * MAV_ONE) / y;
}
function divUp(x, y) {
    return (x * MAV_ONE + y - 1n) / y;
}
function mulDivDown(x, y, k) {
    if (k === 0n)
        k = 1n;
    return (x * y) / k;
}
function mulDivCeil(x, y, k) {
    if (k === 0n)
        k = 1n;
    const r = (x * y) / k;
    return (x * y) % k !== 0n ? r + 1n : r;
}
function mulFloor(x, y) {
    return mulDivDown(x, y, MAV_ONE);
}
function invFloor(x) {
    return ONE_SQUARED / x;
}
function invCeil(denominator) {
 ...
 */
export function tickSqrtPrice(tickSpacing, tick) {
    const absTick = BigInt(Math.abs(tick) * tickSpacing);
    if (absTick > BigInt(MAX_TICK))
        throw new Error(`TickMaxExceeded: ${tick}`);
    let ratio = absTick & 0x1n ? 0xfffcb933bd6fad9d3af5f0b9f25db4d6n : 0x100000000000000000000000000000000n;
    if (absTick & 0x2n)
...
export function tickSqrtPrices(tickSpacing, tick) {
    return {
 ...
 */
export function getTickL(reserveA, reserveB, sqrtLowerTickPrice, sqrtUpperTickPrice) {
    ...
    if (rB === 0n)
        return divDown(rA, diff) >> precisionBump;
    if (rA === 0n)
        return mulDivDown(mulDown(rB, sqrtLowerTickPrice), sqrtUpperTickPrice, diff) >> precisionBump;
    ...
    const sqrtInner = isqrt(inner) * 1000000000n;
 ...
 */
export function getSqrtPrice(reserveA, reserveB, sqrtLowerTickPrice, sqrtUpperTickPrice, liquidity) {
    if (reserveA === 0n)
    ...
    if (sqrtPrice < sqrtLowerTickPrice)
        return sqrtLowerTickPrice;
    if (sqrtPrice > sqrtUpperTickPrice)
        return sqrtUpperTickPrice;
    return sqrtPrice;
}
/** amountToBinNetOfProtocolFee — clip(deltaInErc, ceil(feeBasis·protocolFeeD3/1000)); unused for out. */
function amountToBinNetOfProtocolFee(deltaInErc, feeBasis, protocolFeeD3) {
    if (protocolFeeD3 === 0n)
        return deltaInErc;
 ...
 */
export function computeSwapExactIn(sqrtPrice, tickData, amountIn, tokenAIn, fee, protocolFeeD3, sqrtLowerTickPrice, sqrtUpperTickPrice) {
    ...
    if (tokenAIn) {
        ...
        drainable = denom > 0n;
        if (drainable)
            binAmountIn = mulDivCeil(availableOutput, sqrtPrice, denom);
    }
    else {
        ...
        drainable = denom > 0n;
        if (drainable)
            binAmountIn = divUp(availableOutput, denom);
    }
    let deltaInErc;
    let feeBasis;
    let excess = 0n;
    let binAmountInFinal;
    ...
    if (tokenAIn) {
        ...
        if (endSqrtPrice > sqrtUpperTickPrice)
            endSqrtPrice = sqrtUpperTickPrice;
    }
    else {
        const inv = divDown(binAmountInFinal, L) + invFloor(sqrtPrice);
        endSqrtPrice = inv > 0n ? invFloor(inv) : sqrtLowerTickPrice;
        if (endSqrtPrice < sqrtLowerTickPrice)
            endSqrtPrice = sqrtLowerTickPrice;
    }
...
export function engineTickLimit(tokenAIn) {
 ...
 */
export function simulateMaverickExactIn(pool, amountIn, tickLimit = engineTickLimit(pool.tokenAIn)) {
    if (amountIn <= 0n)
    ...
    while (remainingIn > 0n && iterations < TICK_SEARCH_LIMIT) {
        ...
        if (tokenAIn && currentTick > tickLimit)
            break;
        if (!tokenAIn && currentTick < tickLimit)
            break;
        ...
        const result = computeSwapExactIn(currentSqrtPrice, { currentReserveA: ts.reserveA, currentReserveB: ts.reserveB, currentLiquidity: liquidity }, remainingIn, tokenAIn, fee, protocolFeeD3, sqrtLowerPrice, sqrtUpperPrice);
        totalOut += result.deltaOutErc;
        remainingIn = result.excess;
        currentSqrtPrice = result.endSqrtPrice;
        if (!result.swappedToMaxPrice || remainingIn === 0n)
            break;
        currentTick += direction;
        iterations++;
    }
 ...
 */
export function buildMaverickWalkLadder(pool, amountIn, maxSegments = MAVERICK_WALK_MAX_SEGMENTS) {
    if (amountIn <= 0n)
        return [];
    const { tokenAIn, tickSpacing, fee, protocolFeeD3 } = pool;
    ...
    while (remainingIn > 0n && iterations < TICK_SEARCH_LIMIT) {
        ...
        if (capacity > 0n && effOut > 0n) {
            ...
            if (head <= 0n)
                break;
            if (segs.length > 0 && head >= prevHead)
                break; // non-descending guard (mirrors QL emit)
 ...
 */
export function getDy(pool, amountIn) {
 ...
 */
export function maxInput(pool, probe) {
 ...
 */
export function buildMaverickSegments(pool, amountIn, samples = MAVERICK_SAMPLES) {
    if (amountIn <= 0n)
        return [];
...
export function maverickFeeToPpm(feeWad) {
    return Number((feeWad * 1000000n + MAV_ONE / 2n) / MAV_ONE);
}
//# sourceMappingURL=maverick-math.js.map