...
export const PANCAKE_STABLE_FEE_DENOMINATOR = 10000000000n;
/**
 ...
 */
function pancakeStableDyRaw(state, i, j, dx) {
 ...
 */
export function pancakeStableGetDy(state, i, j, dx) {
    if (dx <= 0n)
        return 0n;
 ...
 */
export function pancakeStableExchangeDy(state, i, j, dx) {
    if (dx <= 0n)
        return 0n;
    if (state.balances.some((b) => b <= 0n))
        return 0n;
 ...
 */
export function buildPancakeStableQLLadder(pool, amountIn) {
    return buildQLLadder((dx) => pool.getDy(dx), amountIn);
}
//# sourceMappingURL=pancakestable-math.js.map