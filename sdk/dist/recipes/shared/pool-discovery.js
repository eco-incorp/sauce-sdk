...
import { BASE_CHAIN_POOL_CONFIG, SwapPoolType, FactoryType, TRADER_JOE_BIN_STEPS, TRADER_JOE_BIN_WINDOW, TRADER_JOE_DEFAULT_BASE_FACTOR, SLIPSTREAM_TICK_SPACINGS, V2_DEFAULT_FEE_PPM, feeToTickSpacing, hasPriceLimit, } from "./constants.js";
import { A_PRECISION_DEFAULT, QL_SEED_DIV, qlSliceHead } from "./curve-math.js";
import { FEE_DENOMINATOR_CRYPTO } from "./cryptoswap-math.js";
import { dodoFeeToPpm } from "./dodo-math.js";
import { WAD as WOMBAT_WAD } from "./wombat-math.js";
import { WOO_FEE_SCALE } from "./woofi-math.js";
import { fermiSampleInputs, FERMI_FEE_SCALE } from "./fermi-math.js";
import { FLUID_FEE_SCALE } from "./fluid-math.js";
import { TESSERA_FEE_SCALE } from "./tessera-math.js";
import { ELFOMO_FEE_SCALE } from "./elfomo-math.js";
import { METRIC_FEE_SCALE, METRIC_INT128_MAX, METRIC_LIMIT_MAX_U128, } from "./metric-math.js";
import { SIZE_PRECISION } from "./size-math.js";
import { PANCAKE_STABLE_FEE_DENOMINATOR } from "./pancakestable-math.js";
import { EKUBO_DEFAULT_PRESETS, EKUBO_INT128_MAX, EKUBO_SLOAD_SELECTOR, ekuboConcentratedConfig, ekuboPoolId, ekuboFeePpm, decodeEkuboBalanceUpdate, } from "./ekubo-math.js";
import { INFINITY_DEFAULT_CL_PRESETS, INFINITY_DYNAMIC_FEE_FLAG, INFINITY_MAX_FEE_PPM, INFINITY_RETURNS_DELTA_MASK, computeInfinityPoolId, combineInfinityFee, decodeInfinityCLTickSpacing, encodeInfinityCLParameters, } from "./infinity-math.js";
import { mentoSampleInputs, MENTO_FEE_SCALE } from "./mento-math.js";
import { balancerV3SampleInputs, balancerV3StableGetDy, BALANCER_V3_FEE_SCALE, } from "./balancer-v3-math.js";
import { eulerFeeToPpm } from "./eulerswap-math.js";
import { getSqrtPrice as getMaverickSqrtPrice, getTickL, tickSqrtPrices, maverickFeeToPpm, } from "./maverick-math.js";
// ── ABIs ──────────────────────────────────────────────────────
...
async function readReturnWords(client, to, data, minWords) {
    try {
        const { data: ret } = await client.call({ to, data });
        if (!ret)
            return undefined;
        const hex = ret.slice(2);
        if (hex.length === 0 || hex.length % 64 !== 0 || hex.length / 64 < minWords)
            return undefined;
        const words = [];
 ...
 */
async function tolerantSqrtPrices(client, pools, slot0Results, liquidityResults) {
    const prices = pools.map((_, i) => {
        const s = slot0Results[i];
        return s.status === "success" ? s.result[0] : undefined;
    });
    await Promise.all(pools.map(async (p, i) => {
        if (prices[i] !== undefined || liquidityResults[i].status !== "success")
            return;
...
function guessAlgebraFeeLayout(wordCount) {
 ...
 */
function algebraFeeAt(words, layout, zeroForOne) {
...
function computeV4PoolId(currency0, currency1, fee, tickSpacing, hooks) {
    return keccak256(encodeAbiParameters([{ type: "address" }, { type: "address" }, { type: "uint24" }, { type: "int24" }, { type: "address" }], [currency0, currency1, fee, tickSpacing, hooks]));
}
// ── New protocol ABIs ────────────────────────────────────────
...
async function discoverV3Pools(tokenIn, tokenOut, client, factories, feeTiers) {
    ...
    const getPoolCalls = factories.flatMap((f) => (f.feeTiers ?? feeTiers).map((fee) => ({
        address: f.address,
        abi: v3FactoryAbi,
    ...
    for (let i = 0; i < validPools.length; i++) {
        ...
        if (sqrtPriceX96 === undefined || liq.status !== "success")
            continue;
        const liquidity = liq.result;
...
async function discoverSlipstreamCLPools(tokenIn, tokenOut, client, factories) {
    ...
    const getPoolCalls = factories.flatMap((f) => (f.slipstreamTickSpacings ?? [...SLIPSTREAM_TICK_SPACINGS]).map((tickSpacing) => ({
        address: f.address,
    ...
    for (let i = 0; i < validPools.length; i++) {
        ...
        const fee = feeRes.status === "success" ? Number(feeRes.result) : 0;
        pools.push({
            address: validPools[i].address,
            tokenIn,
            tokenOut,
...
async function discoverAlgebraPools(tokenIn, tokenOut, client, factories) {
    ...
    for (let i = 0; i < poolAddresses.length; i++) {
        ...
        if (result.status === "success" &&
            result.result &&
            result.result !== ZERO_ADDRESS) {
            validPools.push({ address: result.result, factory: factories[i] });
        }
    }
    if (validPools.length === 0)
        return [];
    ...
    const states = validPools.map((p, i) => {
        ...
        return { price: r[0], fee: algebraFeeAt(r, layout, zeroForOne) };
    });
    await Promise.all(validPools.map(async (p, i) => {
        if (states[i] !== undefined || liquidityResults[i].status !== "success")
            return;
    ...
    for (let i = 0; i < validPools.length; i++) {
        ...
        const liq = liquidityResults[i];
        if (state === undefined || liq.status !== "success")
            continue;
        const liquidity = liq.result;
        if (state.price === 0n || liquidity === 0n)
            continue;
        pools.push({
            address: validPools[i].address,
            tokenIn,
 ...
 */
export async function discoverAlgebraPoolAddresses(tokenIn, tokenOut, client, factories) {
    const out = new Set();
    if (factories.length === 0)
        return out;
    ...
    for (const r of results) {
        if (r.status === "success" && r.result && r.result !== ZERO_ADDRESS) {
            out.add(r.result.toLowerCase());
        }
    }
    return out;
}
// ── V2 Standard discovery ───────────────────────────────────
async function discoverV2Pools(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    ...
    for (let i = 0; i < pairAddresses.length; i++) {
        ...
        if (result.status === "success" &&
            result.result &&
            result.result !== ZERO_ADDRESS) {
            validPairs.push({ address: result.result, factory: factories[i] });
        }
    }
...
async function discoverSolidlyV2Pools(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    ...
    const calls = factories.flatMap((f) => [
        {
            ...
            functionName: "getPool",
            args: [tokenIn, tokenOut, false], // volatile
            factory: f,
            stable: false,
        },
        {
            ...
            abi: solidlyFactoryAbi,
            functionName: "getPool",
            args: [tokenIn, tokenOut, true], // stable
            factory: f,
            stable: true,
        },
    ]);
    ...
    for (let i = 0; i < poolAddresses.length; i++) {
        ...
        if (result.status === "success" && result.result && result.result !== ZERO_ADDRESS) {
            const addr = result.result.toLowerCase();
            if (!seen.has(addr)) {
                seen.add(addr);
                const label = calls[i].stable ? `${calls[i].factory.label} (stable)` : calls[i].factory.label;
                validPairs.push({ address: result.result, factory: { ...calls[i].factory, label } });
            }
        }
    }
    if (validPairs.length === 0)
...
function solidlyFeeToPpm(fee) {
    if (fee === undefined)
        return SOLIDLY_STABLE_DEFAULT_FEE_PPM;
    const n = Number(fee);
    if (n === 0)
        return SOLIDLY_STABLE_DEFAULT_FEE_PPM;
 ...
 */
export async function discoverSolidlyStablePoolsTyped(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    ...
    for (let i = 0; i < addrResults.length; i++) {
        ...
        if (seen.has(key))
            continue;
        seen.add(key);
        valid.push({ address: r.result, factory: factories[i] });
    }
    if (valid.length === 0)
        return [];
    const [token0Results, reserveResults, feeResults] = await Promise.all([
        client.multicall({
            contracts: valid.map((p) => ({ address: p.address, abi: solidlyStablePoolAbi, functionName: "token0" })),
            allowFailure: true,
        }),
        client.multicall({
            contracts: valid.map((p) => ({ address: p.address, abi: solidlyStablePoolAbi, functionName: "getReserves" })),
            allowFailure: true,
        }),
        client.multicall({
            contracts: valid.map((p) => ({ address: p.factory.address, abi: solidlyFactoryAbi, functionName: "getFee", args: [p.address, true] })),
            allowFailure: true,
        }),
    ]);
    ...
    for (let i = 0; i < valid.length; i++) {
        ...
        if (t0.status !== "success" || reserves.status !== "success")
            continue;
        const [reserve0, reserve1] = reserves.result;
        if (reserve0 === 0n || reserve1 === 0n)
            continue;
        const token0 = t0.result;
        ...
        pools.push({
            ...
            token0,
            inIsToken0,
            feePpm: solidlyFeeToPpm(feeResults[i].status === "success" ? feeResults[i].result : undefined),
 ...
 */
export async function discoverSolidlyVolatilePoolsTyped(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    const addrResults = await client.multicall({
        contracts: factories.map((f) => ({
            ...
            abi: solidlyFactoryAbi,
            functionName: "getPool",
            args: [tokenIn, tokenOut, false], // volatile (vAMM)
        })),
        allowFailure: true,
    });
    const seen = new Set();
    ...
    const [stableResults, token0Results, reserveResults, feeResults] = await Promise.all([
        client.multicall({
            contracts: valid.map((p) => ({ address: p.address, abi: solidlyStablePoolAbi, functionName: "stable" })),
            allowFailure: true,
        }),
        client.multicall({
            contracts: valid.map((p) => ({ address: p.address, abi: solidlyStablePoolAbi, functionName: "token0" })),
        ...
        client.multicall({
            contracts: valid.map((p) => ({ address: p.factory.address, abi: solidlyFactoryAbi, functionName: "getFee", args: [p.address, false] })),
            allowFailure: true,
        }),
    ]);
    const pools = [];
    for (let i = 0; i < valid.length; i++) {
        ...
        if (st.status !== "success" || st.result === true)
            continue;
        if (t0.status !== "success" || reserves.status !== "success")
            continue;
        const [reserve0, reserve1] = reserves.result;
        ...
        pools.push({
            ...
            feePpm: feeResults[i].status === "success"
                ? solidlyFeeToPpm(feeResults[i].result)
                : V2_DEFAULT_FEE_PPM,
...
async function readV2PoolState(tokenIn, tokenOut, client, validPairs) {
    ...
    for (let i = 0; i < validPairs.length; i++) {
        const reserves = reserveResults[i];
        const t0 = token0Results[i];
        if (reserves.status !== "success" || t0.status !== "success")
            continue;
        const [reserve0, reserve1] = reserves.result;
        const token0 = t0.result.toLowerCase();
        if (reserve0 === 0n || reserve1 === 0n)
            continue;
...
// ── Curve discovery ─────────────────────────────────────────
async function discoverCurvePools(tokenIn, tokenOut, client, registries) {
    ...
    for (const registry of registries) {
        try {
            ...
            const [bal0, bal1] = await Promise.all([
                client.readContract({ address: poolAddr, abi: curvePoolAbi, functionName: "balances", args: [0n] }).catch(() => 0n),
                client.readContract({ address: poolAddr, abi: curvePoolAbi, functionName: "balances", args: [1n] }).catch(() => 0n),
            ]);
            if (bal0 === 0n || bal1 === 0n)
                continue;
 ...
 */
export async function discoverCurvePoolsTyped(tokenIn, tokenOut, client, registries) {
    if (registries.length === 0)
        return [];
    const pools = [];
    for (const registry of registries) {
        try {
            ...
            const [A, feeRaw, offpegRaw] = await Promise.all([
                client.readContract({ address: poolAddr, abi: curvePoolAbi, functionName: "A" }),
                client.readContract({ address: poolAddr, abi: curvePoolAbi, functionName: "fee" }),
                client
                    .readContract({ address: poolAddr, abi: curvePoolAbi, functionName: "offpeg_fee_multiplier" })
                    .then((v) => v)
                    .catch(() => undefined),
            ]);
            const balances = await Promise.all(Array.from({ length: N }, (_, k) => client.readContract({
                address: poolAddr,
                abi: curvePoolAbi,
            ...
            if (decimalsRaw && decimalsRaw.length >= N) {
                ...
                const coinAddrs = await Promise.all(Array.from({ length: N }, (_, k) => client.readContract({
                    address: poolAddr,
                    abi: curvePoolAbi,
                    functionName: "coins",
                    args: [BigInt(k)],
                })));
                decimals = await Promise.all(coinAddrs.map((addr) => client
                    .readContract({
                    address: addr,
...
function cryptoFeeToPpm(midFee) {
    return Number((midFee * 1000000n + FEE_DENOMINATOR_CRYPTO / 2n) / FEE_DENOMINATOR_CRYPTO);
}
/**
 ...
 */
export async function discoverCryptoSwapPoolsTyped(tokenIn, tokenOut, client, registries) {
    if (registries.length === 0)
        return [];
    const pools = [];
    for (const registry of registries) {
        try {
            ...
            const [A, gamma, priceScale, D, midFee, outFee, feeGamma] = await Promise.all([
                client.readContract({ address: poolAddr, abi: cryptoPoolAbi, functionName: "A" }),
                client.readContract({ address: poolAddr, abi: cryptoPoolAbi, functionName: "gamma" }),
                client.readContract({ address: poolAddr, abi: cryptoPoolAbi, functionName: "price_scale" }),
                client.readContract({ address: poolAddr, abi: cryptoPoolAbi, functionName: "D" }),
                client.readContract({ address: poolAddr, abi: cryptoPoolAbi, functionName: "mid_fee" }),
                client.readContract({ address: poolAddr, abi: cryptoPoolAbi, functionName: "out_fee" }),
                client.readContract({ address: poolAddr, abi: cryptoPoolAbi, functionName: "fee_gamma" }),
            ]);
            const balances = await Promise.all(Array.from({ length: N }, (_, k) => client.readContract({ address: poolAddr, abi: cryptoPoolAbi, functionName: "balances", args: [BigInt(k)] })));
            ...
            if (decimalsRaw && decimalsRaw.length >= N) {
                ...
                const coinAddrs = await Promise.all(Array.from({ length: N }, (_, k) => client.readContract({ address: poolAddr, abi: cryptoPoolAbi, functionName: "coins", args: [BigInt(k)] })));
                decimals = await Promise.all(coinAddrs.map((addr) => client
...
async function discoverBalancerV2Pools(tokenIn, tokenOut, _client, factories) {
 ...
 */
export async function discoverBalancerStablePoolsTyped(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
        ...
        for (const poolAddr of knownPools) {
            ...
            try {
                ...
                const [poolId, ampRaw, scalingRaw, feeRaw] = await Promise.all([
                    client.readContract({ address: poolAddr, abi: balancerStablePoolAbi, functionName: "getPoolId" }),
                    client.readContract({ address: poolAddr, abi: balancerStablePoolAbi, functionName: "getAmplificationParameter" }),
                    client.readContract({ address: poolAddr, abi: balancerStablePoolAbi, functionName: "getScalingFactors" }),
                    client.readContract({ address: poolAddr, abi: balancerStablePoolAbi, functionName: "getSwapFeePercentage" }),
                ]);
                if (!poolId || poolId === ZERO_BYTES32)
                    continue;
                let bptIndex = -1;
                try {
                    bptIndex = Number((await client.readContract({ address: poolAddr, abi: balancerStablePoolAbi, functionName: "getBptIndex" })));
                }
                catch {
                ...
                if (i < 0 || j < 0)
                    continue; // pool does not hold BOTH non-BPT tokens
...
// ── DODO V2 discovery ───────────────────────────────────────
async function discoverDODOPools(tokenIn, tokenOut, client, zoos) {
    if (zoos.length === 0)
        return [];
    const pools = [];
    for (const zoo of zoos) {
        ...
        let reverts = 0;
        for (const [base, quote] of [[tokenIn, tokenOut], [tokenOut, tokenIn]]) {
            let addresses;
            try {
            ...
            for (const addr of addresses) {
                ...
                try {
                    const [baseReserve, quoteReserve] = await Promise.all([
                        client.readContract({ address: addr, abi: dodoPoolAbi, functionName: "_BASE_RESERVE_" }),
                        client.readContract({ address: addr, abi: dodoPoolAbi, functionName: "_QUOTE_RESERVE_" }),
                    ]);
        ...
        if (reverts === 2) {
            console.warn(`[dodo] getDODOPool reverted on both orderings for ${zoo.label} @ ${zoo.address} ` +
 ...
 */
export async function discoverDodoV2PoolsTyped(tokenIn, tokenOut, client, zoos, caller = ZERO_ADDRESS) {
    if (zoos.length === 0)
        return [];
    const pools = [];
    const seen = new Set();
            ...
            for (const addr of addresses) {
                ...
                try {
                    ...
                    const [stateRaw, baseTokenRaw, quoteTokenRaw, lpFeeRaw, mtModelRaw] = await Promise.all([
                        ...
                        client.readContract({ address: pool, abi: dodoPmmAbi, functionName: "_BASE_TOKEN_" }),
                        client.readContract({ address: pool, abi: dodoPmmAbi, functionName: "_QUOTE_TOKEN_" }),
                        client
                    ...
                    if (!sellBase && inLower !== quoteToken.toLowerCase())
                        continue;
                    // An empty pool on the side being sold has no depth.
                    if (sellBase ? Q <= 0n : B <= 0n)
                        continue;
                    const R = Number(Rraw) === 1 ? 1 /* RState.ABOVE_ONE */ : Number(Rraw) === 2 ? 2 /* RState.BELOW_ONE */ : 0 /* RState.ONE */;
                    pools.push({
                        poolType: SwapPoolType.DODOV2,
                        address: pool,
                        baseToken,
...
async function discoverTraderJoeLBPools(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    const pools = [];
    for (const factory of factories) {
        ...
        const binSteps = factory.lbBinSteps ?? TRADER_JOE_BIN_STEPS;
        const calls = binSteps.map((binStep) => ({
            address: factory.address,
            abi: traderJoeLBFactoryAbi,
            functionName: "getLBPairInformation",
            args: [tokenIn, tokenOut, BigInt(binStep)],
            binStep,
        }));
        const results = await client.multicall({
            contracts: calls.map((c) => ({
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            if (result.status !== "success")
                continue;
            const [, pairAddr, , ignoredForRouting] = result.result;
            if (pairAddr && pairAddr !== ZERO_ADDRESS && !ignoredForRouting) {
                validPairs.push({ address: pairAddr, binStep: calls[i].binStep });
            }
        }
        if (validPairs.length === 0)
            continue;
        ...
        for (let i = 0; i < validPairs.length; i++) {
            ...
            if (res.status !== "success" || txRes.status !== "success")
                continue;
            const [reserveX, reserveY] = res.result;
            if (reserveX === 0n || reserveY === 0n)
                continue;
            const tokenX = txRes.result.toLowerCase();
 ...
 */
export async function discoverTraderJoeLBPoolsTyped(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    const pools = [];
    const inLower = tokenIn.toLowerCase();
    for (const factory of factories) {
        ...
        for (let i = 0; i < infos.length; i++) {
            ...
            if (r.status !== "success")
                continue;
            const [, pairAddr, , ignoredForRouting] = r.result;
            if (pairAddr && pairAddr !== ZERO_ADDRESS && !ignoredForRouting) {
                validPairs.push({ address: pairAddr, binStep: binSteps[i] });
            }
        }
        if (validPairs.length === 0)
            continue;
        for (const vp of validPairs) {
            try {
                ...
                const [tokenXRaw, activeIdRaw, binStepRaw, feeParamsRaw] = await Promise.all([
                    client.readContract({ address: vp.address, abi: traderJoeLBPairAbi, functionName: "getTokenX" }),
                    client.readContract({ address: vp.address, abi: traderJoeLBPairAbi, functionName: "getActiveId" }),
                    client.readContract({ address: vp.address, abi: traderJoeLBPairAbi, functionName: "getBinStep" }),
                    client
                ...
                for (let i = 0; i < ids.length; i++) {
                    ...
                    if (r.status !== "success")
                        continue;
                    const [reserveX, reserveY] = r.result;
                    if (reserveX === 0n && reserveY === 0n)
                        continue; // uninitialized / empty bin
...
async function discoverMaverickV2Pools(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    const pools = [];
    for (const factory of factories) {
        // Try both token orderings — Maverick's lookup is order-dependent
        for (const [tokenA, tokenB] of [[tokenIn, tokenOut], [tokenOut, tokenIn]]) {
            try {
                ...
                for (let start = 0; start < MAVERICK_LOOKUP_MAX; start += MAVERICK_LOOKUP_PAGE) {
                    ...
                    if (page.length < end - start)
                        break; // short page → no more pools
                }
                for (const addr of addresses) {
                    if (!addr || addr === ZERO_ADDRESS)
                        continue;
...
// ── WOOFi discovery ─────────────────────────────────────────
async function discoverWOOFiPools(tokenIn, tokenOut, client, woofiConfigs) {
...
function wooFiFeeToPpm(feeRate) {
    return Number((feeRate * 1000000n + WOO_FEE_SCALE / 2n) / WOO_FEE_SCALE);
}
/**
 ...
 */
export async function discoverWooFiPoolsTyped(tokenIn, tokenOut, client, woofiConfigs) {
    if (woofiConfigs.length === 0)
        return [];
    const inLower = tokenIn.toLowerCase();
    const outLower = tokenOut.toLowerCase();
    ...
    for (const cfg of woofiConfigs) {
        ...
        try {
            const [quoteToken, wooracle] = await Promise.all([
                client.readContract({ address: cfg.address, abi: wooPPV2Abi, functionName: "quoteToken" }),
                client.readContract({ address: cfg.address, abi: wooPPV2Abi, functionName: "wooracle" }),
            ]);
            if (!quoteToken || quoteToken === ZERO_ADDRESS)
                continue;
            ...
            if (inIsQuote === outIsQuote)
                continue; // both-quote (impossible) or neither-quote (base→base)
            ...
            const [state, priceDecRaw, feeInfo, decInRaw, decOutRaw, testOut] = await Promise.all([
                client.readContract({ address: wooracle, abi: wooracleV2Abi, functionName: "state", args: [base] }),
                client.readContract({ address: wooracle, abi: wooracleV2Abi, functionName: "decimals", args: [base] }).then((d) => Number(d)).catch(() => 8),
                client.readContract({ address: cfg.address, abi: wooPPV2Abi, functionName: "tokenInfos", args: [base] }).catch(() => null),
                client.readContract({ address: tokenIn, abi: erc20DecimalsAbi, functionName: "decimals" }).then((d) => Number(d)).catch(() => 18),
                client.readContract({ address: tokenOut, abi: erc20DecimalsAbi, functionName: "decimals" }).then((d) => Number(d)).catch(() => 18),
                client.readContract({ address: cfg.address, abi: wooPPV2Abi, functionName: "query", args: [tokenIn, tokenOut, 10n ** BigInt(6)] }).catch(() => 0n),
            ]);
            const [price, spread, coeff, woFeasible] = state;
            if (!woFeasible || price <= 0n)
                continue; // oracle not feasible — the pool would revert
            if (testOut === 0n)
                continue; // pair not supported / no reserve to pay out
 ...
 * Fermi).
 */
export async function discoverFermiPoolsTyped(tokenIn, tokenOut, client, fermiConfigs, amountIn) {
    if (fermiConfigs.length === 0 || amountIn <= 0n)
        return [];
    ...
    for (const cfg of fermiConfigs) {
        ...
        try {
            ...
            const quotes = await Promise.all(sampleIn.map((amt) => client
                .readContract({
                address: cfg.address,
                abi: fermiPoolAbi,
            ...
            if (cumOut.length === 0 || cumOut[0] <= 0n)
                continue; // pair not tradeable / no out
            ...
            if (in0 > 0n && out0 > 0n && out0 < in0) {
                ...
                if (shortfall > 0n && shortfall < FERMI_FEE_SCALE)
                    feePpm = Number(shortfall);
            }
            out.push({
                address: cfg.address,
                tokenIn,
 ...
 */
export async function discoverFluidPoolsTyped(tokenIn, tokenOut, client, fluidConfigs, amountIn) {
    if (fluidConfigs.length === 0 || amountIn <= 0n)
        return [];
        ...
        for (const pool of cfg.fluidPools ?? []) {
            ...
            try {
                ...
                const [t0, t1] = (await client
                    ...
                    .catch(() => [ZERO_ADDRESS, ZERO_ADDRESS]));
                ...
                if (probeOut < probeIn) {
                    ...
                    if (shortfall > 0n && shortfall < FLUID_FEE_SCALE)
                        feePpm = Number(shortfall);
                }
                out.push({
                    address: pool,
 ...
 * understand Tessera).
 */
export async function discoverTesseraPoolsTyped(tokenIn, tokenOut, client, tesseraConfigs, amountIn) {
    if (tesseraConfigs.length === 0 || amountIn <= 0n)
        return [];
    ...
    for (const cfg of tesseraConfigs) {
        ...
        try {
            ...
            if (probeOut < probeIn) {
                ...
                if (shortfall > 0n && shortfall < TESSERA_FEE_SCALE)
                    feePpm = Number(shortfall);
            }
            out.push({
                address: cfg.address,
 ...
 */
export async function discoverElfomoPoolsTyped(tokenIn, tokenOut, client, elfomoConfigs, amountIn) {
    if (elfomoConfigs.length === 0 || amountIn <= 0n)
        return [];
    let probeIn = amountIn / QL_SEED_DIV;
    if (probeIn <= 0n)
        probeIn = 1n;
    ...
    for (const cfg of elfomoConfigs) {
        ...
        try {
            ...
            if (probeOut < probeIn) {
                ...
                if (shortfall > 0n && shortfall < ELFOMO_FEE_SCALE)
                    feePpm = Number(shortfall);
            }
            out.push({
                address: cfg.address,
 ...
 * understand Metric).
 */
export async function discoverMetricPoolsTyped(tokenIn, tokenOut, client, metricConfigs, amountIn) {
    if (metricConfigs.length === 0 || amountIn <= 0n)
        return [];
    // The FIRST QL slice size — the on-chain ladder's seed (curve-math QL_SEED_DIV), so the probe's head
    ...
    if (probeIn <= 0n)
        probeIn = 1n;
    if (probeIn > METRIC_INT128_MAX)
        probeIn = METRIC_INT128_MAX;
    const inLc = tokenIn.toLowerCase();
    const outLc = tokenOut.toLowerCase();
    const out = [];
    const seen = new Set();
    for (const cfg of metricConfigs) {
        ...
        for (const pool of cfg.metricPools ?? []) {
            ...
            try {
                ...
                if (outDelta >= 0n || inDelta <= 0n)
                    continue; // empty pool / wrong-side (0,0) / nonsense
                ...
                if (mid > 0n && ask > bid) {
                    ...
                    if (half > 0n && half < METRIC_FEE_SCALE)
                        feePpm = Number(half);
                }
                out.push({
                    address: pool,
 ...
 */
export async function discoverLiquidCorePoolsTyped(tokenIn, tokenOut, client, liquidCoreConfigs, amountIn) {
    if (liquidCoreConfigs.length === 0 || amountIn <= 0n)
        return [];
 ...
 */
export async function discoverSizePoolsTyped(tokenIn, tokenOut, client, sizeConfigs, amountIn) {
    if (sizeConfigs.length === 0 || amountIn <= 0n)
        return [];
        ...
        try {
            ...
            if (minOut > 0n) {
                const conv = (await client
                    ...
                    .catch(() => null));
                if (conv === null)
                    continue; // TR3A (inventory below the min) / TR5A / TR17 — the venue is dead
                minIn = conv;
            }
            ...
            let feePpm = 0;
            try {
                const factory = (await client.readContract({ address: relayer, abi: sizeRelayerAbi, functionName: "factory" }));
                const pair = (await client.readContract({ address: factory, abi: sizeFactoryAbi, functionName: "getPair", args: [tokenIn, tokenOut] }));
                if (BigInt(pair) !== 0n) {
                    const fee = (await client.readContract({ address: relayer, abi: sizeRelayerAbi, functionName: "swapFee", args: [pair] }));
                    const ppm = (fee * 10n ** 6n) / SIZE_PRECISION;
                    if (ppm > 0n && ppm < 10n ** 6n)
                        feePpm = Number(ppm);
                }
            }
            catch {
 ...
 */
export async function discoverPancakeStablePoolsTyped(tokenIn, tokenOut, client, pancakeStableConfigs, amountIn) {
    if (pancakeStableConfigs.length === 0 || amountIn <= 0n)
        return [];
    // The FIRST QL slice size — the on-chain ladder's seed, so the probe's head is exactly the head
    // the solver's first slice will carry when the state has not moved.
    let probeIn = amountIn / QL_SEED_DIV;
 ...
 */
export async function discoverEkuboPoolsTyped(tokenIn, tokenOut, client, ekuboConfigs, amountIn) {
    if (ekuboConfigs.length === 0 || amountIn <= 0n)
        return [];
    // The FIRST QL slice size — the on-chain ladder's seed, so the probe's head is exactly the head
    ...
    if (probeIn <= 0n)
        probeIn = 1n;
    if (probeIn > EKUBO_INT128_MAX)
        probeIn = EKUBO_INT128_MAX;
    ...
    for (const cfg of ekuboConfigs) {
        ...
        if (!router)
            continue; // misconfigured entry — the router is the quote/swap surface
        ...
        try {
            ...
            const calldata = (EKUBO_SLOAD_SELECTOR +
                candidates.map((c) => c.poolId.slice(2)).join(""));
            const res = await client.call({ to: cfg.address, data: calldata });
            const words = (res.data ?? "0x").slice(2);
            if (words.length < candidates.length * 64)
                continue; // not the Core surface — skip config
            for (let k = 0; k < candidates.length; k++) {
                const cand = candidates[k];
                const stateWord = BigInt("0x" + words.slice(k * 64, k * 64 + 64));
                if (stateWord === 0n || stateWord >> 160n === 0n)
                    continue; // sqrtRatio == 0 ⇒ uninitialized
 ...
 */
export async function discoverMentoPoolsTyped(tokenIn, tokenOut, client, mentoConfigs, amountIn) {
    if (mentoConfigs.length === 0 || amountIn <= 0n)
        return [];
            ...
            for (const provider of providers) {
                ...
                for (const ex of exchanges) {
                    ...
                    if (in0 > 0n && out0 > 0n && out0 < in0) {
                        ...
                        if (shortfall > 0n && shortfall < MENTO_FEE_SCALE)
                            feePpm = Number(shortfall);
                    }
                    out.push({
                        broker,
 ...
 */
export async function discoverBalancerV3PoolsTyped(tokenIn, tokenOut, client, balancerV3Configs, amountIn) {
    if (balancerV3Configs.length === 0 || amountIn <= 0n)
        return [];
    ...
    for (const cfg of balancerV3Configs) {
        ...
        if (!router)
            continue; // a BalancerV3 entry MUST carry the per-chain Router.
        for (const pool of cfg.balancerV3Pools ?? []) {
            ...
            try {
                ...
                const [liveBalances, staticFeeWad, ampRes, tokenInfo] = await Promise.all([
                    client.readContract({ address: vault, abi: balancerV3VaultAbi, functionName: "getCurrentLiveBalances", args: [pool] }).then((r) => r),
                    client.readContract({ address: vault, abi: balancerV3VaultAbi, functionName: "getStaticSwapFeePercentage", args: [pool] }).then((r) => r),
                    client.readContract({ address: pool, abi: balancerV3PoolAbi, functionName: "getAmplificationParameter" }).then((r) => r),
                    client.readContract({ address: vault, abi: balancerV3VaultAbi, functionName: "getPoolTokenInfo", args: [pool] }).then((r) => r),
                ]);
                const amp = ampRes[0];
                const rpIn = tokenInfo[1][inIdx]?.rateProvider ?? ZERO_ADDRESS;
                const rpOut = tokenInfo[1][outIdx]?.rateProvider ?? ZERO_ADDRESS;
                if (rpIn === ZERO_ADDRESS || rpOut === ZERO_ADDRESS)
                    continue; // no rate provider → the QL replay can't scale
                ...
                const [rateIn, rateOut, decInRaw, decOutRaw] = await Promise.all([
                    client.readContract({ address: rpIn, abi: balancerV3RateProviderAbi, functionName: "getRate" }).then((r) => r),
                    client.readContract({ address: rpOut, abi: balancerV3RateProviderAbi, functionName: "getRate" }).then((r) => r),
                ...
                const quotes = await Promise.all(sampleIn.map((amt) => client
                    ...
                    args: [pool, tokenIn, tokenOut, amt, ZERO_ADDRESS, "0x"],
                })
                    .then((r) => r)
                    .catch(() => 0n)));
                const cumIn = [];
                ...
                for (let i = 0; i < sampleIn.length; i++) {
                    ...
                    if (truth <= prevOut)
                        break; // limit edge / degenerate — stop the ladder here
                ...
                if (surgeActive)
                    continue; // surge-active pool — out of scope for the static-fee QL replay this landing
                if (cumOut.length === 0 || cumOut[0] <= 0n)
                    continue; // pair not tradeable / no out
                ...
                if (in0 > 0n && out0 > 0n && out0 < in0) {
                    ...
                    if (shortfall > 0n && shortfall < BALANCER_V3_FEE_SCALE)
                        feePpm = Number(shortfall);
                }
                b3.feePpm = feePpm;
                b3.cumIn = cumIn;
 ...
 */
export async function discoverKyberClassicPools(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    ...
    for (let i = 0; i < validPools.length; i++) {
        ...
        const t0 = token0Results[i];
        if (ti.status !== "success" || t0.status !== "success")
            continue;
        const [reserve0, reserve1, vReserve0, vReserve1, feeInPrecision] = ti.result;
        if (vReserve0 === 0n || vReserve1 === 0n)
            continue;
        const inIsToken0 = tokenIn.toLowerCase() === t0.result.toLowerCase();
        pools.push({
            address: validPools[i].address,
            tokenIn,
            tokenOut,
...
function wombatHaircutToPpm(haircutRate) {
    return Number((haircutRate * 1000000n + WOMBAT_WAD / 2n) / WOMBAT_WAD);
}
/**
 ...
 */
export async function discoverWombatPoolsTyped(tokenIn, tokenOut, client, pools) {
    if (pools.length === 0)
        return [];
    // Resolve each pool's from/to Asset addresses (both must exist for the pair to trade).
    const assetCalls = pools.flatMap((p) => [
        { address: p.address, abi: wombatPoolAbi, functionName: "addressOfAsset", args: [tokenIn] },
        { address: p.address, abi: wombatPoolAbi, functionName: "addressOfAsset", args: [tokenOut] },
    ]);
    ...
    for (let i = 0; i < pools.length; i++) {
        ...
        if (fr.status !== "success" || to.status !== "success")
            continue;
        const fromAsset = fr.result;
        const toAsset = to.result;
        if (!fromAsset || fromAsset === ZERO_ADDRESS || !toAsset || toAsset === ZERO_ADDRESS)
            continue;
    ...
    const [ampResults, haircutResults, fromCashResults, fromLiabResults, toCashResults, toLiabResults] = await Promise.all([
        client.multicall({ contracts: valid.map((v) => ({ address: v.pool.address, abi: wombatPoolAbi, functionName: "ampFactor" })), allowFailure: true }),
        client.multicall({ contracts: valid.map((v) => ({ address: v.pool.address, abi: wombatPoolAbi, functionName: "haircutRate" })), allowFailure: true }),
        client.multicall({ contracts: valid.map((v) => ({ address: v.fromAsset, abi: wombatAssetAbi, functionName: "cash" })), allowFailure: true }),
        client.multicall({ contracts: valid.map((v) => ({ address: v.fromAsset, abi: wombatAssetAbi, functionName: "liability" })), allowFailure: true }),
        client.multicall({ contracts: valid.map((v) => ({ address: v.toAsset, abi: wombatAssetAbi, functionName: "cash" })), allowFailure: true }),
        client.multicall({ contracts: valid.map((v) => ({ address: v.toAsset, abi: wombatAssetAbi, functionName: "liability" })), allowFailure: true }),
    ]);
    // Decimals: read the two tokens once (tokenIn + tokenOut).
    const [decInRaw, decOutRaw] = await Promise.all([
        client.readContract({ address: tokenIn, abi: erc20DecimalsAbi, functionName: "decimals" }).then((d) => Number(d)).catch(() => 18),
    ...
    const out = [];
    for (let i = 0; i < valid.length; i++) {
        if (ampResults[i].status !== "success" || haircutResults[i].status !== "success" ||
            ...
            continue;
        const fromCash = fromCashResults[i].result;
        const fromLiability = fromLiabResults[i].result;
        const toCash = toCashResults[i].result;
        const toLiability = toLiabResults[i].result;
        // A pool with no from-cash to sell into or no to-cash to pay out cannot trade.
        if (fromLiability <= 0n || toLiability <= 0n || toCash <= 0n)
            continue;
        const haircutRate = haircutResults[i].result;
        out.push({
 ...
 */
export async function discoverEulerSwapPoolsTyped(tokenIn, tokenOut, client, factories) {
    // Flatten every candidate pool address across the EulerSwap factory configs.
    const candidates = [];
    for (const f of factories) {
        for (const addr of f.eulerSwapPools ?? [])
            candidates.push({ address: addr, label: f.label });
    }
    ...
    const [idResults, curveResults] = await Promise.all([
        client.multicall({
            contracts: candidates.map((cp) => ({ address: cp.address, abi: eulerSwapPoolAbi, functionName: "getAssets" })),
            allowFailure: true,
        }),
        client.multicall({
            contracts: candidates.map((cp) => ({ address: cp.address, abi: eulerSwapPoolAbi, functionName: "curve" })),
            allowFailure: true,
        }),
    ]);
    const valid = [];
    for (let i = 0; i < candidates.length; i++) {
        ...
        const isV1 = curveOk
            ? curveResults[i].result.toLowerCase() === EULER_CURVE_V1
            : true; // ambiguous — provisional v1, reconciled against the getters below
        if (asset0 === ti && asset1 === to)
            valid.push({ cp: candidates[i], inIsToken0: true, isV1, curveOk });
        else if (asset0 === to && asset1 === ti)
            valid.push({ cp: candidates[i], inIsToken0: false, isV1, curveOk });
    }
    if (valid.length === 0)
        return [];
    ...
    const [resR, pR, dpR, limR] = await Promise.all([
        client.multicall({ contracts: valid.map((v) => ({ address: v.cp.address, abi: eulerSwapPoolAbi, functionName: "getReserves" })), allowFailure: true }),
        client.multicall({ contracts: valid.map((v) => ({ address: v.cp.address, abi: eulerSwapPoolAbi, functionName: "getParams" })), allowFailure: true }),
        client.multicall({ contracts: valid.map((v) => ({ address: v.cp.address, abi: eulerSwapPoolAbi, functionName: "getDynamicParams" })), allowFailure: true }),
        client.multicall({ contracts: valid.map((v) => ({ address: v.cp.address, abi: eulerSwapPoolAbi, functionName: "getLimits", args: [tokenIn, tokenOut] })), allowFailure: true }),
    ]);
    const out = [];
    for (let i = 0; i < valid.length; i++) {
        if (resR[i].status !== "success")
            continue;
        ...
        let bundle = null;
        if (isV1) {
            if (pR[i].status !== "success")
                continue; // v1 pool with an unreadable getParams() ⇒ drop
            const p = pR[i].result;
            ...
            if (dpR[i].status !== "success")
                continue; // v2 pool with an unreadable getDynamicParams() ⇒ drop
            const dp = dpR[i].result;
            bundle = {
        ...
        const [reserve0, reserve1] = resR[i].result;
 ...
 */
export async function discoverMaverickV2PoolsTyped(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    const inLower = tokenIn.toLowerCase();
            ...
            for (const addr of addresses) {
                ...
                try {
                    const [tokenARaw, stateRaw, tsRaw] = await Promise.all([
                        client.readContract({ address: pool, abi: maverickV2PoolAbi, functionName: "tokenA" }),
                        client.readContract({ address: pool, abi: maverickV2PoolAbi, functionName: "getState" }),
                        client.readContract({ address: pool, abi: maverickV2PoolAbi, functionName: "tickSpacing" }),
                    ]);
                    const tokenAIn = inLower === tokenARaw.toLowerCase();
                    ...
                    for (let k = 0; k < tickNums.length; k++) {
                        ...
                        if (r.status !== "success")
                            continue;
                        const st = r.result;
                        if (st.reserveA === 0n && st.reserveB === 0n)
                            continue;
                    ...
                    if (!active)
                        continue; // active tick must carry liquidity to seed the walk
                    ...
                    if (activeL === 0n)
                        continue;
                    const poolSqrtPrice = getMaverickSqrtPrice(active.reserveA, active.reserveB, sqrtLowerPrice, sqrtUpperPrice, activeL);
                    pools.push({
                        poolType: SwapPoolType.MaverickV2,
                        address: pool,
                        tokenAIn,
 ...
 */
async function discoverV4Pools(tokenIn, tokenOut, client, factories, feeTiers) {
    if (factories.length === 0)
        return [];
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        if (s.status !== "success" || l.status !== "success")
            continue;
        const sqrtPriceX96 = s.result[0];
        const liquidity = l.result;
        if (sqrtPriceX96 === 0n || liquidity === 0n)
            continue;
        pools.push({
 ...
 */
async function discoverInfinityCLPools(tokenIn, tokenOut, client, factories) {
    if (factories.length === 0)
        return [];
    ...
    for (const f of factories) {
        for (const p of f.infinityPresets ?? INFINITY_DEFAULT_CL_PRESETS) {
            candidates.push({
                ...
                hooks: ZERO_HOOKS,
                poolId: computeInfinityPoolId(currency0, currency1, ZERO_HOOKS, f.address, p.fee, encodeInfinityCLParameters(p.tickSpacing)),
            });
        }
        ...
        if (hookedIds.length > 0 && allowlist.length > 0) {
            ...
            for (let i = 0; i < hookedIds.length; i++) {
                ...
                if (k.status !== "success")
                    continue;
                const [c0, c1, hooks, poolManager, fee, parameters] = k.result;
                // The recovered key must be for THIS pair on THIS manager (an uninitialized id reads
                // the zero key and fails the pair check).
                if (c0.toLowerCase() !== currency0.toLowerCase())
                    continue;
                if (c1.toLowerCase() !== currency1.toLowerCase())
                    continue;
                if (poolManager.toLowerCase() !== f.address.toLowerCase())
                    continue;
                if (!allowlist.includes(hooks.toLowerCase()))
                    continue;
                if (fee === INFINITY_DYNAMIC_FEE_FLAG)
                    continue; // Tier C: dynamic fee — quoter-only
                if (fee > INFINITY_MAX_FEE_PPM)
                    continue; // honeypot fee cap
                if ((BigInt(parameters) & INFINITY_RETURNS_DELTA_MASK) !== 0n)
                    continue; // non-deterministic amounts
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        if (s.status !== "success" || l.status !== "success")
            continue;
        const [sqrtPriceX96, tick, protocolFee, lpFee] = s.result;
        const liquidity = l.result;
        if (sqrtPriceX96 === 0n || liquidity === 0n)
            continue;
        pools.push({
 ...
 */
export async function discoverInfinityCLPoolsTyped(tokenIn, tokenOut, client, factories) {
    return discoverInfinityCLPools(tokenIn, tokenOut, client, factories.filter((f) => f.factoryType === FactoryType.PancakeInfinityCL));
}
// ── Unified discovery ───────────────────────────────────────
/**
 ...
 */
export async function discoverPools(tokenIn, tokenOut, client, poolConfig = BASE_CHAIN_POOL_CONFIG) {
    ...
    const [v3Pools, slipstreamPools, v4Pools, algebraPools, v2Pools, solidlyV2Pools, curvePools, balancerPools, dodoPools, traderJoePools, maverickPools, woofiPools, infinityPools] = await Promise.all([
