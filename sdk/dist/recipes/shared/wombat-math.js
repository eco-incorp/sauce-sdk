 ...
 */
function swapQuoteFunc(Ax, Ay, Lx, Ly, Dx, A) {
    if (Lx === 0n || Ly === 0n)
        return 0n;
    if (Ax <= 0n || Ay <= 0n)
        return 0n;
 ...
 */
export function quotePotentialSwap(pool, dx) {
    if (dx <= 0n)
        return 0n;
    const fromAmountWad = (dx * WAD) / pool.decIn; // toWad
    const idealOutWad = swapQuoteFunc(pool.fromCash, pool.toCash, pool.fromLiability, pool.toLiability, fromAmountWad, pool.ampFactor);
    if (idealOutWad <= 0n)
        return 0n;
 ...
 */
export function buildWombatSegments(pool, amountIn, samples = WOMBAT_SAMPLES) {
    if (amountIn <= 0n)
        return [];
 ...
 */
export function buildWombatQLLadder(pool, amountIn) {
    return buildQLLadder((dx) => quotePotentialSwap(pool, dx), amountIn);
}
//# sourceMappingURL=wombat-math.js.map