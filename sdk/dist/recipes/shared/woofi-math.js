 ...
 */
export function wooFiInputCap(pool, fallbackMax) {
    let cap = fallbackMax;
...
export function query(pool, dx) {
    ...
    if (pool.sellBase) {
        ...
        if (factor <= 0n)
            return 0n;
        let quoteAmount = (((dx * pool.price * pool.quoteDec) / pool.priceDec) * factor) / WOO_WAD / pool.baseDec;
    ...
    if (factor <= 0n)
        return 0n;
    const baseAmount = (((q * pool.baseDec * pool.priceDec) / pool.price) * factor) / WOO_WAD / pool.quoteDec;
 ...
 */
export function buildWooFiQLLadder(pool, amountIn) {
    return buildQLLadder((dx) => query(pool, dx), amountIn);
}
export function buildWooFiSegments(pool, amountIn, samples = WOOFI_SAMPLES) {
    if (amountIn <= 0n)
        return [];
...
//# sourceMappingURL=woofi-math.js.map