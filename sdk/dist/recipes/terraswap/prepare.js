...
import { MIN_SQRT_RATIO, MAX_SQRT_RATIO, BASE_CHAIN_POOL_CONFIG, CHAIN_POOL_CONFIGS } from "../shared/constants.js";
...
function maxPriceLimitForPair(tokenA, tokenB) {
...
function resolvePoolConfig(chainConfig) {
    if (chainConfig.poolConfig)
        return chainConfig.poolConfig;
    return CHAIN_POOL_CONFIGS[chainConfig.name] ?? BASE_CHAIN_POOL_CONFIG;
}
async function prepareChain(chainConfig) {
    ...
    const priceLimitedPools = quotedPriceLimited
    ...
    console.log(`  [${chainConfig.name}] ${priceLimitedPools.length} V3 (price-limited), ` +
...
async function findGlobalPriceLimit(chains) {
...
// ── Initial preparation ──────────────────────────────────────
export async function prepareTerraSwap(config) {
    const chains = await Promise.all(config.chains.map(prepareChain));
    const globalPriceLimit = await findGlobalPriceLimit(chains);
    console.log(`  TerraSwap prepared: ${chains.length} chains, globalPriceLimit=${globalPriceLimit}`);
    for (const c of chains) {
        console.log(`    [${c.config.name}] ${c.priceLimitedPools.length} V3, ${c.noLimitPools.length} V2, ${c.multiHopRoutes.length} multi-hop`);
    }
    return { chains, globalPriceLimit };
}
// ── Subsequent series preparation (from post-swap state) ─────
export async function prepareNextSeries(previousChains, leftovers) {
    const newChains = [];
