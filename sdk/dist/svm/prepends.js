...
import { getRequestHeapFrameInstruction, getSetComputeUnitLimitInstruction, getSetComputeUnitPriceInstruction, } from '@solana-program/compute-budget';
import { HEAP_FRAME_BYTES } from './engine.js';
import { TOKEN_PROGRAM_ADDRESS, findAssociatedTokenPda, getCreateAssociatedTokenIdempotentInstruction, getSyncNativeInstruction, } from '@solana-program/token';
 ...
 */
export function buildHeapFramePrepend(bytes = HEAP_FRAME_BYTES) {
    return getRequestHeapFrameInstruction({ bytes });
}
export function buildComputeBudgetPrepend({ unitLimit, microLamportsPerCu }) {
    const instructions = [getSetComputeUnitLimitInstruction({ units: unitLimit })];
...
export async function buildAtaPrepend({ payer, owner, mint, tokenProgram }) {
...
export async function buildWrapSolPrepends({ payer, owner, lamports }) {
...
//# sourceMappingURL=prepends.js.map