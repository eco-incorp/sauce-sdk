 ...
 * hard-coded ceiling.
 */
const ALPHAQ_STATS_ACCOUNTS = Object.freeze({
...
const runtimeStatsBySymbol = new Map();
 ...
 */
export function primeAlphaqStatsAccounts(rpc) {
    if (primePromise)
        return primePromise;
    primePromise = (async () => {
        ...
        for (const acc of accounts) {
            ...
            if (symbol.length > 0)
                runtimeStatsBySymbol.set(symbol, acc.pubkey);
        }
    })();
    return primePromise;
}
/** Test-only: seed the runtime overlay without a real RPC (LiteSVM/jest fixtures). */
export function __setAlphaqStatsAccountsForTest(entries) {
    runtimeStatsBySymbol.clear();
    for (const [symbol, acct] of Object.entries(entries))
        runtimeStatsBySymbol.set(symbol, acct);
}
/** Test-only: undo {@link __setAlphaqStatsAccountsForTest} / a completed prime. */
export function __resetAlphaqStatsAccountsForTest() {
    runtimeStatsBySymbol.clear();
    primePromise = null;
}
function resolveAlphaqStatsAccount(symbol) {
    return ALPHAQ_STATS_ACCOUNTS[symbol] ?? runtimeStatsBySymbol.get(symbol);
}
function decodeSymbol(data) {
    let end = 0;
    while (end < 16 && data[end] !== 0)
        end++;
...
function bs58EncodeUnchecked(bytes) {
    let value = 0n;
 ...
 */
export async function fetchAlphaqPoolConfig(load, pool) {
    const data = await load(pool);
    if (data === null)
        throw new Error(`alphaq market ${pool} not found`);
    if (data.length !== MARKET_SIZE) {
        throw new Error(`alphaq market ${pool} has unexpected size ${data.length} (want ${MARKET_SIZE})`);
    }
    const symbol = decodeSymbol(data);
    if (symbol.length === 0)
        throw new Error(`alphaq market ${pool} has a blank symbol`);
    ...
    if (!STABLE_MINTS.has(mintA) || !STABLE_MINTS.has(mintB)) {
        throw new Error(`alphaq market ${pool} ("${symbol}") is not a curated stablecoin pair — this integration's ` +
    ...
    if (statsAccount === undefined) {
        throw new Error(`alphaq market ${pool} ("${symbol}") has no known stats account — not in the captured table and ` +
 ...
 */
function cpQuote(x, rawA, rawB) {
    if (x <= 0n || rawA <= 0n || rawB <= 0n)
        return 0n;
