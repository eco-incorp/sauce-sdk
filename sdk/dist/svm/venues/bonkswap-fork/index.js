...
export const FEE_DENOMINATOR = 1000000000000n;
 ...
 */
export function makeBonkswapForkAdapter(slug, programId) {
    return {
        slug,
        programId,
        async fetchPoolConfig(load, pool) {
            ...
            if (constKHi === 0n && constKLo === 0n)
                throw new Error(`${slug} pool ${pool} has a zero constK`);
            const feeAt = (offset, name) => {
                const lo = readUintLE(data, offset, 8);
                const hi = readUintLE(data, offset + 8, 8);
                if (hi !== 0n)
                    throw new Error(`${slug} pool ${pool} ${name} exceeds a u64 (hi word ${hi} nonzero) — refusing to truncate`);
                return lo;
            };
...
export const BONKSWAP_PROGRAM_ID = 'BSwp6bEBihVLdqJRKGgzjcGLHkcTuzmSo1TQkHepzH8p';
/** Bonkswap's `state` PDA (seed "bonkswapstatev1") — verified via getProgramDerivedAddress and matches a real swap tx's account #0. */
export const BONKSWAP_STATE = '2QWN6WjrJ3RAk51ecxLxaLPfFCYLAnmWJwJ1oKA92CRD';
/** Bonkswap's vault-owning authority PDA (State.programAuthority) — matches every pool's vault SPL-token `owner` field. */
export const BONKSWAP_PROGRAM_AUTHORITY = '8NyaPDJeC2eaBGpkRpZKnD9S448AZGgjSvumFe92DRK2';
export const bonkswap = makeBonkswapForkAdapter('bonkswap', BONKSWAP_PROGRAM_ID);
/** Guacswap — a whitelabel fork (see module header). */
export const GUACSWAP_PROGRAM_ID = 'Gswppe6ERWKpUTXvRPfXdzHhiCyJvLadVvXGfdpBqcE1';
/** Guacswap's `state` PDA (seed "guacswapstatev1"). */
export const GUACSWAP_STATE = 'EzV4QKycvp4EDuAoTGjyY9HUS9Zn15vcmHkHJdNKHCnY';
/** Guacswap's vault-owning authority PDA. */
export const GUACSWAP_PROGRAM_AUTHORITY = 'FRwL3qJHuT5iYTNopCyhU1tyDn1HwtHbSa1ieYF6q2m9';
