 ...
 */
export async function fetchByrealPoolConfig(load, pool) {
    ...
    for (let i = 0; i < DYNAMIC_FEE_INFO_LEN; i++) {
        if (data[OFF_DYNAMIC_FEE_INFO + i] !== 0) {
            throw new Error(`${SLUG}: pool ${pool} has a dynamic fee configured — its swap walks tick-spacing-bounded steps with a ` +
