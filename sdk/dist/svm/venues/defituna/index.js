...
export function defitunaWindowFor(cfg) {
...
export const defitunaLadder = {
    ...
    helpers() {
        ...
        return orcaWhirlpoolLadder.helpers();
    },
    ...
    paramsFor(base) {
        ...
        for (let k = 0; k < DEFITUNA_MAX_BOUNDARIES; k++) {
            ...
            words.push((BigInt(boundary.tick) + BIAS) | (BigInt(boundary.byteOffset) << 32n) | (BigInt(boundary.arrayIndex) << ARRAY_INDEX_SHIFT), boundary.sqrtPrice >> 64n, boundary.sqrtPrice & ((1n << 64n) - 1n));
        }
        if (window.edge === null)
            words.push(0n, 0n, 0n);
        else
            words.push(BigInt(window.edge.tick) + BIAS, window.edge.sqrtPrice >> 64n, window.edge.sqrtPrice & ((1n << 64n) - 1n));
        return words;
    },
    quoteRefs(base, slot) {
