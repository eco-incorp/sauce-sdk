...
async function vaultPda(pool, mint) {
...
export async function deriveGatorswapPool(mintA, mintB) {
    const [pda] = await getProgramDerivedAddress({
        programAddress: GATORSWAP_PROGRAM_ID,
...
async function fetchPoolConfig(load, pool) {
    ...
    if (refData === null)
        throw new Error(`${SLUG} pool ${pool} reference pool ${refPool} account not found`);
    if (!hasDiscriminator(refData, PUMPSWAP_POOL_DISCRIMINATOR)) {
        throw new Error(`${SLUG} pool ${pool} reference ${refPool} is not a recognized PumpSwap Pool account (this adapter only models the PumpSwap-reference case)`);
    }
 ...
 */
function refVaultsForDirection(cfg) {
...
export const gatorswapLadder = {
    ...
    depthReserves(base, state) {
        ...
        if (gaData === undefined || gbData === undefined)
    ...
    continuousFees() {
        ...
        return { gammaPpm: ((BPS_DEN - REF_HAIRCUT_BPS) * 1000000n) / BPS_DEN, muPpm: 1000000n };
    },
};
