...
export const HADRON_ASSET_REGISTRY = {
...
export function hadronConfig(cfg) {
    if (cfg.venue !== SLUG)
        throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
    return cfg;
}
export async function fetchHadronConfig(load, pool) {
    ...
    if (entry === undefined) {
        throw new Error(`${SLUG} pool ${pool}: mintA ${mintA} is not in the known asset registry — refresh HADRON_ASSET_REGISTRY in sdk/src/svm/venues/hadron/index.ts`);
    }
    if (entry.mintB !== mintB) {
        throw new Error(`${SLUG} pool ${pool}: mintA ${mintA}'s registered/validated quote mint is ${entry.mintB}, but this pool's mintB is ${mintB} — ` +
    ...
    if (vaultAData === null)
        throw new Error(`${SLUG} pool ${pool}: vaultA ${vaultA} does not exist`);
    if (vaultAData.length !== HADRON_SPL_TOKEN_ACCOUNT_SIZE) {
        throw new Error(`${SLUG} pool ${pool}: vaultA ${vaultA} has unexpected size ${vaultAData.length} (want ${HADRON_SPL_TOKEN_ACCOUNT_SIZE} — token-2022 vaults are not supported)`);
    }
    const vaultBData = await load(vaultB);
    if (vaultBData === null)
        throw new Error(`${SLUG} pool ${pool}: vaultB ${vaultB} does not exist`);
    if (vaultBData.length !== HADRON_SPL_TOKEN_ACCOUNT_SIZE) {
        throw new Error(`${SLUG} pool ${pool}: vaultB ${vaultB} has unexpected size ${vaultBData.length} (want ${HADRON_SPL_TOKEN_ACCOUNT_SIZE} — token-2022 vaults are not supported)`);
    }
    return {
        venue: SLUG,
        pool,
