...
function heliumTreasuryConfig(cfg) {
    if (cfg.venue !== SLUG)
        throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
    return cfg;
}
const ref = (slot, role) => `s${slot}:${role}`;
export async function fetchHeliumTreasuryPoolConfig(load, pool) {
    const data = await load(pool);
    if (data === null)
        throw new Error(`${SLUG} treasury management ${pool} does not exist`);
    ...
    if (treasuryData === null)
        throw new Error(`${SLUG} treasury management ${pool}: treasury token account ${treasury} does not exist`);
    ...
    if (supplyMintData === null)
        throw new Error(`${SLUG} treasury management ${pool}: supply mint ${supplyMint} does not exist`);
    ...
    if (cbData === null)
        throw new Error(`${SLUG} treasury management ${pool}: circuit breaker ${circuitBreaker} does not exist`);
    ...
    return {
        ...
        treasury,
        circuitBreaker,
        thresholdType: thresholdTypeByte,
        threshold,
        freezeUnixTime,
    };
}
...
export const heliumTreasuryLadder = {
    slug: SLUG,
