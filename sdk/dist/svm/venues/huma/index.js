...
export const HUMA_CURATED_MODE_CONFIGS = {
...
const poolRef = (cfg) => `${SLUG}:${cfg.modeConfig}`;
async function derivePda(programAddress, seeds) {
    const encoder = getAddressEncoder();
    const rawSeeds = seeds.map((s) => (typeof s === 'string' ? encoder.encode(s) : s));
...
export async function humaLenderStatePda(modeConfig, lender) {
    const encoder = getAddressEncoder();
    return derivePda(HUMA_PROGRAM_ID, [
        utf8.encode('lender_state'),
        encoder.encode(modeConfig),
...
function decodePoolState(data, modeConfig) {
...
export function humaFeeBpsFor(liquidRatioBps, feeTiers) {
    for (const tier of feeTiers) {
        if (liquidRatioBps < tier.ltBps)
            return tier.feeBps;
    }
    return 0n;
}
export const huma = {
    ...
    async fetchPoolConfig(load, pool) {
        ...
        const poolConfigAccount = HUMA_CURATED_MODE_CONFIGS[modeConfig];
        ...
        if (poolConfigData === null)
            throw new Error(`${SLUG} pool_config account ${poolConfigAccount} not found`);
        const decodedConfig = decodePoolConfig(poolConfigData);
        const poolState = await derivePda(HUMA_PROGRAM_ID, [utf8.encode('pool_state'), getAddressEncoder().encode(poolConfigAccount)]);
        const poolStateData = await load(poolState);
        if (poolStateData === null)
            throw new Error(`${SLUG} pool_state account ${poolState} not found`);
        const decodedState = decodePoolState(poolStateData, modeConfig);
        const poolAuthority = await derivePda(HUMA_PROGRAM_ID, [utf8.encode('pool_authority'), getAddressEncoder().encode(poolConfigAccount)]);
        const modeMint = await derivePda(HUMA_PROGRAM_ID, [
            utf8.encode('mode_mint'),
            getAddressEncoder().encode(poolConfigAccount),
            getAddressEncoder().encode(modeConfig),
        ]);
        const modeMintData = await load(modeMint);
        if (modeMintData === null)
            throw new Error(`${SLUG} mode_mint account ${modeMint} not found`);
        ...
        if (poolUnderlyingTokenData === null)
            throw new Error(`${SLUG} pool_underlying_token account ${poolUnderlyingToken} not found`);
        const poolOwnerTreasuryUnderlyingToken = await findAta(decodedConfig.poolOwnerTreasury, decodedConfig.underlyingMint);
        const liquidityDeploymentState = decodedConfig.liquiditySource !== null
            ? await derivePda(HUMA_PROGRAM_ID, [utf8.encode('deployment_state'), getAddressEncoder().encode(decodedConfig.liquiditySource)])
            : null;
        return {
            venue: SLUG,
            pool: modeConfig,
    ...
    emitQuote(cfg, i, amountIn) {
        const c = asConfig(cfg);
        if (amountIn <= 0n)
            throw new Error(`${SLUG} amountIn must be positive, got ${amountIn}`);
        if (amountIn > U64_MAX)
            throw new Error(`${SLUG} amountIn must fit u64, got ${amountIn}`);
    ...
    referenceQuote(cfg, state, amountIn) {
        ...
        if (poolState === undefined)
            throw new Error(`${SLUG} referenceQuote state is missing pool_state ${c.poolState}`);
        if (modeMintData === undefined)
            throw new Error(`${SLUG} referenceQuote state is missing mode_mint ${c.modeMint}`);
        if (poolUnderlyingData === undefined)
            throw new Error(`${SLUG} referenceQuote state is missing pool_underlying_token ${c.poolUnderlyingToken}`);
        if (poolState[9] !== 1)
            throw new Error(`${SLUG} pool ${c.poolConfigAccount} is not On (status byte ${poolState[9]})`);
 ...
 * exactly.
 */
export function humaFeeAssignLines(c, ratioVar, feeVar) {
...
function encodeDepositData(assets) {
...
function encodeInstantWithdrawData(shares) {
...
function depositAccounts(c, user) {
    return [
...
function instantWithdrawAccounts(c, user) {
