...
async function fetchHyloStabilityPoolConfig(load, pool) {
    const data = await load(pool);
    if (data === null)
        throw new Error(`hylo-stability-pool ${pool} account not found`);
...
export const hyloStabilityPool = {
    ...
    fetchPoolConfig: (load, pool) => fetchHyloStabilityPoolConfig(load, pool),
};
// ---------------------------------------------------------------------------
// Ladder (fragment emission + reference mirror).
// ---------------------------------------------------------------------------
function hyloConfig(base) {
    if (base.venue !== 'hylo-stability-pool')
        throw new Error(`hylo-stability-pool ladder adapter got a '${base.venue}' pool config`);
    return base;
}
...
export const hyloStabilityPoolLadder = {
    slug: 'hylo-stability-pool',
    shapeKey(base) {
    ...
    quoteRefs(base, slot) {
        ...
        if (cfg.direction === 'withdraw')
            accounts.push({ ref: ref(slot, 'poolcfg'), address: HYLO_STABILITY_POOL_CONFIG });
        return accounts;
    },
    ...
    buildSwapV2(base, slot, user) {
        const cfg = hyloConfig(base);
        const roled = (role, addr, writable, signer) => writable || signer
    ...
    referenceQuote(base, state, params) {
        ...
        const bytes = (addr) => {
            ...
            if (data === undefined)
                throw new Error(`hylo-stability-pool ladder reference is missing account ${addr}`);
            return data;
        };
    ...
    depthReserves(base, state) {
        ...
        const bytes = (addr) => {
            const data = state[addr];
            if (data === undefined)
                throw new Error(`hylo-stability-pool ladder depth is missing account ${addr}`);
            return data;
        };
        const poolBal = readUintLE(bytes(STABLECOIN_POOL), 64, 8);
        const lpSup = readUintLE(bytes(LP_TOKEN_MINT), 36, 8);
    ...
    continuousFees(base, _state, params) {
        ...
        if (cfg.direction === 'withdraw') {
            ...
            const feePpm = feeBits > 0n ? (feeBits * 1000000n) / feeScale : 0n;
