...
export function invariantSqrtPriceAtTick(tickIndex) {
    const tick = Math.abs(tickIndex);
    if (tick > MAX_TICK)
        throw new Error(`invariant: tick ${tickIndex} out of bounds`);
    let price = FEE_ONE;
    for (const [mask, c] of TICK_BITS) {
        if (tick & mask)
            price = (price * c) / FEE_ONE;
    }
    if (tickIndex >= 0)
        return price * (PRICE_ONE / FEE_ONE);
...
export function invariantDeltaX(sqrtA, sqrtB, liquidity, up) {
...
export function invariantDeltaY(sqrtA, sqrtB, liquidity, up) {
    const deltaPrice = sqrtA > sqrtB ? sqrtA - sqrtB : sqrtB - sqrtA;
    if (up)
        return ceilDiv(ceilDiv(deltaPrice * liquidity, LIQ_ONE), PRICE_ONE);
...
export function invariantNextSqrtXUp(price, liquidity, amount) {
    if (amount === 0n)
        return price;
...
export function invariantNextSqrtYDown(price, liquidity, amount) {
    if (amount === 0n)
        return price;
 ...
 */
export function invariantComputeSwapStepIn(currentSqrt, targetSqrt, liquidity, amount, fee) {
    if (liquidity === 0n)
        return { nextSqrt: targetSqrt, amountIn: 0n, amountOut: 0n, feeAmount: 0n };
...
function tickToPosition(tick, tickSpacing) {
    if (tick % tickSpacing !== 0)
        throw new Error(`invariant: tick ${tick} not divisible by spacing ${tickSpacing}`);
    const bitmapIndex = Math.trunc(tick / tickSpacing) + TICK_LIMIT;
    const byte = Math.trunc(bitmapIndex / 8);
    const bit = ((bitmapIndex % 8) + 8) % 8;
    return [byte, bit];
}
function getBit(bitmap, tick, tickSpacing) {
    const [byte, bit] = tickToPosition(tick, tickSpacing);
    return ((bitmap[byte] >> bit) & 1) === 1;
}
function getSearchLimit(tick, tickSpacing, up) {
...
function nextInitialized(bitmap, tick, tickSpacing) {
...
function prevInitialized(bitmap, tick, tickSpacing) {
...
async function deriveTickPda(pool, tickIndex) {
...
export function invariantWindowFor(cfg) {
 ...
 */
async function resolveWindowFromBitmap(load, pool, bitmap, currentTick, tickSpacing, xToY) {
    const boundaries = [];
    ...
    for (let i = 0; i < INVARIANT_MAX_BOUNDARIES; i++) {
        ...
        if (data === null || data.length !== TICK_ACCOUNT_SIZE || !hasDiscriminator(data, TICK_DISCRIMINATOR))
            break;
...
export async function fetchInvariantPoolConfig(load, pool) {
    ...
    if (!hasDiscriminator(data, POOL_DISCRIMINATOR))
        throw new Error(`${SLUG}: pool ${pool} has a foreign discriminator (not a Pool account)`);
    const codec = getAddressCodec();
    const tokenXMint = codec.decode(data.subarray(OFF_TOKEN_X, OFF_TOKEN_X + 32));
    const tokenYMint = codec.decode(data.subarray(OFF_TOKEN_Y, OFF_TOKEN_Y + 32));
    const tokenXReserve = codec.decode(data.subarray(OFF_TOKEN_X_RESERVE, OFF_TOKEN_X_RESERVE + 32));
    const tokenYReserve = codec.decode(data.subarray(OFF_TOKEN_Y_RESERVE, OFF_TOKEN_Y_RESERVE + 32));
    const tickmap = codec.decode(data.subarray(OFF_TICKMAP, OFF_TICKMAP + 32));
    ...
    if (tickmapData === null)
        throw new Error(`${SLUG}: tickmap ${tickmap} of pool ${pool} not found`);
...
function effectiveWindow(cfg, state, live) {
    const xToY = cfg.direction === 'xToY';
    const window = invariantWindowFor(cfg);
    const result = { valid: false, targets: [], signs: [], liquidityChanges: [] };
    for (const boundary of window.boundaries) {
        ...
        if (data === undefined)
            throw new Error(`${SLUG} ladder reference is missing account ${boundary.address}`);
        if (readUintLE(data, 0, 8) !== TICK_DISC_U64)
            continue; // stale/foreign account since prepare — skip it
...
function walkStep(cursor, win, fee, xToY, rm) {
...
function emitWalkStep(p, xToY, k, v, indent) {
    const dxUp = (lo, hi) => `ivDeltaX(${lo}, ${hi}, ${v.l}, 1)`;
    const dyUp = (lo, hi) => `ivDeltaY(${lo}, ${hi}, ${v.l}, 1)`;
    const tg = `${p}tg${k}`;
    return [
...
export const invariantLadder = {
    ...
    quoteRefs(base, slot) {
        ...
        return [
            ...
            ...window.boundaries.map((boundary, i) => ({ ref: ref(slot, `t${i}`), address: boundary.address })),
        ];
    },
    emitSetup(base, slot, params, enableVar) {
        ...
        for (let k = 0; k < window.boundaries.length; k++) {
            ...
            lines.push(`  const ${p}tg${k} = ${target};`, `  let ${p}sg${k} = 0; let ${p}ln${k} = 0; let ${p}ok${k} = 0;`, `  if (${enabled} !== 0 && (${keep})) {`, `    if (accountUint(${tickRef}, 0, 8) === ${TICK_DISC_U64}) {`, `      ${p}sg${k} = accountUint(${tickRef}, ${OFF_TICK_SIGN}, 1);`, `      ${p}ln${k} = accountUint(${tickRef}, ${OFF_TICK_LIQUIDITY_CHANGE}, 16);`, `      ${p}ok${k} = 1;`, `      ${p}nb = ${p}nb + 1;`, `    }`, `  }`);
        }
        lines.push(`  let ${p}vld = 0;`, `  if (${p}l0 > 0) { ${p}vld = 1; }`);
        lines.push(`  let ${p}t1 = 0; let ${p}ca = 0; let ${p}fx = 0; let ${p}nx = 0; let ${p}i2 = 0; let ${p}ou = 0; let ${p}fe = 0;`, `  let ${p}wsp = 0; let ${p}wl = 0; let ${p}wex = 0; let ${p}wout = 0; let ${p}rm = 0;`, `  let ${p}lo = 0; let ${p}lx = 0; let ${p}wcx = 0;`);
        return lines.join('\n');
    },
    emitLadderQuote(base, slot, rung, x, outVar) {
        ...
        lines.push(`      if (${v.ex} === 0 && ${v.rm} === 0) { ${p}lo = ${v.out}; ${p}lx = ${x}; }`, `      else { ${p}lo = ${v.out}; ${p}lx = ${x} - ${v.rm}; ${p}wcx = 1; }`, `    }`, `    const ${outVar} = ${p}lo;`);
        return lines.join('\n');
    },
    ...
    continuousFees(base, state) {
        const cfg = invariantConfig(base);
        const live = liveFromState(cfg, state);
        const feePpm = live.fee / 1000000n; // fee is 1e12-scaled; ppm is 1e6-scaled
        return { gammaPpm: 1000000n - feePpm, muPpm: 1000000n };
    },
};
