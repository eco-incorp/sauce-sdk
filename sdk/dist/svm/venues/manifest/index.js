...
export const MARKET_DISCRIMINANT = 4859840929024028656n;
/** Price is stored as QuoteAtomsPerBaseAtom.inner = price * 1e18 (u128 LE). */
export const PRICE_D18 = 1000000000000000000n;
...
export function manifestWindowFor(cfg) {
...
const nodeBase = (dataIndex) => MARKET_FIXED_SIZE + dataIndex;
const treeLeft = (data, idx) => Number(readUintLE(data, nodeBase(idx) + OFF_NODE_LEFT, 4));
const treeRight = (data, idx) => Number(readUintLE(data, nodeBase(idx) + OFF_NODE_RIGHT, 4));
const treeParent = (data, idx) => Number(readUintLE(data, nodeBase(idx) + OFF_NODE_PARENT, 4));
function isLeftChild(data, idx) {
 ...
 */
function nextLower(data, idx) {
    if (idx === NIL)
        return NIL;
    const left = treeLeft(data, idx);
    if (left !== NIL) {
        let cur = left;
        for (let guard = 0; guard < 1_000_000 && treeRight(data, cur) !== NIL; guard++)
            cur = treeRight(data, cur);
        return cur;
    }
    let cur = idx;
    for (let guard = 0; guard < 1_000_000 && isLeftChild(data, cur); guard++)
        cur = treeParent(data, cur);
 ...
 */
function resolveWindow(data, bestOffset) {
    const orders = [];
    ...
    for (let guard = 0; guard < 1_000_000 && cur !== NIL && orders.length < MANIFEST_MAX_ORDERS; guard++) {
        ...
        if (orderType === ORDER_TYPE_GLOBAL)
            break; // taker IOC halts at the first global maker
        if (lastValidSlot !== NO_EXPIRATION_LAST_VALID_SLOT)
            break; // the model carries no clock
 ...
 * account.
 */
export async function fetchManifestConfig(load, pool) {
    const data = await load(pool);
    if (data === null)
        throw new Error(`${SLUG}: market account ${pool} not found`);
