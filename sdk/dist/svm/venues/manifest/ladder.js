...
import { MANIFEST_MAX_ORDERS, MANIFEST_PROGRAM_ID, MARKET_FIXED_SIZE, OFF_ORDER_PRICE, OFF_ORDER_SEQ, OFF_ORDER_SIZE, PRICE_D18, manifestWindowFor, } from './index.js';
...
export function manifestQuoteForBase(inner, base, roundUp) {
    if (base === 0n)
        return 0n;
    const p = inner * base;
    if (p >= Q128)
...
function coldWalk(levels, baseIn, x) {
    if (x <= 0n)
        return 0n;
    let remaining = x;
    let out = 0n;
        ...
        if (baseIn) {
            ...
            if (remaining > cap) {
                ...
                if (blAtCap >= level.size) {
                    ...
                    if (level.aux >= SENT)
                        break; // defensive — unreachable once blAtCap >= size
                    out += level.size;
                    remaining -= level.aux;
                }
                else {
                ...
                if (bl >= level.size) {
                    if (level.aux >= SENT)
                        break; // full-order quote itself overflows — venue would reject
                    out += level.size;
 ...
 */
function coldWalkConsumed(levels, baseIn, x) {
    if (x <= 0n)
...
function emitWalk(p, baseIn, xExpr, outVar, tag, indent, decl) {
    const step = baseIn
        ? [
...
function emitOrderRead(p, slot, k, baseIn, params) {
...
export const manifestLadder = {
    ...
    referenceQuote(base, state, params) {
        ...
        return (x) => coldWalk(levels, baseIn, x);
    },
    ...
    referenceCapacities(base, state, params) {
        ...
        const baseIn = cfg.direction === 'baseIn';
        const levels = effectiveLevels(cfg, state, params);
        return (grid) => grid.map((g) => (g <= 0n ? 0n : coldWalkConsumed(levels, baseIn, g).consumed));
    },
    /**
