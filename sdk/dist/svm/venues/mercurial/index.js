...
const FEE_DENOMINATOR = 10000000000n;
function mercurialConfig(cfg) {
    if (cfg.venue !== SLUG)
 ...
 */
async function deriveMercurialAuthority(pool, nonce) {
    const encoder = getAddressEncoder();
    const digest = createHash('sha256')
        .update(encoder.encode(pool))
        .update(Uint8Array.of(nonce))
        .update(encoder.encode(MERCURIAL_PROGRAM_ID))
...
export async function fetchMercurialPoolConfig(load, pool) {
    ...
    if (data[0] !== 2)
        throw new Error(`${SLUG}: pool ${pool} is not SwapV2 (version tag ${data[0]})`);
    if (data[1] !== 1)
    ...
    if (vaultAData === null)
        throw new Error(`${SLUG}: pool ${pool} vault A ${vaultA} not found`);
    if (vaultBData === null)
        throw new Error(`${SLUG}: pool ${pool} vault B ${vaultB} not found`);
 ...
 * multiplier pool).
 */
function outFromVirtualY(live, y) {
    if (live.vdst <= y)
        return 0n;
...
export const mercurialLadder = {
    ...
    emitLadderQuote(_base, slot, rung, x, outVar) {
        ...
        lines.push(`    let ${outVar} = 0;`, `    if (s${slot}d > 0 && ${x} > 0) {`, `      s${slot}wy = stableYW(s${slot}amp, s${slot}vsrc + ${x} * s${slot}smu, s${slot}d, s${slot}wy);`, `      if (s${slot}vdst > s${slot}wy) {`, `        const s${slot}dv${rung} = s${slot}vdst - s${slot}wy - 1;`, `        const s${slot}fv${rung} = Math.mulDiv(s${slot}dv${rung}, s${slot}fn, ${FEE_DENOMINATOR});`, `        ${outVar} = (s${slot}dv${rung} - s${slot}fv${rung}) / s${slot}dmu;`, '      }', '    }');
        return lines.join('\n');
    },
    ...
    continuousFees(base, state) {
        ...
        return { gammaPpm: 1000000n, muPpm: 1000000n - (live.fn * 1000000n) / FEE_DENOMINATOR };
    },
};
