...
async function loadAccount(load, addr, what) {
    ...
    if (data === null)
        throw new Error(`moonit ${what} ${addr} not found`);
    return data;
}
...
function bakeCoefficients(totalSupplyRaw, coefBRaw, marketcapThresholdRaw, decimals) {
    const tdScale = 10n ** decimals;
    const cdScale = 1000000000n; // Sol, the only supported collateral currency
    const mcdScale = 1000000000n; // Sol, the only supported marketcap currency
...
function livePosition(cfg, state) {
...
function referenceBuy(y, m, aw, bw, td, feeBps, curveAmount) {
    if (y === 0n)
        return 0n;
    ...
    const cScaled = mulDiv(SCALE_A, net, 1000000000n);
...
function referenceSell(x, m, aw, bw, td, feeBps) {
    if (x === 0n)
        return 0n;
    const xx = x > m ? m : x;
    const twoMN = 2n * m - xx;
    ...
    const term1 = mulDiv(step3, 1000000000n, SCALE_A);
    const stepB = mulDiv(bw, xx, td);
    const term2 = mulDiv(stepB, 1000000000n, SCALE_B);
    const gross = term1 + term2;
    const fee = mulDiv(gross, feeBps, 10000n);
    return gross - fee;
}
export const moonitLadder = {
    slug: SLUG,
    ...
    referenceQuote(base, state, params) {
        ...
        if (cfg.direction === 'quoteToBase') {
            return (y) => referenceBuy(y, m, aw, bwFixed, tdScale, feeBps, curveAmount);
        }
        return (x) => referenceSell(x, m, aw, bwFixed, tdScale, feeBps);
    },
    depthReserves(base, state) {
        const cfg = asCfg(base);
        const { m, curveAmount } = livePosition(cfg, state);
        const [awHi, awLo, bwFixed, tdScale] = moonitLadder.paramsFor(cfg);
        const aw = (awHi << 64n) | awLo;
    ...
    continuousFees(base, _state, params) {
        ...
        return cfg.direction === 'quoteToBase' ? { gammaPpm: kept, muPpm: 1000000n } : { gammaPpm: 1000000n, muPpm: kept };
    },
};
