...
const pairOff = (i, field) => PAIRS_BASE + i * PAIR_STRIDE + field;
/**
...
function isqrt(value) {
    if (value < 0n)
        throw new Error(`${SLUG} isqrt needs a non-negative value, got ${value}`);
    if (value < 2n)
        return value;
    let x = value;
    let y = (x + 1n) / 2n;
...
export function perenaSwapOut(live, x) {
    ...
    return mulDivFloor(raw, 10000n - MARGIN_BPS, 10000n);
}
async function fetchPoolConfig(load, pool) {
    ...
    const rateOkAtFetch = p0.rateNum === p0.rateDenom && p0.rateNum > 0n && p1.rateNum === p1.rateDenom && p1.rateNum > 0n;
...
function quoteAccounts(cfg) {
    const c = asPerenaConfig(cfg);
    return [{ ref: poolRef(c, 'pool'), address: c.pool }];
}
function liveStateFrom(c, pool) {
    const inIdx = c.direction === 'aToB' ? 0 : 1;
    const outIdx = c.direction === 'aToB' ? 1 : 0;
    return {
        ampRaw: c.ampRaw,
        aRaw: c.aRaw,
 ...
 */
function readLines(c, poolRefJson, readPrefix) {
    const inIdx = c.direction === 'aToB' ? 0 : 1;
    const outIdx = c.direction === 'aToB' ? 1 : 0;
    const p = readPrefix;
 ...
 */
function arithmeticLines(c, readPrefix, tmpPrefix, x, outVar) {
    const r = readPrefix;
    const p = tmpPrefix;
    ...
    return [
        ...
        `                ${outVar} = Math.mulDiv(${p}raw, ${10000n - MARGIN_BPS}, 10000);`,
        '              }',
        '            }',
        '          }',
        '        }',
 ...
 */
function perenaAccounts(c, user, refFn) {
...
const poolRef = (c, role) => `${SLUG}:${c.pool}:${role}`;
...
function referenceQuote(cfg, state, amountIn, _now) {
    const c = asPerenaConfig(cfg);
    const pool = state[c.pool];
    if (pool === undefined)
        throw new Error(`${SLUG} referenceQuote is missing account ${c.pool}`);
...
export const perenaLadder = {
    ...
    continuousFees(cfg, state) {
        ...
        const pool = state[c.pool];
        if (pool === undefined)
            throw new Error(`${SLUG} ladder fees are missing account ${c.pool}`);
        const fn = readUintLE(pool, OFF_FEE_NUM, 4);
        const fd = readUintLE(pool, OFF_FEE_DENOM, 4);
        return { gammaPpm: 1000000n, muPpm: fd === 0n ? 1000000n : 1000000n - (fn * 1000000n) / fd };
    },
};
