...
async function ata(owner, mint, tokenProgram) {
    ...
    return pda([new Uint8Array(encoder.encode(owner)), new Uint8Array(encoder.encode(tokenProgram)), new Uint8Array(encoder.encode(mint))], ASSOCIATED_TOKEN_PROGRAM);
}
/**
...
function detectTokenProgram(mint, data) {
    ...
    while (offset + 4 <= data.length) {
        ...
        if (extensionType === 1) {
            throw new Error(`pumpswap gate: mint ${mint} carries a token-2022 TransferFeeConfig extension (vault deltas would not match user amounts)`);
        }
...
async function loadAccount(load, addr, what) {
    ...
    if (data === null)
        throw new Error(`pumpswap ${what} ${addr} not found`);
    return data;
}
/**
 ...
 */
function buyEffectiveQuoteIn(spendable, lp, protocol, creator) {
    ...
    if (eff < 2n)
        throw new Error(`pumpswap quote amountIn ${spendable} is too small (effective quote input below 2)`);
    return eff;
}
function writeU64LE(data, offset, value) {
    for (let i = 0; i < 8; i++)
        data[offset + i] = Number((value >> BigInt(8 * i)) & 0xffn);
}
function checkAmountIn(amountIn) {
    if (amountIn <= 0n || amountIn > U64_MAX) {
        throw new Error(`pumpswap amountIn must be a positive u64, got ${amountIn}`);
    }
}
const readonly = (addr) => ({ ref: addr, address: addr });
const writable = (addr) => ({ ref: addr, address: addr, writable: true });
export const pumpswapAdapter = {
    ...
    async fetchPoolConfig(load, pool) {
        ...
        const [globalConfigData, feeConfigData, baseMintData, quoteMintData, baseVaultData, quoteVaultData] = await Promise.all([
        ...
        const encoder = getAddressEncoder();
        const poolAuthority = await pda(['pool-authority', new Uint8Array(encoder.encode(baseMint))], PUMP_BONDING_PROGRAM);
        ...
        if (canonical) {
            ...
            if (tiers.length === 0)
                throw new Error(`pumpswap fee config ${FEE_CONFIG} has no fee tiers`);
        ...
        if (coinCreator === DEFAULT_PUBKEY)
            creatorFeeBps = 0n;
        const protocolFeeRecipient = pubkeyAt(globalConfigData, 57);
        const buybackFeeRecipient = pubkeyAt(globalConfigData, 643);
        const coinCreatorVaultAuthority = await pda(['creator_vault', new Uint8Array(encoder.encode(coinCreator))], PUMPSWAP_PROGRAM_ID);
        const [protocolFeeRecipientTokenAccount, coinCreatorVaultAta, buybackFeeRecipientTokenAccount, poolV2] = await Promise.all([
    ...
    buildSwap(cfg, user, amountIn) {
        ...
        if (c.poolV2 !== undefined)
            accounts.push(readonly(c.poolV2));
    ...
    referenceQuote(cfg, state, amountIn, _now) {
        ...
        if (baseVaultData === undefined)
            throw new Error(`pumpswap referenceQuote is missing base vault ${c.baseVault} in state`);
        if (quoteVaultData === undefined)
            throw new Error(`pumpswap referenceQuote is missing quote vault ${c.quoteVault} in state`);
        const baseReserve = readUintLE(baseVaultData, 64, 8);
        const quoteReserve = readUintLE(quoteVaultData, 64, 8);
        if (c.direction === 'quoteToBase') {
            ...
            const fees = ceilDiv(eff * c.lpFeeBps, BPS) + ceilDiv(eff * c.protocolFeeBps, BPS) + ceilDiv(eff * c.creatorFeeBps, BPS);
            const over = eff + fees - amountIn;
            if (over > 0n)
                eff -= over;
            if (eff < 2n)
                throw new Error(`pumpswap quote amountIn ${amountIn} is too small (effective quote input below 2)`);
