...
export function arrayStartIndex(tickIndex, tickSpacing) {
    const n = TICK_ARRAY_SIZE * tickSpacing;
    let start = Math.trunc(tickIndex / n);
    if (tickIndex < 0 && tickIndex % n !== 0)
        start -= 1;
    return start * n;
}
/**
 ...
 */
export function windowStartTicks(tickCurrentIndex, tickSpacing, zeroForOne) {
...
function tickHasLimitOrders(array, offset) {
    ...
    return (readUintLE(array, base + OFF_TICK_ORDERS_AMOUNT, 8) !== 0n ||
...
async function resolveWindow(load, pool, tickCurrentIndex, tickSpacing, zeroForOne) {
    ...
    if (readable > 0 && !maxStopped) {
        ...
        if (zeroForOne) {
            ...
            if (lastStart + TICK_ARRAY_SIZE * tickSpacing > MAX_TICK)
                tick = MAX_TICK;
        }
 ...
 */
export async function fetchRaydiumClmmConfig(load, pool) {
