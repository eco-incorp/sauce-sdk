...
const CONFIG_OFFSETS = { tradeFeeRate: 12, creatorFeeRate: 108 };
/** SPL token account (Tokenkeg and Token-2022): mint @0, amount u64 LE @64. */
const VAULT_MINT_OFFSET = 0;
const VAULT_AMOUNT_OFFSET = 64;
function cpConfig(cfg) {
    if (cfg.venue !== SLUG)
        throw new Error(`raydium-cp-swap adapter got a '${cfg.venue}' pool config`);
    return cfg;
}
/**
...
function requireU64Amount(amountIn) {
...
function u64le(value) {
    const bytes = [];
    for (let i = 0n; i < 8n; i++)
        bytes.push(Number((value >> (8n * i)) & 0xffn));
    return bytes;
}
...
function creatorFeeOnInput(cfg) {
    return (cfg.creatorFeeOn === 0 ||
...
export const raydiumCpSwap = {
    ...
    async fetchPoolConfig(load, pool) {
        ...
        if (data === null)
            throw new Error(`raydium-cp-swap pool ${pool} not found`);
        ...
        if (configData === null)
            throw new Error(`raydium-cp-swap amm config ${ammConfig} not found`);
        ...
        for (const [vault, mint] of [
            ...
            if (vaultData === null)
                throw new Error(`raydium-cp-swap vault ${vault} not found`);
    ...
    emitQuote(base, i, amountIn) {
        ...
        const reserve = (side, name) => `  const ${name}${i} = accountUint(${JSON.stringify(side.vault)}, ${VAULT_AMOUNT_OFFSET}, 8)` +
        ...
        const ceil = (product) => `(${product} + ${FEE_RATE_DENOMINATOR - 1n}) / ${FEE_RATE_DENOMINATOR}`;
        ...
        if (creatorFeeOnInput(cfg)) {
            ...
            lines.push(`  const fee${i} = ${ceil(`${amountIn} * ${rate}`)};`, `  const net${i} = ${amountIn} - fee${i};`, `  const q${i} = Math.mulDiv(net${i}, rout${i}, rin${i} + net${i});`);
        }
        else {
            // trade fee on input, creator fee ceil'd out of the swapped output.
            lines.push(`  const fee${i} = ${ceil(`${amountIn} * ${tradeRate}`)};`, `  const net${i} = ${amountIn} - fee${i};`, `  const os${i} = Math.mulDiv(net${i}, rout${i}, rin${i} + net${i});`, `  const q${i} = os${i} - ${cfg.enableCreatorFee ? `(${ceil(`os${i} * ${creatorRate}`)})` : '0'};`);
        }
        return lines.join('\n');
    },
    buildSwap(base, user, amountIn) {
        const cfg = cpConfig(base);
        requireU64Amount(amountIn);
        const { input, output } = sides(cfg);
        ...
        const fixed = (address, writable) => writable ? { ref: address, address, writable: true } : { ref: address, address };
        return {
            programId: address(PROGRAM_ID),
            data,
            accounts: [
    ...
    referenceQuote(base, state, amountIn, now) {
        const cfg = cpConfig(base);
        requireU64Amount(amountIn);
        const bytes = (address) => {
            const data = state[address];
            if (data === undefined)
                throw new Error(`raydium-cp-swap referenceQuote is missing account ${address}`);
            return data;
        };
        ...
        const reserve = (vault, protocolFees, fundFees, creatorFees) => {
            const value = readUintLE(bytes(vault), VAULT_AMOUNT_OFFSET, 8) -
                readUintLE(pool, protocolFees, 8) -
                readUintLE(pool, fundFees, 8) -
                readUintLE(pool, creatorFees, 8);
            if (value < 0n)
                throw new Error(`raydium-cp-swap vault ${vault} balance is below its accrued fees`);
            return value;
        };
        ...
        const feeOnInput = creatorFeeOn === 0 ||
