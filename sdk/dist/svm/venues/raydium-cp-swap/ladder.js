...
const PROTOCOL_FEES = [341, 349]; // token0, token1
const FUND_FEES = [357, 365];
const CREATOR_FEES = [397, 405];
...
export const raydiumCpSwapLadder = {
    ...
    emitSetup(base, slot, params) {
        ...
        const reserve = (side, vaultRef) => `accountUint(${JSON.stringify(ref(slot, vaultRef))}, ${VAULT_AMOUNT_OFFSET}, 8)` +
            ` - accountUint(${pool}, ${PROTOCOL_FEES[side]}, 8)` +
            ` - accountUint(${pool}, ${FUND_FEES[side]}, 8)` +
            ` - accountUint(${pool}, ${CREATOR_FEES[side]}, 8)`;
        return [
            `  const s${slot}rin = ${reserve(inSide, 'vin')};`,
            `  const s${slot}rout = ${reserve(outSide, 'vout')};`,
    ...
    referenceQuote(base, state, params) {
        ...
        const inSide = cfg.inputIsToken0 ? 0 : 1;
        const outSide = 1 - inSide;
        const reserve = (side, vault) => readUintLE(bytes(vault), VAULT_AMOUNT_OFFSET, 8) -
    ...
    continuousFees(base, state, params) {
        ...
        if (config === undefined)
            throw new Error(`${SLUG} ladder fees are missing account ${cfg.ammConfig}`);
        const tr = readUintLE(config, TRADE_FEE_RATE, 8);
        const cr = readUintLE(config, CREATOR_FEE_RATE, 8);
        const crMode = params[0];
        return {
