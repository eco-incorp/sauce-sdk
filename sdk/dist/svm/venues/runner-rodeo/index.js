...
export const runnerRodeoLadder = {
    slug: SLUG,
    shapeKey(base) {
    ...
    referenceQuote(base, state, params) {
        ...
        if (quoteVaultBytes === undefined)
            throw new Error(`${SLUG} ladder reference is missing account ${cfg.quoteVault}`);
    ...
    depthReserves(base, state) {
        ...
        const quoteVaultBytes = state[String(cfg.quoteVault)];
        if (quoteVaultBytes === undefined)
            throw new Error(`${SLUG} ladder depth is missing account ${cfg.quoteVault}`);
        const rEff = readUintLE(stateBytes, 144, 8) - readUintLE(stateBytes, 160, 8);
        const qEff = readUintLE(quoteVaultBytes, 64, 8) + readUintLE(stateBytes, 152, 8);
    ...
    continuousFees(base, _state, params) {
        ...
        if (cfg.direction === 'quoteToBase') {
            return { gammaPpm: ((BPS - totalBps) * 1000000n) / BPS, muPpm: 1000000n };
        }
        return { gammaPpm: 1000000n, muPpm: ((BPS - totalBps) * 1000000n) / BPS };
    },
};
