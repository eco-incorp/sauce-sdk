...
async function fetchPoolConfig(load, pool) {
    ...
    if (data === null)
        throw new Error(`${SLUG} wrapper ${pool} account not found`);
...
export const saberDecimalsWrapperLadder = {
    slug: SLUG,
    ...
    emitSetup(base, slot, params) {
        ...
        if (cfg.direction === 'withdraw') {
            ...
            lines.push(`  let s${slot}cap = ${U64_MAX} - s${slot}mult * s${slot}bal;`, `  if (s${slot}cap < 0) { s${slot}cap = 0 }`);
        }
        return lines.join('\n');
    },
    ...
    referenceQuote(base, state) {
        ...
        if (data === undefined)
            throw new Error(`${SLUG} reference is missing vault ${cfg.vault}`);
    ...
    depthReserves(base, state) {
        ...
        const data = state[cfg.vault];
        if (data === undefined)
            throw new Error(`${SLUG} depth is missing vault ${cfg.vault}`);
        const bal = readUintLE(data, VAULT_AMOUNT_OFFSET, 8);
        if (cfg.direction === 'withdraw') {
