...
function asSaberConfig(cfg) {
    if (cfg.venue !== SLUG)
        throw new Error(`saber-stableswap adapter got a config for venue '${cfg.venue}'`);
    return cfg;
}
function assertU64AmountIn(amountIn) {
    if (amountIn <= 0n)
        throw new Error(`saber-stableswap amountIn must be positive, got ${amountIn}`);
    if (amountIn > U64_MAX)
        throw new Error(`saber-stableswap amountIn must fit u64, got ${amountIn}`);
}
/**
 ...
 */
function deriveSwapAuthority(pool, nonce) {
    const encoder = getAddressEncoder();
    const digest = createHash('sha256')
        .update(encoder.encode(pool))
        .update(Uint8Array.of(nonce))
        .update(encoder.encode(SABER_STABLESWAP_PROGRAM_ID))
 ...
 */
function computeAmpFactor(cfg, now) {
...
export const saberStableswap = {
    ...
    async fetchPoolConfig(load, pool) {
        ...
        if (data === null)
            throw new Error(`saber-stableswap pool account ${pool} not found`);
        ...
        if (data[0] !== 1)
            throw new Error(`saber-stableswap pool ${pool} is not initialized (is_initialized = ${data[0]})`);
        if (data[1] !== 0)
            throw new Error(`saber-stableswap pool ${pool} is paused (is_paused = ${data[1]})`);
    ...
    emitQuote(cfg, i, amountIn) {
        ...
        lines.push(`  const d${i} = stableD(amp${i}, srcRes${i}, dstRes${i});`, `  const y${i} = stableY(amp${i}, srcRes${i} + ${amountIn}, d${i});`, `  let q${i} = 0;`, `  if (paused${i} === 0 && dstRes${i} > y${i}) {`, `    const dy${i} = dstRes${i} - y${i} - 1;`, `    q${i} = dy${i} - Math.mulDiv(dy${i}, ${c.tradeFeeNumerator}, ${c.tradeFeeDenominator});`, `  }`);
        return lines.join('\n');
    },
    ...
    referenceQuote(cfg, state, amountIn, now) {
        const c = asSaberConfig(cfg);
        if (amountIn < 0n)
            throw new Error(`saber-stableswap amountIn must be non-negative, got ${amountIn}`);
        if (amountIn === 0n)
            return 0n; // on-chain amount_in == 0 is a no-op success
        if (amountIn > U64_MAX)
            throw new Error(`saber-stableswap amountIn must fit u64, got ${amountIn}`);
        const poolData = state[c.pool];
        if (poolData === undefined)
            throw new Error(`saber-stableswap referenceQuote state is missing pool ${c.pool}`);
        const vaultAData = state[c.vaultA];
        if (vaultAData === undefined)
            throw new Error(`saber-stableswap referenceQuote state is missing vault ${c.vaultA}`);
        const vaultBData = state[c.vaultB];
        if (vaultBData === undefined)
            throw new Error(`saber-stableswap referenceQuote state is missing vault ${c.vaultB}`);
        if (poolData[1] !== 0)
            return 0n; // paused — the fragment quotes 0
        ...
        if (y > U64_MAX)
            throw new Error(`saber-stableswap pool ${c.pool} compute_y result exceeds u64`);
        if (dstReserve <= y)
            return 0n; // checked_sub would fail on-chain — no output
