...
import { SANCTUM_INFINITY_REGISTRY, WSOL_MINT, } from './registry.js';
...
export function sanctumInfinityPoolKey(inMint, outMint) {
 ...
 */
async function resolveInfinityLeg(load, mint, lstStateList) {
    ...
    if (lstIndex < 0)
        throw new Error(`${SLUG}: mint ${mint} is not in the live lst_state_list`);
    ...
    const reg = SANCTUM_INFINITY_REGISTRY.get(mint);
    if (reg === undefined)
        throw new Error(`${SLUG}: mint ${mint} has no vendored stake-pool registry entry`);
    const constants = CALC_CONSTANTS[reg.family];
    if (constants.calcProgram !== solValueCalculator) {
        throw new Error(`${SLUG}: mint ${mint} registry family '${reg.family}' disagrees with live sol_value_calculator ${solValueCalculator}`);
    }
    const stakePoolData = await load(reg.stakePool);
    if (stakePoolData === null)
        throw new Error(`${SLUG}: stake pool ${reg.stakePool} does not exist`);
...
function readSlabFees(slab, mint) {
...
export const sanctumInfinity = {
    ...
    async fetchPoolConfig(load, pool) {
        ...
        if (poolStateData === null)
            throw new Error(`${SLUG}: pool_state ${POOL_STATE_ID} does not exist`);
        if (poolStateData[13] !== 0)
            throw new Error(`${SLUG}: pool is disabled`);
        if (poolStateData[14] !== 0)
            throw new Error(`${SLUG}: pool is rebalancing`);
        if (lstStateList === null)
            throw new Error(`${SLUG}: lst_state_list does not exist`);
        if (slab === null)
            throw new Error(`${SLUG}: slab does not exist`);
        ...
        if (inLeg.isInputDisabled)
            throw new Error(`${SLUG}: input LST ${pair.inMint} has input disabled`);
        ...
        if (feeNanosSum < 0n)
            throw new Error(`${SLUG}: ${pair.inMint}->${pair.outMint} is net-negative-fee (disallowed)`);
        if (feeNanosSum > 1000000000n)
            throw new Error(`${SLUG}: ${pair.inMint}->${pair.outMint} fee exceeds 100%`);
    ...
    quoteAccounts(base) {
        ...
        if (cfg.in.stakePool !== undefined)
            accs.push({ ref: `${cfg.pool}:inStakePool`, address: cfg.in.stakePool });
        if (cfg.out.stakePool !== undefined)
            accs.push({ ref: `${cfg.pool}:outStakePool`, address: cfg.out.stakePool });
        return accs;
    },
};
// ── ladder ────────────────────────────────────────────────────────────────
...
export const sanctumInfinityLadder = {
    ...
    quoteRefs(base, slot) {
        const cfg = infinityConfig(base);
        const accs = [{ ref: ref(slot, 'outPoolReserves'), address: cfg.out.poolReserves }];
        ...
        if (cfg.in.stakePool !== undefined)
            accs.push({ ref: ref(slot, 'inStakePool'), address: cfg.in.stakePool });
        if (cfg.out.stakePool !== undefined)
            accs.push({ ref: ref(slot, 'outStakePool'), address: cfg.out.stakePool });
        return accs;
    },
    ...
    buildSwapV2(base, slot, user) {
        ...
        if (cfg.in.isWsol) {
            ...
            accounts.push({ ref: ref(slot, 'inCalcProgram'), address: c.calcProgram }, { ref: ref(slot, 'inCalcState'), address: c.calcState }, { ref: ref(slot, 'inStakePool'), address: cfg.in.stakePool }, { ref: ref(slot, 'inCalcPoolProgram'), address: c.poolProgram }, { ref: ref(slot, 'inCalcPoolProgramData'), address: c.poolProgramData });
        }
        if (cfg.out.isWsol) {
            ...
            const c = CALC_CONSTANTS[family];
            accounts.push({ ref: ref(slot, 'outCalcProgram'), address: c.calcProgram }, { ref: ref(slot, 'outCalcState'), address: c.calcState }, { ref: ref(slot, 'outStakePool'), address: cfg.out.stakePool }, { ref: ref(slot, 'outCalcPoolProgram'), address: c.poolProgram }, { ref: ref(slot, 'outCalcPoolProgramData'), address: c.poolProgramData });
        }
        accounts.push({ ref: ref(slot, 'pricingProgram'), address: FLAT_SLAB_PROGRAM_ID }, { ref: ref(slot, 'slab'), address: SLAB_ID });
        return {
            programId: SANCTUM_INFINITY_PROGRAM_ID,
            prefix,
    ...
    referenceQuote(base, state, params) {
        ...
        const inTL = cfg.in.isWsol ? 1n : readUintLE(bytes(cfg.in.stakePool), OFF_SPL_TOTAL_LAMPORTS, 8);
        const inPTS = cfg.in.isWsol ? 1n : readUintLE(bytes(cfg.in.stakePool), OFF_SPL_POOL_TOKEN_SUPPLY, 8);
        const outTL = cfg.out.isWsol ? 1n : readUintLE(bytes(cfg.out.stakePool), OFF_SPL_TOTAL_LAMPORTS, 8);
        const outPTS = cfg.out.isWsol ? 1n : readUintLE(bytes(cfg.out.stakePool), OFF_SPL_POOL_TOKEN_SUPPLY, 8);
        ...
        return (x) => {
            ...
            if (inSolValue === 0n)
                return 0n;
            const postFeeNanos = 1000000000n - feeNanosSum;
            if (postFeeNanos < 0n)
                return 0n;
            const outSolValue = (inSolValue * postFeeNanos) / 1000000000n;
            if (outSolValue === 0n)
                return 0n;
    ...
    continuousFees(_base, _state, params) {
        ...
        const feePpm = feeNanosSum / 1000n;
        let gammaPpm = 1000000n - feePpm;
        if (gammaPpm < 0n)
            gammaPpm = 0n;
        return { gammaPpm, muPpm: 1000000n };
    },
};
function registryFamilyFor(leg) {
    const reg = SANCTUM_INFINITY_REGISTRY.get(leg.mint);
    if (reg === undefined)
        throw new Error(`${SLUG}: mint ${leg.mint} has no registry entry (buildSwapV2)`);
    return reg.family;
}
/** Test/tooling seam: clear the discovery-key registry between fixtures. */
export function __resetSanctumInfinityKeysForTest() {
    PAIR_BY_KEY.clear();
}
/** Test/tooling seam: look up a previously-minted discovery key (mirrors what `fetchPoolConfig` does). */
export function sanctumInfinityLookupPair(pool) {
    return PAIR_BY_KEY.get(pool);
}
