...
class Cursor {
    ...
    u8() {
        ...
        if (v === undefined)
            throw new Error('sanctum-stake-pool: truncated account data');
        this.o += 1;
        return v;
    }
    u64() {
        if (this.o + 8 > this.buf.length)
            throw new Error('sanctum-stake-pool: truncated account data');
        let v = 0n;
        for (let i = 7; i >= 0; i--)
            v = (v << 8n) | BigInt(this.buf[this.o + i]);
        this.o += 8;
        return v;
    }
    pubkey() {
        if (this.o + 32 > this.buf.length)
            throw new Error('sanctum-stake-pool: truncated account data');
...
function withdrawAuthorityAddress(programId, pool, bump) {
...
function getPubkeyBytes(addr) {
    return Uint8Array.from(getAddressEncoder().encode(addr));
}
async function fetchSanctumStakePoolConfig(load, pool, programId, slug) {
    ...
    if (clockData === null)
        throw new Error('sanctum-stake-pool: Clock sysvar account not found');
...
function makeSanctumStakePool(slug, programId) {
    return {
        slug,
        programId,
        fetchPoolConfig: (load, pool) => fetchSanctumStakePoolConfig(load, pool, programId, slug),
    };
}
...
// ---------------------------------------------------------------------------
function sanctumConfig(slug, base) {
    if (base.venue !== slug)
        throw new Error(`${slug} ladder adapter got a '${base.venue}' pool config`);
    return base;
}
const ref = (slot, role) => `s${slot}:${role}`;
function makeSanctumStakePoolLadder(slug, programId) {
    ...
    return {
        ...
        referenceQuote(base, state, params) {
            ...
            return (x) => {
                if (x === 0n)
                    return 0n;
                let minted = x;
                if (total > 0n && supply > 0n)
                    minted = (x * supply) / total;
                if (minted === 0n)
                    return 0n;
                let fee = 0n;
                if (fd > 0n && fn > 0n) {
        ...
        continuousFees(_base, _state, params) {
            ...
            let gamma = 1000000n;
            if (fn > 0n && fd > 0n)
                gamma -= (fn * 1000000n) / fd;
            return { gammaPpm: gamma, muPpm: 1000000n };
        },
    };
}
export const sanctumStakePoolLadder = makeSanctumStakePoolLadder('sanctum-stake-pool', SANCTUM_STAKE_POOL_PROGRAM_ID);
export const sanctumStakePool2Ladder = makeSanctumStakePoolLadder('sanctum-stake-pool-2', SANCTUM_STAKE_POOL_2_PROGRAM_ID);
export const sanctumStakePool3Ladder = makeSanctumStakePoolLadder('sanctum-stake-pool-3', SANCTUM_STAKE_POOL_3_PROGRAM_ID);
export const sanctumStakePool4Ladder = makeSanctumStakePoolLadder('sanctum-stake-pool-4', SANCTUM_STAKE_POOL_4_PROGRAM_ID);
