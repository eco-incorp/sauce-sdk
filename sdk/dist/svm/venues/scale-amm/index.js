...
export const scaleAmmLadder = {
    ...
    emitSetup(base, slot, params) {
        ...
        return [
            `  const s${slot}rA = accountUint(${pool}, ${OFFSETS.reservesA}, 16);`,
            `  const s${slot}rB = accountUint(${pool}, ${OFFSETS.reservesB}, 16);`,
            `  const s${slot}shift = accountUint(${pool}, ${OFFSETS.shift}, 16);`,
            `  const s${slot}pbps = accountUint(${config}, ${CONFIG_OFFSETS.platformFeeBps}, 2);`,
            ...cfg.feeBeneficiaries.map((_, i) => `  const s${slot}s${i} = accountUint(${pool}, ${OFFSETS.feeBeneficiaries + i * 34 + 32}, 2);`),
    ...
    buildSwapV2(base, slot, user) {
        ...
        const accounts = [
            fixed('pool', cfg.pool, true),
            { ref: user.owner, signer: true, writable: true },
            fixed('owner', cfg.owner),
    ...
    referenceQuote(base, state, params) {
        ...
        const curveState = {
            reservesA: readUintLE(pool, OFFSETS.reservesA, 16),
            reservesB: readUintLE(pool, OFFSETS.reservesB, 16),
            shift: readUintLE(pool, OFFSETS.shift, 16),
            platformFeeBps: readUintLE(config, CONFIG_OFFSETS.platformFeeBps, 2),
            shareBps: cfg.feeBeneficiaries.map((_, i) => readUintLE(pool, OFFSETS.feeBeneficiaries + i * 34 + 32, 2)),
        };
