...
export function readUintLE(data, offset, width) {
...
export function pubkeyAt(data, offset) {
...
export function readFeeBeneficiaries(data, offset) {
    const out = [];
...
export async function ata(owner, mint, tokenProgram) {
    const encoder = getAddressEncoder();
 ...
 */
export function detectTokenProgram(mint, data) {
    if (data.length === 82)
...
function feeTotal(amount, state) {
    let fee = (amount * state.platformFeeBps) / 10000n;
    for (const bps of state.shareBps)
        fee += (amount * bps) / 10000n;
    return fee;
}
/** TS mirror of the `qScaleCurve` SauceScript helper — see module doc for the derivation. */
export function computeScaleQuote(state, amountIn, direction) {
    if (amountIn === 0n)
        return 0n;
...
export function scaleDepthReserves(state, direction) {
    const vA = state.reservesA + state.shift;
    const vB = state.reservesB;
...
//# sourceMappingURL=scale-common.js.map