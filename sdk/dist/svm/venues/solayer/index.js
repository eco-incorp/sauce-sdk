...
export async function fetchSolayerPoolConfig(load, pool) {
    ...
    const authority = codec.decode(data.subarray(OFF_AUTHORITY, OFF_AUTHORITY + 32));
    const receiptMint = codec.decode(data.subarray(OFF_RECEIPT_MINT, OFF_RECEIPT_MINT + 32));
    const lstMint = codec.decode(data.subarray(OFF_LST_MINT, OFF_LST_MINT + 32));
    const vault = codec.decode(data.subarray(OFF_VAULT, OFF_VAULT + 32));
    ...
    if (receiptMintData === null)
        throw new Error(`${SLUG} pool ${pool} receipt mint ${receiptMint} not found`);
    if (lstMintData === null)
        throw new Error(`${SLUG} pool ${pool} lst mint ${lstMint} not found`);
    ...
    if (vaultData === null)
        throw new Error(`${SLUG} pool ${pool} vault ${vault} not found`);
