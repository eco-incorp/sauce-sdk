 ...
 */
function emitSplineInline(tag, knots, qExpr, outVar) {
    ...
    for (let k = 1; k <= 7; k++) {
        ...
        const dq = `${tag}dq${k}`;
        const dx = `${tag}dx${k}`;
        lines.push(`      if (${done} === 0 && ${xk} > ${qExpr}) {`, `        const ${dy} = (${yk} - ${yl}) & ${M};`, `        const ${dq} = (${qExpr} - ${xl}) & ${M};`, `        const ${dx} = (${xk} - ${xl}) & ${M};`, `        ${outVar} = ((((${dy} * ${dq}) & ${M}) + (${dx} >> 1)) / ${dx} + ${yl}) & ${M};`, `        ${done} = 1;`, `      }`, `      if (${done} === 0 && ${nm} === ${k}) { ${outVar} = ${yk}; ${done} = 1 }`, `      if (${done} === 0 && ${xk} >= ${qExpr}) { ${outVar} = ${yk}; ${done} = 1 }`);
    }
...
function emitSplineReads(p, tag, poolRef, base) {
    const xs = [];
    const ys = [];
    const lines = [];
    for (let i = 0; i < 8; i++) {
...
function readSplineRef(data, base) {
    const x = [];
    const y = [];
    for (let i = 0; i < 8; i++)
        x.push(readUintLE(data, base + SPLINE_X_STRIDE + 8 * i, 8));
    for (let i = 0; i < 8; i++)
        y.push(readUintLE(data, base + SPLINE_Y_STRIDE + 8 * i, 8));
...
function signExtend(raw) {
    if (raw >= H64)
        return { mag: MOD64 - raw, neg: true };
...
// ---------------------------------------------------------------------------
function emitSetupLines(cfg, slot, kParam, enableVar) {
    const p = `s${slot}`;
    ...
    const lines = [
        ...
        // underflow gate: skew < -1e7 => deactivate
...
function emitQuoteAt(cfg, slot, xExpr, tag, depthVars) {
    const p = `s${slot}`;
    const dir0 = cfg.direction === 0;
    const t = `${p}${tag}`;
...
export const solfiV2Ladder = {
    ...
    paramCount: 1,
    paramsFor(base) {
        return [solfiConfig(base).registryK];
    },
    quoteRefs(base, slot) {
    ...
    referenceQuote(base, state, params, now) {
        ...
        return (x) => (live === null ? 0n : solfiColdQuote(cfg, live, x));
    },
...
function liveState(cfg, state, k, nowSlot) {
    ...
    if (oracleRaw === undefined)
        throw new Error(`${SLUG} reference is missing oracle ${cfg.oracle}`);
    if (va === undefined || vb === undefined)
    ...
    if (slot > expSlot)
        return null; // stale oracle -> deactivate
    ...
    let convA = 0n;
    if (expNeg)
        convA = (vaultA * man) / pow;
    else
        convA = vaultA * man * pow;
    let diffMag = 0n;
    let diffNeg = false;
    if (vaultB >= convA)
        diffMag = vaultB - convA;
    else {
        diffNeg = true;
    ...
    if (!bigImb) {
        ...
        if (skewIsPos) {
            ...
            if (!dNeg && skewNeg)
                dGreater = true;
            else if (!dNeg && !skewNeg && dMag > skewMag)
                dGreater = true;
            else if (dNeg && skewNeg && dMag < skewMag)
                dGreater = true;
            if (dGreater) {
    ...
    if (!skewNeg && skewMag > hi.mag)
        skewMag = hi.mag;
    if (skewNeg && skewMag > loMag.mag)
        skewMag = loMag.mag;
    if (skewNeg && skewMag > 10000000n)
        return null; // underflow -> deactivate
    let adjNum = 10000000n;
    if (!skewNeg)
        adjNum = 10000000n + skewMag;
    else
        adjNum = 10000000n - skewMag;
    const adjMan = (man * adjNum) / 10000000n;
    let sAge = 0n;
    if (slot >= oSlot)
        sAge = slot - oSlot;
    ...
    if (sf > thr) {
        ...
        if (confClamped < 1000n)
            confClamped = 1000n;
        if (confClamped > 100000n)
            confClamped = 100000n;
        f2 = confClamped;
    }
...
function solfiRawQuote(cfg, live, x) {
    ...
    if (live.dir0) {
        ...
        if (live.expNeg)
            pre = (x * live.pow) / live.adjMan;
        else if (live.adjMan !== 0n)
            pre = x / (live.adjMan * live.pow);
    }
    const bAmt = live.dir0 ? pre : x;
    const depth = solfiSplineRef(live.depthKnots, bAmt);
    const term = (live.ag * depth * live.f2 * live.feeScale) / 10000000000000n;
    const impact = live.s4 + live.oFee + live.k + term;
    if (impact > 10000000n)
        return null;
    const rawOut = (pre * (10000000n - impact)) / 10000000n;
    if (rawOut > live.outCap110)
        return null;
