 ...
 */
function swapFee(x, numerator, denominator) {
...
function netAmountIn(slug, cfg, amountIn) {
    ...
    if (netIn <= 0n)
        throw new Error(`${slug} amountIn ${amountIn} is consumed entirely by fees`);
    return netIn;
}
/**
 ...
 */
function deriveSwapAuthority(programId, pool, bumpSeed) {
...
const poolRef = (slug, cfg, role) => `${slug}:${cfg.pool}:${role}`;
/**
 ...
 * each is waiting on.
 */
export function makeSplTokenSwapForkAdapter(slug, programId, unresolvedGate) {
    return {
        slug,
        ...
        async fetchPoolConfig(load, pool) {
            ...
            if (data[0] !== 1)
                throw new Error(`${slug} pool ${pool} version must be 1 (SwapV1), got ${data[0]}`);
            if (data[1] !== 1)
        ...
        referenceQuote(cfg, state, amountIn, _now) {
            ...
            for (const vault of [cfg.vaultIn, cfg.vaultOut]) {
                ...
                if (data === undefined)
                    throw new Error(`${slug} vault ${vault} missing from state`);
                vaults.push(data);
            }
...
export const TOKEN_SWAP_V1_PROGRAM_ID = 'SwaPpA9LAaLfeLi3a68M4DjnLqgtticKg6CnyNwgAC8';
 ...
 */
export const DEXLAB_PROGRAM_ID = 'DSwpgjMvXhtGn6BsbqmacdBZyfLj6jSWf3HJpdJtmg6N';
export const dexlab = makeSplTokenSwapForkAdapter('dexlab', DEXLAB_PROGRAM_ID, 'Swap instruction needs >11 accounts on the real deployed binary (confirmed via real-CPI probe); exact extra account(s) not yet identified');
/**
 ...
 */
export const SAROS_PROGRAM_ID = 'SSwapUtytfBdBn1b9NUGG6foMVPtcWgpRU32HToDUZr';
export const saros = makeSplTokenSwapForkAdapter('saros', SAROS_PROGRAM_ID, "realized fee measured at a reproducible 4 bps across 5 sizes, diverging from the pool's own on-chain trade_fee/owner_trade_fee fields (30 bps read); real fee model not yet identified");
/**
 ...
 */
export const ORCA_V1_PROGRAM_ID = 'DjVE6JNiYqPL2QXyCUUh8rNjHrbz9hXHNYt99MQ59qw1';
export const orcaV1 = makeSplTokenSwapForkAdapter('orca-v1', ORCA_V1_PROGRAM_ID);
/** Penguin. */
export const PENGUIN_PROGRAM_ID = 'PSwapMdSai8tjrEXcxFeQth87xC4rRsa4VA5mhGhXkP';
export const penguin = makeSplTokenSwapForkAdapter('penguin', PENGUIN_PROGRAM_ID);
/** StepN (Dooar). */
export const STEPN_PROGRAM_ID = 'Dooar9JkhdZ7J3LHN3A7YCuoGRUggXhQaG4kijfLGU2j';
