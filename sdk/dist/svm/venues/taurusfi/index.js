...
const SLUG = 'taurusfi';
export const TAURUSFI_PROGRAM_ID = address('9VX8EKBg6vM6tA68xaDsPkbrx26XConZjkQmhVApUptc');
...
export const TAURUSFI_PRICE_SCALE = 1000000000n;
 ...
 */
export function decodeTaurusFiPriceScaled(bits) {
 ...
 */
export const TAURUSFI_POOL_REGISTRY = {
 ...
 */
export async function fetchTaurusFiConfig(load, pool) {
    const entry = TAURUSFI_POOL_REGISTRY[pool];
    if (entry === undefined) {
        throw new Error(`${SLUG}: pool ${pool} is not in TAURUSFI_POOL_REGISTRY — this venue has no plaintext pool-identity ` +
    ...
    if (priceData === null)
        throw new Error(`${SLUG}: price account ${entry.priceAccount} not found`);
...
export const taurusFiLadder = {
    slug: SLUG,
    /** Simple CP-style curve (no window walk / Newton iteration), 4 rungs — same shape as bisonfi. */
    defaultRungs: 4,
    shapeKey(base) {
        return `${SLUG}:${taurusFiConfig(base).direction}`;
    },
    helpers() {
        return [
            {
                name: 'qTaurusFi',
                source: ['function qTaurusFi(x, rin, rout) {', '  if (x === 0) { return 0 }', '  return Math.mulDiv(x, rout, rin + x);', '}'].join('\n'),
            },
        ];
    },
    /** Everything the on-chain formula needs is either a live account read or a codegen-time-baked
     *  literal (the decimal-scale ratio, static per registry entry) — no runtime params. */
    paramCount: 0,
    paramsFor() {
        return [];
    },
    quoteRefs(base, slot) {
        const cfg = taurusFiConfig(base);
        const forward = cfg.direction === '0to1';
        return [
            { ref: ref(slot, 'price'), address: cfg.priceAccount },
            { ref: ref(slot, forward ? 'vault1' : 'vault0'), address: forward ? cfg.vault1 : cfg.vault0 },
        ];
    },
    emitSetup(base, slot) {
        const cfg = taurusFiConfig(base);
        ...
        const voutRef = JSON.stringify(ref(slot, forward ? 'vault1' : 'vault0'));
        const { num: scaleNum, den: scaleDen } = decimalScale(cfg);
        // rin = rout / (rate * NUM/DEN) = rout * (rate's denominator * DEN) / (rate's numerator * NUM) —
        // rin is INFLATED by DEN/NUM (see file header "Quote curve": discounting rout instead would
        // cancel out of the small-x marginal rate entirely). rate (quote-per-base raw, direction '0to1')
        // = priceScaled*scaleNum / (TAURUSFI_PRICE_SCALE*scaleDen); direction '1to0' uses the reciprocal
        // rate. Both forms are exactly ONE mulDiv (full-precision floor), LOCKSTEP with `referenceQuote`.
        const rinNum = forward
            ? `${TAURUSFI_PRICE_SCALE} * ${scaleDen} * ${OUT_DISCOUNT_DEN}`
            : `s${slot}price * ${scaleNum} * ${OUT_DISCOUNT_DEN}`;
        const rinDen = forward
            ? `s${slot}price * ${scaleNum} * ${OUT_DISCOUNT_NUM}`
            : `${TAURUSFI_PRICE_SCALE} * ${scaleDen} * ${OUT_DISCOUNT_NUM}`;
        return [
            `  const s${slot}bits = accountUint(${priceRef}, 0, 8);`,
            `  const s${slot}exp = (s${slot}bits >> 52) & ${F64_EXP_MASK};`,
            `  const s${slot}mant = (s${slot}bits & ${F64_MANTISSA_MASK}) | ${F64_IMPLICIT_BIT};`,
            `  const s${slot}price = Math.mulDiv(s${slot}mant, ${TAURUSFI_PRICE_SCALE}, 1 << (${F64_BIAS_PLUS_MANTISSA_BITS} - s${slot}exp));`,
            // rout is the REAL, undiscounted live vault balance — the honest capacity ceiling (see file
            // header); the conservative haircut lives entirely in rin's inflation above.
            `  const s${slot}rout = accountUint(${voutRef}, ${VAULT_AMOUNT_OFFSET}, 8);`,
            `  const s${slot}rin = Math.mulDiv(s${slot}rout, ${rinNum}, ${rinDen});`,
        ].join('\n');
    },
    emitQuoteCall(_base, slot, x) {
        return `qTaurusFi(${x}, s${slot}rin, s${slot}rout)`;
    },
    buildSwapV2(base, slot, user) {
        const cfg = taurusFiConfig(base);
        const forward = cfg.direction === '0to1';
        const [mintIn, mintOut] = forward ? [cfg.mint0, cfg.mint1] : [cfg.mint1, cfg.mint0];
        const userVault0Ata = forward ? user.inAta : user.outAta;
        const userVault1Ata = forward ? user.outAta : user.inAta;
        ...
        return {
            ...
            suffix: Uint8Array.from(TAIL_BYTES),
            patch: 'in',
            accounts: [
                roled('vault0', cfg.vault0, true),
                roled('vault1', cfg.vault1, true),
                { ref: user.owner, signer: true, writable: true },
                { ref: userVault0Ata, writable: true },
                { ref: userVault1Ata, writable: true },
                roled('vaultAuthority', cfg.vaultAuthority, true),
                roled('config', cfg.globalConfig),
                roled('price', cfg.priceAccount),
                roled('authority', cfg.globalAuthority),
                roled('tp1', TOKEN_PROGRAM),
                roled('tp2', TOKEN_PROGRAM),
                roled('mintIn', mintIn),
                roled('mintOut', mintOut),
                roled('sysvarIx', SYSVAR_INSTRUCTIONS),
            ],
        };
    },
    referenceQuote(base, state) {
        const cfg = taurusFiConfig(base);
        const forward = cfg.direction === '0to1';
        const priceData = state[cfg.priceAccount];
        if (priceData === undefined)
            throw new Error(`${SLUG} reference is missing price account ${cfg.priceAccount}`);
        const voutAddr = forward ? cfg.vault1 : cfg.vault0;
        const voutData = state[voutAddr];
        if (voutData === undefined)
            throw new Error(`${SLUG} reference is missing vault ${voutAddr}`);
        const priceScaled = decodeTaurusFiPriceScaled(readUintLE(priceData, 0, 8));
        // rout is the REAL, undiscounted live vault balance (the honest capacity ceiling); the
        // conservative haircut lives entirely in rin's inflation below — LOCKSTEP with emitSetup.
        const rout = readUintLE(voutData, VAULT_AMOUNT_OFFSET, 8);
        const { num: scaleNum, den: scaleDen } = decimalScale(cfg);
        const rin = forward
            ? (rout * (TAURUSFI_PRICE_SCALE * scaleDen * OUT_DISCOUNT_DEN)) / (priceScaled * scaleNum * OUT_DISCOUNT_NUM)
            : (rout * (priceScaled * scaleNum * OUT_DISCOUNT_DEN)) / (TAURUSFI_PRICE_SCALE * scaleDen * OUT_DISCOUNT_NUM);
        return (x) => {
            if (x === 0n)
                return 0n;
            return (x * rout) / (rin + x);
    ...
    depthReserves(base, state) {
        const cfg = taurusFiConfig(base);
        const v0Data = state[cfg.vault0];
        const v1Data = state[cfg.vault1];
        if (v0Data === undefined || v1Data === undefined)
            throw new Error(`${SLUG} depth is missing a vault`);
        // Real (undiscounted) vault balances — the true liquidity depth for the relative-depth filter;
        // the OUT_DISCOUNT haircut above is a QUOTE-only safety margin, not a claim about real depth.
        const r0 = readUintLE(v0Data, VAULT_AMOUNT_OFFSET, 8);
        const r1 = readUintLE(v1Data, VAULT_AMOUNT_OFFSET, 8);
    ...
    continuousFees() {
        ...
        // gammaPpm carries the whole OUT_DISCOUNT_NUM/DEN haircut and muPpm stays at par.
        return { gammaPpm: (1000000n * OUT_DISCOUNT_NUM) / OUT_DISCOUNT_DEN, muPpm: 1000000n };
    },
};
