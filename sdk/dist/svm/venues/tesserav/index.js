...
export const MID_SCALE = 1000000000000000n; // 1e15
...
export const PRICE_PPM_DEN = 1000000n;
...
export function level0PriceOffset(direction) {
...
export function level0CumOffset(direction) {
 ...
 */
export async function fetchTesseraVConfig(load, pool, direction) {
    if (direction === 'bToA') {
    ...
    if (mid === 0n)
        throw new Error(`${SLUG}: pool ${pool} has an unprimed mid price (never quoted by the maker)`);
    ...
    return {
        ...
        auth: pool === address('FLckHLGMJy5gEoXWwcE68Nprde1D4araK4TGLw4pQq2n')
            ...
            : (() => {
                throw new Error(`${SLUG}: pool ${pool} has no known auth/vault mapping yet — only the SOL/USDC pool ` +
