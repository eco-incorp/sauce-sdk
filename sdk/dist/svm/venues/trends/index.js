...
function coldQuote(sell, live, icap, x) {
...
export const trendsLadder = {
    ...
    emitSetup(base, slot) {
        ...
        if (sell) {
            lines.push(`  const ${p}qoff = ${p}vqr - ${p}qr;`, `  if (${p}qoff > 0) { ${p}icap = (${p}qr * ${p}vbr) / ${p}qoff }`);
        }
        else {
            lines.push(`  const ${p}boff = ${p}vbr - ${p}br;`, `  if (${p}boff > 0) {`, `    const ${p}icapeff = (${p}br * ${p}vqr) / ${p}boff;`, `    ${p}icap = ${p}icapeff + ${p}icapeff / ${FEE_INVERSE_STEP};`, `  }`);
        }
        return lines.join('\n');
    },
    ...
    emitLadderQuote(base, slot, _rung, x, outVar) {
        ...
        if (sell) {
            lines.push(`    if (${p}cx > 0) {`, `      const ${p}gr = (${p}vqr * ${p}cx) / (${p}vbr + ${p}cx);`, `      const ${p}fe = (${p}gr * ${FEE_BPS_TOTAL}) / ${FEE_DEN};`, `      ${outVar} = ${p}gr - ${p}fe;`, `    }`);
        }
        else {
            lines.push(`    if (${p}cx > 0) {`, `      const ${p}fe = (${p}cx * ${FEE_BPS_TOTAL}) / ${FEE_DEN};`, `      const ${p}ei = ${p}cx - ${p}fe;`, `      if (${p}ei > 0) { ${outVar} = (${p}vbr * ${p}ei) / (${p}vqr + ${p}ei) }`, `    }`);
        }
        return lines.join('\n');
    },
    ...
    emitFinalQuote(base, slot, x, outVar) {
        ...
        if (sell) {
            lines.push(`  if (${p}fcx > 0) {`, `    const ${p}fgr = (${p}vqr * ${p}fcx) / (${p}vbr + ${p}fcx);`, `    const ${p}ffe = (${p}fgr * ${FEE_BPS_TOTAL}) / ${FEE_DEN};`, `    ${outVar} = ${p}fgr - ${p}ffe;`, `  }`);
        }
        else {
            lines.push(`  if (${p}fcx > 0) {`, `    const ${p}ffe = (${p}fcx * ${FEE_BPS_TOTAL}) / ${FEE_DEN};`, `    const ${p}fei = ${p}fcx - ${p}ffe;`, `    if (${p}fei > 0) { ${outVar} = (${p}vbr * ${p}fei) / (${p}vqr + ${p}fei) }`, `  }`);
        }
        return lines.join('\n');
    },
    ...
    continuousFees(base) {
        ...
        const netPpm = 1000000n - FEE_BPS_TOTAL * 100n; // 980_000
        return sell ? { gammaPpm: 1000000n, muPpm: netPpm } : { gammaPpm: netPpm, muPpm: 1000000n };
    },
};
