...
/**
 * A ladder swap CPI template (adapter contract v2, the SvmRoute recipe):
...
/**
 * Adapter contract v2 — amount-PARAMETRIC fragments for the SvmRoute
 ...
 */
export interface SvmVenueLadder {
    slug: string;
    /**
     ...
     * (and quoteSvmRoute) evaluates it lamport-identically to the engine.
     ...
     */
    referenceLadderQuotes?(cfg: PoolConfig, state: AccountBytesMap, params: readonly bigint[], now?: bigint): (grid: readonly bigint[]) => bigint[];
    /**
     ...
     * capacityInputVar is.
     */
    referenceCapacities?(cfg: PoolConfig, state: AccountBytesMap, params: readonly bigint[], now?: bigint): (grid: readonly bigint[]) => bigint[];
    /**
     * Effective (reserveIn, reserveOut) from state — relative-depth filter +
     * continuous-oracle input. `now` as in referenceQuote.
     */
    depthReserves(cfg: PoolConfig, state: AccountBytesMap, now?: bigint): {
        reserveIn: bigint;
        reserveOut: bigint;
    };
    /**
     ...
     */
    continuousFees(cfg: PoolConfig, state: AccountBytesMap, params: readonly bigint[]): {
        gammaPpm: bigint;
        muPpm: bigint;
    };
}
