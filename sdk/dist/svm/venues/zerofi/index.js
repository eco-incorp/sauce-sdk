 ...
 * neighbor's fee.
 */
export const ZEROFI_POOL_FEE_PPM = {
    ...
    Et6HnPjetV8AzmxNAfzKJ6ax5VM82ZB7phY465Ns5iZW: 1300n,
    ...
    // 4500ppm for margin.
    '5Dp922oSaj9rdLHTxZzq1B7QV7KmwY7U7K1JNCRhc45J': 4500n,
};
/**
 ...
 */
export const ZEROFI_POOL_AUTHORITY = {};
/**
...
export const zerofi = {
    ...
    async fetchPoolConfig(load, pool, direction = 0) {
        ...
        if (feePpm === undefined) {
            throw new Error(`${SLUG} pool ${pool} has no independently-verified fee (resolved per POOL, never guessed — see ZEROFI_POOL_FEE_PPM)`);
        }
        const authorityCfg = ZEROFI_POOL_AUTHORITY[pool];
        if (authorityCfg === undefined) {
            throw new Error(`${SLUG} pool ${pool} has no registered authority configured (ZEROFI_POOL_AUTHORITY) — the live program ` +
        ...
        if (oracleData === null)
            throw new Error(`${SLUG} pool ${pool} oracle ${oracle} account not found`);
        if (oracleData.length !== ORACLE_ACCOUNT_SIZE) {
            throw new Error(`${SLUG} pool ${pool} oracle ${oracle} is ${oracleData.length} bytes, expected ${ORACLE_ACCOUNT_SIZE}`);
        }
        ...
        if (scale.num > U64_MAX || scale.den > U64_MAX) {
            throw new Error(`${SLUG} pool ${pool} price scale (num=${scale.num}, den=${scale.den}) exceeds u64 after reduction — refusing rather than emitting an overflow-prone fragment`);
        }
        return {
            venue: SLUG,
            pool,
 ...
 */
export function zerofiSwapAccounts(c, make, refFor) {
