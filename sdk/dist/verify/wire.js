 ...
 */
export function scanMinimalPush(bytes, pos, maxWidthBytes = SETTLE_WIRE.PUSH_MAX) {
    const op = bytes[pos];
...
export function encodeMinimalPush(value, maxWidthBytes = SETTLE_WIRE.PUSH_MAX) {
    if (value < 0n)
        throw new RangeError(`encodeMinimalPush: value ${value} is negative`);
    let hex = value.toString(16);
    if (hex.length % 2 === 1)
        hex = "0" + hex;
    let bytes = hexToBytesLocal(hex);
    if (bytes.length === 0)
        bytes = new Uint8Array([0x00]); // value === 0 → L=1, data=0x00
