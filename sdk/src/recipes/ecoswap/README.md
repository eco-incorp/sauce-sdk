...
its funded members through ONE unified dispatch (leg pools + all 21 quote-ladder families), with a per-route
...
```
   ...
   routes: chain-order per leg — read the REALIZED leg input balance, split it
           ...
           venues, ONE unified 21-family dispatch; last funded member absorbs
...
- **Multi-hop routes are full EcoSwaps per leg.** An N-hop route is a LIVE composite venue: each leg
  ...
  pools) and the 21 quote-ladder families (leg-venue ladders built on-chain in setup, sized by the
