...
import type { PoolConfig, SwapUser } from '../../../svm/venues/types.js';
import {
  accountEntry,
  bindAddress,
  collectHelpers,
...
/** A flat view of a route slot with its leg-specific swap user. */
interface FlatSlot {
  slot: EcoSwapSvmSlot;
  /** buildSwapV2 receives this (leg-0: {A → inter}, leg-1: {inter → B}). */
  legUser: SwapUser;
  leg: 0 | 1;
}
...
function emitLegBlock(params: {
  ...
  rungs: number[];
  rungBase: number[];
  /** SauceScript expression for the leg's total input (`amountIn` | `realizedX`). */
  inputExpr: string;
  /** The ref whose accountUint before/after delta IS this leg's realized output. */
  deltaRef: string;
  /** const name to bind for that delta (`realizedX` | `realizedB`). */
  deltaVar: string;
  /** Emit the pre-CPI predicted-vs-minOut throw (leg-1 only — the route terminal). */
  checkMinOut: boolean;
  codec: ReturnType<typeof getAddressCodec>;
}): string[] {
  ...
  flats.forEach(({ slot }, i) => {
    ...
    for (let j = 1; j <= r; j++) {
      ...
      if (j < r) lines.push(`    const ${g} = ${inputExpr} >> ${r - j};`);
      if (mode === 'expression') {
  ...
  lines.push(
    `  let rem${L} = ${inputExpr};`,
    `  for (let it${L} = 0; it${L} < ${legRungs} && rem${L} > 0; it${L}++) {`,
    `    let best${L} = ${flatEnd};`,
    `    for (let sc${L} = ${flatStart}; sc${L} < ${flatEnd}; sc${L}++) {`,
    `      if (ptr[sc${L}] < rl[sc${L}]) {`,
    `        if (best${L} === ${flatEnd}) { best${L} = sc${L} }`,
    `        if (best${L} !== sc${L}) {`,
    `          const cc${L} = rb[sc${L}] + ptr[sc${L}];`,
    `          const bb${L} = rb[best${L}] + ptr[best${L}];`,
    `          if (dout[cc${L}] * din[bb${L}] > dout[bb${L}] * din[cc${L}]) { best${L} = sc${L} }`,
    '        }',
    '      }',
    '    }',
    `    if (best${L} === ${flatEnd}) { throw "fill" }`,
    `    const rr${L} = rb[best${L}] + ptr[best${L}];`,
    `    let tk${L} = din[rr${L}];`,
    `    if (tk${L} > rem${L}) { tk${L} = rem${L} }`,
    `    fill[best${L}] += tk${L};`,
    `    rem${L} -= tk${L};`,
    `    if (tk${L} === din[rr${L}]) { ptr[best${L}] = ptr[best${L}] + 1 }`,
    '  }',
    `  if (rem${L} > 0) { throw "fill" }`,
  );
  ...
  flats.forEach(({ slot, legUser }, i) => {
    ...
    lines.push(
      `  if (fill[${flat}] > 0 && p${flat} > 0) {`,
      `    const s${flat}pfx = Uint8Array.from([${Array.from(template.prefix).join(', ')}]);`,
      ...(template.suffix.length > 0
        ? [`    const s${flat}sfx = Uint8Array.from([${Array.from(template.suffix).join(', ')}]);`]
        : []),
      `    const s${flat}amt = abi.encode(le8(${patched}));`,
      `    const s${flat}cd = ${parts[0]}.concat(${parts.slice(1).join(', ')});`,
      `    contract.call(${target}, s${flat}cd, [${accounts}]);`,
      '  }',
    );
  });
...
function generateRouteSource(input: GenerateEcoSwapSvmRouteInput): {
  source: string;
  ...
  const user0: SwapUser = { inAta: user.inAta, outAta: interRef, owner: user.owner };
  const user1: SwapUser = { inAta: interRef, outAta: user.outAta, owner: user.owner };
  const flats: FlatSlot[] = [
    ...input.leg0.map((slot): FlatSlot => ({ slot, legUser: user0, leg: 0 })),
    ...input.leg1.map((slot): FlatSlot => ({ slot, legUser: user1, leg: 1 })),
  ];
  ...
  const lines: string[] = [LE8_HELPER, ...collectHelpers(flats.map((f) => f.slot))];
 ...
 */
export function ecoSwapSvmRouteShapeKey(leg0: readonly EcoSwapSvmSlot[], leg1: readonly EcoSwapSvmSlot[]): string {
  const key = (slots: readonly EcoSwapSvmSlot[]): string =>
    slots
      .map((slot) => {
        const { adapter, cfg, swapOverride } = slot;
        const rungs = resolveSlotRungs(slot);
        let base = adapter.shapeKey(cfg);
        if (rungs !== 4) base += `~r${rungs}`;
...
export function generateEcoSwapSvmRoute(input: GenerateEcoSwapSvmRouteInput): GeneratedEcoSwapSvmRoute {
  ...
  for (const [key, value] of [
    ...
    if (value.length === 0) throw new Error(`ecoSwapSvm route ${key} ref must not be empty`);
  }
  ...
  if (!accountPlan) throw new Error('svm route compile produced no account plan');
  if (!argsLayout) throw new Error('staged svm route compile produced no args layout');
  ...
  flats.forEach(({ slot, legUser }, i) => {
    for (const account of slot.adapter.quoteRefs(slot.cfg, i)) bindAddress(addressByRef, account.ref, account.address);
  ...
  const metas = (accountPlan as AccountPlan).metas.map((meta) => {
  ...
  return {
    source,
    bytecode: (bytecode as Uint8Array[])[0],
    argsLayout: argsLayout as ArgsLayout,
    accountPlan: { ...(accountPlan as AccountPlan), metas },
