...
import type { MegaSwapConfig, MegaSwapResult, PreparedPool } from "../shared/types.js";
 ...
 */
export async function prepareMegaSwap(
  config: MegaSwapConfig,
  client: PublicClient,
  sauceRouterAddress: Hex,
): Promise<MegaSwapResult> {
  ...
  const quotedPools: PreparedPool[] = [];
  for (const pool of pools) {
    const quote = await quotePool(
      pool,
      amountIn,
      maxPriceLimit,
      sauceRouterAddress,
  ...
  const minDelta = quotedPools.reduce(
  ...
  const deepestPool = filteredPools.reduce((best, p) =>
    p.delta < best.delta ? p : best,
  );
  ...
  const expectedOutput = filteredPools.reduce(
    (sum, p) => sum + p.quote.amountOut,
    0n,
  );
