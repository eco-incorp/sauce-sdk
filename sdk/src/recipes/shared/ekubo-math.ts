...
export const EKUBO_SLOAD_SELECTOR = "0x380eb4e0" as Hex;

/** One frozen (fee, tickSpacing) preset — a CONCENTRATED extension-0 pool config candidate. */
export interface EkuboPreset {
  /** The fee as a uint64 0.64-fixed fraction — round(pct × 2^64) (live-confirmed per tier). */
  fee: bigint;
  /** The concentrated tickSpacing (micro-ticks, base 1.000001; bits 30..0 of poolTypeConfig). */
  tickSpacing: number;
}

/** fee u64 = round(num/den × 2^64) — the preset fee-word formula (confirmed vs every live tier). */
const feeWord = (num: bigint, den: bigint): bigint => (num * (1n << 64n) + den / 2n) / den;
 ...
 */
export const EKUBO_DEFAULT_PRESETS: EkuboPreset[] = [
  { fee: feeWord(3n, 100_000n), tickSpacing: 100 }, // 0.003%/100  — USDe/USDC + USDe/USDT (the top pool's tier)
  { fee: feeWord(1n, 10_000n), tickSpacing: 100 }, //  0.01%/100  — USDe/sUSDe
  { fee: feeWord(1n, 10_000n), tickSpacing: 200 }, //  0.01%/200  — ETH/wstETH
  { fee: feeWord(5n, 10_000n), tickSpacing: 1000 }, // 0.05%/1000 — ETH/WBTC side tier
  { fee: feeWord(5n, 10_000n), tickSpacing: 4988 }, // 0.05%/4988 — ETH/USDC + ETH/USDT (the volatile workhorse)
  { fee: feeWord(1n, 1_000n), tickSpacing: 1000 }, //  0.1%/1000  — ETH/WBTC
  { fee: feeWord(3n, 1_000n), tickSpacing: 4988 }, //  0.3%/4988  — ETH/USDC side tier
  { fee: feeWord(5n, 1_000n), tickSpacing: 4988 }, //  0.5%/4988  — ETH/USDC side tier
  { fee: feeWord(5n, 1_000n), tickSpacing: 100 }, //   0.5%/100   — USDC/USDT (initialized; a zero-L husk at the review re-probe — kept, dead candidates drop at the quote probe)
  ...
  { fee: feeWord(5n, 1_000_000n), tickSpacing: 50 }, // 0.0005%/50 — USDT/USDC (the venue's TOP stable-stable tier: id 0x6fde3244…895d, ~$5.6M/24h, initialized + live L; review re-probe 2026-07-04)
];
 ...
 */
export function ekuboConcentratedConfig(fee: bigint, tickSpacing: number): Hex {
  const word = (fee << 32n) | (1n << 31n) | BigInt(tickSpacing);
  return ("0x" + word.toString(16).padStart(64, "0")) as Hex;
}
 ...
 */
export function ekuboPoolId(token0: Hex, token1: Hex, config: Hex): Hex {
  return keccak256(
    encodeAbiParameters(
...
export function ekuboFeePpm(fee: bigint): number {
  return Number((fee * 1_000_000n + (1n << 63n)) / (1n << 64n));
}
 ...
 */
export function decodeEkuboBalanceUpdate(word: bigint): { delta0: bigint; delta1: bigint } {
  const HALF = 1n << 127n;
  const LANE = 1n << 128n;
  const hi = word >> 128n;
...
/**
 * One Ekubo venue DESCRIPTOR (the router + the full virtual-pool key, oriented for the swap). This
 * is ALL prepare ships (the QL family contract: descriptor-only, zero sampled values) — the
 * on-chain solver builds the ladder LIVE from Router.quote at cook, and the exec re-derives the
 * direction from the key tokens (derive-don't-trust; the stamp below serves the oracle/diagnostics).
 */
export interface EkuboVenue {
  /** MEVCaptureRouter — the quote/swap/approve target (qlv qd[0]; rides msVen to the exec). */
  router: Hex;
  /** PoolKey token0 (ascending sort; native ETH pools — token0 == 0 — are out of scope, qd[6]). */
  token0: Hex;
  /** PoolKey token1 (qd[7]). */
  token1: Hex;
  /** The packed pool config word (extension | fee u64 | poolTypeConfig; bytes32, qd[8]). */
  config: Hex;
  /** Direction: true ⇔ tokenIn == token1 (the quote/swap isToken1 bit; qd[1], oracle/diagnostic). */
  isToken1: boolean;
  /** keccak256 of the key — the pool's identity inside Core AND the CLAIM key (qd[9]). */
  poolId: Hex;
  /** The venue's tokenIn (the edge from-token; diagnostics — the exec derives from token0/token1). */
  tokenIn: Hex;
  /** The venue's tokenOut (diagnostics). */
  tokenOut: Hex;
  /** Diagnostic rounded ppm of the preset fee word (the quote is post-fee). */
  feePpm: number;
...
/**
 * The ORACLE/REFERENCE model of one Ekubo venue: the descriptor plus a `getDy` QUOTE model — the
 * cumulative |out| (the absolute value of the negative out-side delta) that
 * `Router.quote(key, isToken1, +dx, 0, 0)` returns for total input `dx`. Ekubo's closed form is the
 * bespoke micro-tick/compact-float math this family deliberately does NOT port, so the model is
 * caller-supplied:
 *   - the local EVM tests replay the EkuboCoreFixture curve bit-for-bit, and
 *   - the prod-mirror prefetches the REAL etched router's quotes at the DETERMINISTIC QL grid
 *     (`ekuboQLGridInputs`) and answers by exact-point lookup.
 * `getDy` must return 0 where the real quote reverts (PoolNotInitialized / a dead venue) — the
 * ladder then self-truncates in lockstep with the on-chain probe-then-decode build. An OVERSIZED dx
 * must return the PARTIAL-FILL |out| (the real quote's graceful liquidity-exhausted cap): the next
 * grid point quotes the same value, the differenced slice-out is 0, and both ladders stop
 * identically — at most one final slice carries capacity the pool cannot absorb, and the exec swaps
 * only the quoted CONSUMED input (the remainder rides the terminal refund).
 */
export interface EkuboPool extends EkuboVenue {
 ...
 */
export function buildEkuboQLLadder(pool: EkuboPool, amountIn: bigint): MergeSegment[] {
