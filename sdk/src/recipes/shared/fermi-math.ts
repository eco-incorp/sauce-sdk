 ...
 */
export function fermiSampleInputs(amountIn: bigint, samples: number = FERMI_SAMPLES): bigint[] {
 ...
 */
export function getAmountOut(pool: FermiPool, dx: bigint): bigint {
  if (dx <= 0n) return 0n;
...
export function buildFermiSegments(
  pool: FermiPool,
  amountIn: bigint,
  _samples: number = FERMI_SAMPLES,
): FermiSegment[] {
  if (amountIn <= 0n) return [];
  const n = pool.cumIn.length;
  if (n === 0) return [];
  const segs: FermiSegment[] = [];
  let prevIn = 0n;
  let prevOut = 0n;
  for (let i = 0; i < n; i++) {
