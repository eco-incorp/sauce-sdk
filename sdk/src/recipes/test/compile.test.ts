 ...
 *
 * Run: npx tsx --test src/recipes/test/compile.test.ts
 */
...
describe("recipe sauce scripts compile", () => {
  for (const [name, rel] of SCRIPTS) {
    it(`${name} (${rel}) compiles to bytecode`, () => {
      ...
      // SRC_ROOT (sdk/src) resolves "./artifacts/*.json"; recipeDir resolves recipe-local JSON imports.
      const result: any = compile(src, { baseDirs: [SRC_ROOT, RECIPES, recipeDir] });
