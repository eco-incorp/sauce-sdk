...
function curveArgs(
  ...
  //   pools   = [] (no direct V2/V3/V4 venue) ; netCache = [] ; routing = [] (no routes)
  //   segs    = the 6-col sampled-segment rows [refIdx, capacity, sqrtAdjNear, sqrtAdjFar, segKind,
  //             venue] the bestKind===1 cursor consumes (directCount=0, so the merge has only segs).
  return [
    [
      BigInt(tokenIn),
      BigInt(tokenOut),
