/**
 * GigaSwap fork test — deterministic, pinned to a specific Base block.
 *
 * Verifies:
 ...
 *  - Multiple pools were used (quote-based splitting)
 *
 * Env:
 *   BASE_RPC_URL   — Base archive/full-node RPC (required)
 *
 * Run:
 *   BASE_RPC_URL=<url> npx tsx src/recipes/test/gigaswap.test.ts
 */
...
import { gigaSwap } from "../gigaswap/index";
...
const HARDHAT_DIR = join(__dirname, "..", "..", "..", "..", "dev-tools");
...
async function main() {
  ...
  if (!forkUrl) {
    ...
    console.error(
      "Usage: BASE_RPC_URL=<url> npx tsx src/recipes/test/gigaswap.test.ts",
    );
    process.exit(1);
  }

  console.log("\nGigaSwap Fork Test (quote-based split + global price limit)");
  ...
  try {
    ...
    // Prepare + compile (off-chain: quote-based depth + split)
    console.log(`\nPreparing GigaSwap: ${swapAmount} WETH -> USDC...`);
    ...
    // Log prepared data
    console.log(
      `  V3 pools (price-limited): ${result.prepared.priceLimitedPools.length}`,
    );
    for (const dp of result.prepared.priceLimitedPools) {
      console.log(
        `    [${dp.pool.poolType}] ${dp.pool.source} ${dp.pool.address.slice(0, 10)}... fee=${dp.pool.fee}`,
      );
    }
    console.log(
      `  V2/other pools (no-limit): ${result.prepared.noLimitPools.length}`,
    );
    for (const dp of result.prepared.noLimitPools) {
      console.log(
        `    [${dp.pool.poolType}] ${dp.pool.source} ${dp.pool.address.slice(0, 10)}... split=${dp.splitAmount}`,
      );
    }
    console.log(
    ...
    for (const r of result.prepared.multiHopRoutes) {
      console.log(
        `    via ${r.route.intermediateToken.slice(0, 10)}... : ${r.route.hop1Pool.address.slice(0, 10)}...(fee:${r.route.hop1Pool.fee}) -> ${r.route.hop2Pool.address.slice(0, 10)}...(fee:${r.route.hop2Pool.fee}) split=${r.splitAmount}`,
      );
    }
    ...
    // Log pool type breakdown
    const poolTypeCounts = new Map<number, number>();
    for (const dp of [...result.prepared.priceLimitedPools, ...result.prepared.noLimitPools]) {
      poolTypeCounts.set(dp.pool.poolType, (poolTypeCounts.get(dp.pool.poolType) ?? 0) + 1);
    }
    console.log("\n  Pool type breakdown:");
    const poolTypeNames = ["UniV2", "UniV3", "UniV4", "Curve", "BalancerV2", "DODOV2", "TraderJoeLB", "MaverickV2", "WOOFi"];
    for (const [type, count] of poolTypeCounts) {
      console.log(`    ${poolTypeNames[type] ?? `Unknown(${type})`}: ${count}`);
    }
    ...
    console.log(`\n  Bytecodes: ${result.bytecodes.length} segments`);
    ...
    // V3 Swap events — with quote-based splitting, expect multiple pool swaps
    ...
    const totalTargets =
      result.prepared.priceLimitedPools.length +
      result.prepared.noLimitPools.length +
      result.prepared.multiHopRoutes.length;
    if (totalTargets > 1) {
      assert(
        swapLogs.length >= 2,
        `Multiple V3 Swap events (${swapLogs.length}) — quote-based splitting worked`,
      );
    }

    // ── Pool discovery assertions ─────────────────────────────
    // Verify that multi-protocol discovery found pools from multiple sources

    assert(
      result.prepared.priceLimitedPools.length > 0,
      `Found price-limited (V3) pools (${result.prepared.priceLimitedPools.length})`,
    );

    // Verify pool categorization is correct
    for (const dp of result.prepared.priceLimitedPools) {
      assert(
        dp.pool.priceLimited === true,
        `V3 pool ${dp.pool.source} correctly flagged as price-limited`,
      );
    }
    for (const dp of result.prepared.noLimitPools) {
      assert(
        dp.pool.priceLimited === false,
        `V2/other pool ${dp.pool.source} correctly flagged as no-limit`,
      );
    }

    // Verify source labels are populated
    const allPools = [...result.prepared.priceLimitedPools, ...result.prepared.noLimitPools];
    for (const dp of allPools) {
      assert(
        dp.pool.source.length > 0,
        `Pool ${dp.pool.address.slice(0, 10)} has source label: "${dp.pool.source}"`,
      );
    }

    // Check that we found multiple unique sources (verifies multi-protocol discovery)
    const uniqueSources = new Set(allPools.map((dp) => dp.pool.source));
    assert(
      uniqueSources.size >= 2,
      `Discovered pools from ${uniqueSources.size} unique sources (${[...uniqueSources].join(", ")})`,
    );

    // Verify global price limit is set (should be non-zero for V3 pools)
    if (result.prepared.priceLimitedPools.length > 0) {
      assert(
        result.prepared.globalPriceLimit > 0n,
        `Global price limit is set (${result.prepared.globalPriceLimit})`,
      );
    }

    // Verify no-limit pools have non-zero splits
    for (const dp of result.prepared.noLimitPools) {
      assert(
        dp.splitAmount > 0n,
        `V2 pool ${dp.pool.source} has non-zero split (${dp.splitAmount})`,
      );
    }
  } finally {
    stopNode(pid);
