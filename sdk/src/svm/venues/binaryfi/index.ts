...
function binaryfiConfig(cfg: PoolConfig): BinaryFiPoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as BinaryFiPoolConfig;
}
...
export const binaryfi = {
   ...
   */
  async fetchPoolConfig(load: AccountLoader, pool: Address): Promise<BinaryFiPoolConfig> {
    const data = await load(pool);
    if (data === null) throw new Error(`binaryfi market ${pool} does not exist`);
    ...
    if (floorRate === undefined) {
      throw new Error(
        `binaryfi market ${pool}: no calibrated floor rate for asset mint ${assetMint} — refresh sdk/src/svm/venues/binaryfi/asset-configs.ts`,
      );
    }
    ...
    for (const [role, vault] of [
      ...
      if (vaultBytes === null) throw new Error(`binaryfi market ${pool}: ${role}Vault ${vault} does not exist`);
      if (vaultBytes.length !== SPL_TOKEN_ACCOUNT_SIZE) {
        throw new Error(
          `binaryfi market ${pool}: ${role}Vault ${vault} has unexpected size ${vaultBytes.length} (want ${SPL_TOKEN_ACCOUNT_SIZE} — token-2022 vaults are not supported)`,
        );
      }
    }
...
export const binaryfiLadder = {
  slug: SLUG,

  shapeKey(base: PoolConfig): string {
    binaryfiConfig(base);
    return `${SLUG}:quoteToBase`;
  },
    ...
    return [
      {
        name: 'qBinaryfi',
        source: [
          'function qBinaryfi(x, floorNum, floorDen, reserveOut) {',
          '  if (x === 0) { return 0 }',
          '  const raw = Math.mulDiv(x, floorNum, floorDen);',
          '  const capped = raw < reserveOut ? raw : reserveOut;',
          '  return capped;',
          '}',
        ].join('\n'),
      },
  ...
  paramsFor(base: PoolConfig): bigint[] {
    const cfg = binaryfiConfig(base);
    return [cfg.floorRateNum, cfg.floorRateDen];
  },

  quoteRefs(base: PoolConfig, slot: number): VenueAccount[] {
    const cfg = binaryfiConfig(base);
    return [{ ref: ref(slot, 'reserveOut'), address: cfg.assetVault }];
  },

  emitSetup(base: PoolConfig, slot: number, params: readonly string[]): string {
    binaryfiConfig(base);
    const reserveOutRef = JSON.stringify(ref(slot, 'reserveOut'));
    return [
      `  const s${slot}reserveOut = accountUint(${reserveOutRef}, ${VAULT_AMOUNT_OFFSET}, 8);`,
      `  const s${slot}floorNum = ${params[0]};`,
      `  const s${slot}floorDen = ${params[1]};`,
    ].join('\n');
  },

  emitQuoteCall(_base: PoolConfig, slot: number, x: string): string {
    return `qBinaryfi(${x}, s${slot}floorNum, s${slot}floorDen, s${slot}reserveOut)`;
  },
  ...
  buildSwapV2(base: PoolConfig, slot: number, user: SwapUser): LadderSwapTemplate {
    ...
    return {
      programId: BINARYFI_PROGRAM_ID,
      prefix: Uint8Array.from([0x08]),
      suffix: Uint8Array.from([0, 0, 0, 0, 0, 0, 0, 0]),
      patch: 'in',
      accounts: [
        { ref: user.owner, signer: true, writable: true },
        { ref: ref(slot, 'config'), address: BINARYFI_CONFIG },
        { ref: ref(slot, 'market'), address: cfg.pool, writable: true },
        { ref: ref(slot, 'vaultAuthority'), address: cfg.vaultAuthority },
        { ref: ref(slot, 'assetMint'), address: cfg.assetMint },
        { ref: ref(slot, 'quoteMint'), address: cfg.quoteMint },
        { ref: ref(slot, 'assetVault'), address: cfg.assetVault, writable: true },
        { ref: ref(slot, 'quoteVault'), address: cfg.quoteVault, writable: true },
        { ref: user.inAta, writable: true },
        { ref: user.outAta, writable: true },
        { ref: ref(slot, 'tp1'), address: TOKEN_PROGRAM },
        { ref: ref(slot, 'tp2'), address: TOKEN_PROGRAM },
  ...
  referenceQuote(base: PoolConfig, state: AccountBytesMap, params: readonly bigint[]): (x: bigint) => bigint {
    const cfg = binaryfiConfig(base);
    const assetVaultBytes = state[cfg.assetVault];
    if (assetVaultBytes === undefined) throw new Error(`${SLUG} ladder reference is missing account ${cfg.assetVault}`);
    const reserveOut = readUintLE(assetVaultBytes, VAULT_AMOUNT_OFFSET, 8);
    const [floorNum, floorDen] = params;
    return (x: bigint): bigint => {
      if (x === 0n) return 0n;
      const raw = (x * floorNum!) / floorDen!;
      return raw < reserveOut ? raw : reserveOut;
    };
  },
  ...
  depthReserves(base: PoolConfig, state: AccountBytesMap): { reserveIn: bigint; reserveOut: bigint } {
    ...
    return {
      reserveIn: readUintLE(bytes(cfg.quoteVault), VAULT_AMOUNT_OFFSET, 8),
      reserveOut: readUintLE(bytes(cfg.assetVault), VAULT_AMOUNT_OFFSET, 8),
    };
  },

  continuousFees(): { gammaPpm: bigint; muPpm: bigint } {
    // Measurement-only oracle input (never a gate — see optimal.ts's own
    // header). This venue's real curve is a flat rate + hard cap, not a
    // fee-scaled constant-product form, so there is no representative
    // (gammaPpm, muPpm) pair to derive; report "no fee" (full retention),
    // the same "nothing separate to report" convention humidifi's own
    // continuousFees uses for the identical reason.
    return { gammaPpm: FEE_PPM_DENOM, muPpm: FEE_PPM_DENOM };
  },
};
