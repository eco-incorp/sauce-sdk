...
async function loadAccount(load: AccountLoader, addr: Address, what: string): Promise<Uint8Array> {
  ...
  if (data === null) throw new Error(`boop-fun ${what} ${addr} not found`);
  return data;
}
 ...
 */
function assertClassicMint(mint: Address, data: Uint8Array): void {
  if (data.length !== 82) {
    throw new Error(
      `boop-fun mint ${mint} is not a classic Tokenkeg mint (${data.length} bytes, expected 82) — token-2022 base mints are unsupported (no base-token-program CPI slot)`,
    );
  }
}
...
export const boopFun = {
  ...
  async fetchPoolConfig(load: AccountLoader, pool: Address): Promise<BoopFunPoolConfig> {
...
export const boopFunLadder: SvmVenueLadderV2 = {
  slug: SLUG,
  shapeKey(base) {
  ...
  helpers(base) {
    ...
    if (cfg.direction === 'quoteToBase') {
      // buy_token: fee off the top (floor), invariant swap on the net — see module header formula.
      return [
        {
          name: 'qBoopBuy',
          source: [
            'function qBoopBuy(x, vsol, rsol, rtok, feeBps) {',
            '  if (x === 0) { return 0 }',
            '  const fee = (x * feeBps) / 10000;',
            '  const net = x - fee;',
            '  return Math.mulDiv(rtok, net, vsol + rsol + net);',
            '}',
          ].join('\n'),
        },
      ];
    }
    // sell_token: invariant swap first, fee off the gross (floor) — see module header formula.
    return [
      {
        name: 'qBoopSell',
        source: [
          'function qBoopSell(x, vsol, rsol, rtok, feeBps) {',
          '  if (x === 0) { return 0 }',
          '  const gross = Math.mulDiv(vsol + rsol, x, rtok + x);',
  ...
  /** Two params: swapFeeBasisPoints, virtualSolReserves (a per-curve DEPLOY-TIME CONSTANT — never
   *  read live, so it rides as a param instead of an extra accountUint read; see module header). */
  paramCount: 2,
  paramsFor(base) {
    const cfg = asCfg(base);
    return [cfg.swapFeeBasisPoints, cfg.virtualSolReserves];
  },
  quoteRefs(base, slot) {
    const cfg = asCfg(base);
    // Both REAL reserves (sol_reserves@104, token_reserves@112) live in the SAME bonding-curve
    // account — one read, like the pump.fun bonding curve's own single-account setup.
    return [{ ref: ref(slot, 'bc'), address: cfg.pool }];
  },
  emitSetup(base, slot, params) {
    return [
      `  const s${slot}rsol = accountUint(${JSON.stringify(ref(slot, 'bc'))}, 104, 8);`,
      `  const s${slot}rtok = accountUint(${JSON.stringify(ref(slot, 'bc'))}, 112, 8);`,
      `  const s${slot}fee = ${params[0]};`,
      `  const s${slot}vsol = ${params[1]};`,
    ].join('\n');
  },
  emitQuoteCall(base, slot, x) {
    const cfg = asCfg(base);
    const helper = cfg.direction === 'quoteToBase' ? 'qBoopBuy' : 'qBoopSell';
    return `${helper}(${x}, s${slot}vsol, s${slot}rsol, s${slot}rtok, s${slot}fee)`;
  },
  buildSwapV2(base, slot, user: SwapUser): LadderSwapTemplate {
    ...
    if (!sell) {
      // buy_token: disc ++ buy_amount u64 (patched) ++ amount_out_min u64 = 1.
      const prefix = Uint8Array.from(BUY_TOKEN_DISCRIMINATOR);
      const suffix = Uint8Array.from([1, 0, 0, 0, 0, 0, 0, 0]);
      const accounts: VenueAccount[] = [
        roled('mint', cfg.mint),
        roled('bondingCurve', cfg.pool, true),
        roled('tradingFeesVault', cfg.tradingFeesVault, true),
        roled('bondingCurveVault', cfg.bondingCurveVault, true),
        roled('bondingCurveSolVault', cfg.bondingCurveSolVault, true),
        // recipient_token_account: the bought BASE tokens land here (a normal SPL contract).
        { ref: user.outAta, writable: true },
        // buyer: ALSO the source of the trade's native-SOL payment (system_program::transfer
        // nested inside this CPI reads the buyer's OWN lamport balance — user.inAta is unused on
        // the BUY side, see the module header "CONSEQUENCE FOR A RECIPE'S SwapUser CONTRACT").
        { ref: user.owner, writable: true, signer: true },
        roled('config', CONFIG),
        roled('vaultAuthority', VAULT_AUTHORITY),
        roled('wsol', WSOL),
        roled('systemProgram', SYSTEM_PROGRAM),
        roled('tokenProgram', TOKEN_PROGRAM),
        roled('associatedTokenProgram', ASSOCIATED_TOKEN_PROGRAM),
      ];
      return { programId: BOOP_FUN_PROGRAM_ID, prefix, suffix, patch: 'in', accounts };
    }
    // sell_token: disc ++ sell_amount u64 (patched) ++ amount_out_min u64 = 1.
    const prefix = Uint8Array.from(SELL_TOKEN_DISCRIMINATOR);
    const suffix = Uint8Array.from([1, 0, 0, 0, 0, 0, 0, 0]);
    const accounts: VenueAccount[] = [
      roled('mint', cfg.mint),
      roled('bondingCurve', cfg.pool, true),
      ...
      roled('bondingCurveVault', cfg.bondingCurveVault, true),
      roled('bondingCurveSolVault', cfg.bondingCurveSolVault, true),
      // seller_token_account: the sold BASE tokens leave from here (a normal SPL contract).
      { ref: user.inAta, writable: true },
      { ref: user.owner, writable: true, signer: true },
      // recipient: the trade's native-SOL proceeds land HERE, directly on the owner's wallet —
      // user.outAta is unused on the SELL side (same module-header consequence, mirrored).
  ...
  referenceQuote(base, state: AccountBytesMap, params: readonly bigint[]) {
    ...
    if (bytes === undefined) throw new Error(`${SLUG} ladder reference is missing account ${cfg.pool}`);
    const rSol = readUintLE(bytes, 104, 8);
    const rTok = readUintLE(bytes, 112, 8);
    const [feeBps, vsol] = params as [bigint, bigint];
    if (cfg.direction === 'quoteToBase') {
      return (x: bigint) => {
        if (x === 0n) return 0n;
        const fee = (x * feeBps) / BPS;
        const net = x - fee;
        return (rTok * net) / (vsol + rSol + net);
      };
    }
    return (x: bigint) => {
      if (x === 0n) return 0n;
      const gross = ((vsol + rSol) * x) / (rTok + x);
  ...
  depthReserves(base, state: AccountBytesMap) {
    const cfg = asCfg(base);
    const bytes = state[String(cfg.pool)];
    if (bytes === undefined) throw new Error(`${SLUG} ladder depth is missing account ${cfg.pool}`);
    const rSol = cfg.virtualSolReserves + readUintLE(bytes, 104, 8);
    const rTok = readUintLE(bytes, 112, 8);
    return cfg.direction === 'quoteToBase' ? { reserveIn: rSol, reserveOut: rTok } : { reserveIn: rTok, reserveOut: rSol };
  },
  continuousFees(base, _state, params) {
    const cfg = asCfg(base);
    const [feeBps] = params as [bigint, bigint];
    const gamma = ((BPS - feeBps) * 1_000_000n) / BPS;
    return cfg.direction === 'quoteToBase' ? { gammaPpm: gamma, muPpm: 1_000_000n } : { gammaPpm: 1_000_000n, muPpm: gamma };
  },
};
