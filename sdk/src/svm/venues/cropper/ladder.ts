...
/** TS mirror of wpDA. */
export function cropperDeltaA(l: bigint, lo: bigint, hi: bigint, roundUp: boolean): bigint {
...
/** TS mirror of wpDB. */
export function cropperDeltaB(l: bigint, lo: bigint, hi: bigint, roundUp: boolean): bigint {
  if (hi <= lo || l === 0n) return 0n;
  const p = l * (hi - lo);
  if (p >= Q128) return SENT;
  let q = p >> 64n;
...
/** TS mirror of wpNxA. */
export function cropperNextSqrtA(sp: bigint, l: bigint, amt: bigint): bigint {
  if (amt === 0n) return sp;
  if (l === 0n) return 0n;
  const t = l * sp;
  if (t >= G192) return 0n;
  ...
  let q = num / den;
  if (q * den < num) q += 1n;
  if (q < CROPPER_MIN_SQRT_PRICE || q > CROPPER_MAX_SQRT_PRICE) return 0n;
  return q;
}
...
function cropperConfig(cfg: PoolConfig): CropperPoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as CropperPoolConfig;
}

function liveFromState(cfg: CropperPoolConfig, state: AccountBytesMap): CropperLive {
 ...
 */
function effectiveWindow(cfg: CropperPoolConfig, state: AccountBytesMap, live: CropperLive, params: readonly bigint[]): EffectiveWindow {
  ...
  for (let k = 0; k < CROPPER_MAX_BOUNDARIES && k < nb; k++) {
    ...
    if (aToB ? btick > live.bt : btick <= live.bt) continue;
 ...
 */
function walkStep(cursor: WalkCursor, win: EffectiveWindow, live: CropperLive, aToB: boolean, rm: bigint): bigint {
  ...
  let nx = tg;
  if (fx > ca) nx = aToB ? cropperNextSqrtA(cursor.sp, cursor.l, ca) : cursor.sp + (ca << 64n) / cursor.l;
  if (nx === 0n) {
    cursor.exhausted = true;
    return rm;
  }
...
function coldWalk(win: EffectiveWindow, live: CropperLive, aToB: boolean, x: bigint): bigint | null {
...
function coldWalkClamped(win: EffectiveWindow, live: CropperLive, aToB: boolean, x: bigint): { out: bigint; cap: bigint } {
