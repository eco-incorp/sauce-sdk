 ...
 */
export const FLINT_HAIRCUT_USDT_USDC_PPM = 700_000n;
export const FLINT_HAIRCUT_PUMPCMXQ_TO_USDT_PPM = 900_000n;
export const FLINT_HAIRCUT_USDT_TO_PUMPCMXQ_PPM = 150_000n;
 ...
 */
export const FLINT_PAIR_REGISTRY: Record<string, FlintPairEntry> = {
  [USDC_MARKET as string]: {
    marketA: USDT_MARKET,
    vaultA: USDT_VAULT,
    mintA: USDT_MINT,
...
function haircutFor(cfg: FlintPoolConfig): bigint {
  if (cfg.direction === 'BtoA') {
    const h = cfg.entry.haircutBtoAPpm;
    if (h === undefined) throw new Error(`${SLUG} pool ${cfg.pool} has no haircutBtoAPpm for its 'BtoA' direction`);
    return h;
  }
  return cfg.entry.haircutAtoBPpm;
}

async function fetchPoolConfig(load: AccountLoader, pool: Address): Promise<FlintPoolConfig> {
  const entry = FLINT_PAIR_REGISTRY[pool as string];
  if (entry === undefined) {
    throw new Error(`${SLUG}: ${pool} is not a curated pair (see FLINT_PAIR_REGISTRY)`);
  }
  const data = await load(pool);
  if (data === null) throw new Error(`${SLUG} market ${pool} account not found`);
...
export const flintLadder: SvmVenueLadder = {
  ...
  defaultRungs: 4,
  /** Every curated pair/direction shares the identical quote-fragment shape (2 vaults, same haircut formula). */
  shapeKey(base): string {
    const cfg = flintConfig(base);
    return `${SLUG}:cp:${cfg.direction}`;
  },
  helpers() {
    return [
      {
        name: 'qFlint',
        source: [
          'function qFlint(x, rin, rout, discountPpm) {',
          '  if (x === 0) { return 0 }',
          '  const raw = Math.mulDiv(x, rout, rin + x);',
          '  return Math.mulDiv(raw, discountPpm, 1000000);',
          '}',
        ].join('\n'),
      },
    ];
  ...
  paramCount: 1,
  paramsFor(base): bigint[] {
    return [haircutFor(flintConfig(base))];
  },
  quoteRefs(base, slot) {
    const cfg = flintConfig(base);
    const d = directed(cfg);
    return [
      { ref: ref(slot, 'vin'), address: d.vaultIn },
      { ref: ref(slot, 'vout'), address: d.vaultOut },
    ];
  },
  emitSetup(base, slot, params) {
    ...
    return [
      ...
      `  const s${slot}discount = ${params[0]};`,
    ].join('\n');
  },
  emitQuoteCall(_base, slot, x) {
    return `qFlint(${x}, s${slot}rin, s${slot}rout, s${slot}discount)`;
  },
  buildSwapV2(base, slot, user: SwapUser): LadderSwapTemplate {
    ...
    const accounts: VenueAccount[] = [
      ...
      roled('sysvar', SYSVAR_INSTRUCTIONS),
      roled('marketIn', d.marketIn, true),
      roled('marketOut', d.marketOut, true),
      { ref: user.inAta, writable: true },
      roled('vaultIn', d.vaultIn, true),
      { ref: user.outAta, writable: true },
      roled('vaultOut', d.vaultOut, true),
    ];
    if (e.checkedMint !== undefined) {
      accounts.push(roled('mint', e.checkedMint));
    }
    accounts.push(roled('tpIn', d.tokenProgramIn));
    accounts.push(roled('tpOut', d.tokenProgramOut));
    accounts.push(roled('flintSelf', FLINT_PROGRAM_ID));
    return {
      programId: FLINT_PROGRAM_ID,
      prefix: Uint8Array.from([e.disc]),
      suffix: new Uint8Array(e.tailZeros),
      patch: 'in',
      accounts,
    };
  },
  referenceQuote(base, state: AccountBytesMap) {
    const cfg = flintConfig(base);
    const d = directed(cfg);
    const haircutPpm = haircutFor(cfg);
    const vinData = state[d.vaultIn];
    const voutData = state[d.vaultOut];
    if (vinData === undefined) throw new Error(`${SLUG} reference is missing vault ${d.vaultIn}`);
    if (voutData === undefined) throw new Error(`${SLUG} reference is missing vault ${d.vaultOut}`);
    const rin = readUintLE(vinData, AMOUNT_OFF, 8);
    const rout = readUintLE(voutData, AMOUNT_OFF, 8);
    return (x: bigint) => {
      if (x === 0n) return 0n;
      const raw = (x * rout) / (rin + x);
      return (raw * haircutPpm) / 1_000_000n;
    };
  },
  depthReserves(base, state: AccountBytesMap) {
    ...
    const vinData = state[d.vaultIn];
    const voutData = state[d.vaultOut];
    if (vinData === undefined || voutData === undefined) throw new Error(`${SLUG} depth is missing a vault`);
    return {
  ...
  continuousFees(base) {
    ...
    // actually quotes.
    return { gammaPpm: 1_000_000n, muPpm: haircutFor(flintConfig(base)) };
  },
};
