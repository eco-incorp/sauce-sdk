...
export const GAVEL_POOL_DISCRIMINANT = 9_886_743_430_086_513_268n;
...
function decodeAddress(data: Uint8Array, offset: number): Address {
  return getAddressCodec().decode(data.subarray(offset, offset + 32)) as Address;
}
 ...
 */
export async function fetchGavelConfig(load: AccountLoader, pool: Address): Promise<GavelPoolConfig> {
...
function gavelBuy(x: bigint, vld: bigint, br: bigint, qr: bigint, bs: bigint, qs: bigint, fee: bigint): bigint {
  if (vld === 0n || x === 0n) return 0n;
...
function gavelSell(x: bigint, vld: bigint, br: bigint, qr: bigint, bs: bigint, qs: bigint, fee: bigint): bigint {
  if (vld === 0n || x === 0n) return 0n;
  let sb = 0n;
  let sq = 0n;
  if (bs * qr > qs * br) {
...
export const gavelLadder: SvmVenueLadderV2 = {
  ...
  referenceQuote(
    ...
    return cfg.direction === 'quoteIn'
      ? (x: bigint) => gavelBuy(x, vld, br, qr, bs, qs, fee)
      : (x: bigint) => gavelSell(x, vld, br, qr, bs, qs, fee);
  },

  depthReserves(base: PoolConfig, state: Record<string, Uint8Array>): { reserveIn: bigint; reserveOut: bigint } {
    const cfg = gavelConfig(base);
    const data = state[cfg.pool];
  ...
  continuousFees(_base: PoolConfig, _state: Record<string, Uint8Array>, params: readonly bigint[]): { gammaPpm: bigint; muPpm: bigint } {
