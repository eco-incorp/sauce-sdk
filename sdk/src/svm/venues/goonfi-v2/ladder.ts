 ...
 * obric-v2's `${p}lo`/`${p}cap` exactly) while the COLD final quote (used
 * once, at the merge's actually-elected amount) cleanly returns 0 past either
 * edge — the same ladder-vs-final split obric-v2 uses and for the same
 * reason.
 */
...
export function goonfiColdQuote(cfg: GoonfiV2PoolConfig, x: bigint, live: LiveState): bigint {
  ...
  if (x > live.thresholds[8]!) return 0n;
  let fee = live.fees[0]!;
