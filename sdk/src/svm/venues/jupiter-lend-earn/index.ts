...
function config(base: PoolConfig): JupiterLendEarnPoolConfig {
  if (base.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${base.venue}' pool config`);
  return base as JupiterLendEarnPoolConfig;
}

async function fetchJupiterLendEarnConfig(load: AccountLoader, pool: Address): Promise<JupiterLendEarnPoolConfig> {
  const data = await load(pool);
  if (data === null) throw new Error(`jupiter-lend-earn Lending ${pool} account not found`);
  ...
  const liquidityReserve = decoder.decode(
    data.subarray(OFF_TOKEN_RESERVES_LIQUIDITY, OFF_TOKEN_RESERVES_LIQUIDITY + 32),
  );
  const supplyPositionOnLiquidity = decoder.decode(
    data.subarray(OFF_SUPPLY_POSITION_ON_LIQUIDITY, OFF_SUPPLY_POSITION_ON_LIQUIDITY + 32),
  );
  ...
  if (reserveData === null) throw new Error(`jupiter-lend-earn TokenReserve ${liquidityReserve} account not found`);
 ...
 */
async function assertClassicMint(load: AccountLoader, mint: Address): Promise<void> {
  const data = await load(mint);
  if (data === null) throw new Error(`jupiter-lend-earn mint ${mint} account not found`);
  if (data.length !== 82) {
    throw new Error(
      `jupiter-lend-earn mint ${mint} has non-classic size ${data.length} (expected 82) — Token-2022/extension mints are not implemented by this adapter`,
    );
  }
}

/** PDA(["rate_model", mint], LIQUIDITY_PROGRAM) — verified live for all three test markets. */
async function deriveRateModel(mint: Address): Promise<Address> {
...
function makeJupiterLendEarn(): {
  ...
  return {
    ...
    fetchPoolConfig: (load: AccountLoader, pool: Address): Promise<PoolConfig> => fetchJupiterLendEarnConfig(load, pool),
  };
}
...
export const jupiterLendEarnLadder: SvmVenueLadder = {
  ...
  emitSetup(cfg, slot) {
    ...
    if (c.direction === 'redeem') {
      ...
      lines.push(
        `  const s${slot}vaultbal = accountUint(${JSON.stringify(ref(slot, 'vault'))}, ${OFF_TOKEN_AMOUNT}, 8);`,
        `  const s${slot}vaultcap = s${slot}vaultbal - s${slot}vaultbal / 10000;`,
      );
    }
    return lines.join('\n');
  },
  ...
  buildSwapV2(cfg, slot, user: SwapUser): LadderSwapTemplate {
    const c = config(cfg);
    const w = (r: string, addr: Address): VenueAccount => ({ ref: ref(slot, r), address: addr, writable: true });
    const ro = (r: string, addr: Address): VenueAccount => ({ ref: ref(slot, r), address: addr });
  ...
  referenceQuote(cfg, state: AccountBytesMap) {
    const c = config(cfg);
    const lending = state[c.pool];
    if (lending === undefined) throw new Error(`${SLUG} ladder reference is missing account ${c.pool}`);
    ...
    if (c.direction === 'deposit') {
      return (x: bigint): bigint => {
        if (x === 0n || locked || price === 0n) return 0n;
        return (x * EXCHANGE_PRICES_PRECISION) / price;
      };
    }

    const vaultData = state[c.vault];
    if (vaultData === undefined) throw new Error(`${SLUG} ladder reference is missing account ${c.vault}`);
    const vaultBal = readU64(vaultData, OFF_TOKEN_AMOUNT);
    const vaultCap = vaultBal - vaultBal / 10000n;
    return (x: bigint): bigint => {
      if (x === 0n || locked) return 0n;
      let out = (x * price) / EXCHANGE_PRICES_PRECISION;
      if (out > vaultCap) out = vaultCap;
      return out;
    };
  },
  depthReserves(cfg, state: AccountBytesMap) {
    const c = config(cfg);
    const lending = state[c.pool];
    if (lending === undefined) throw new Error(`${SLUG} ladder depth is missing account ${c.pool}`);
    const price = readU64(lending, OFF_TOKEN_EXCHANGE_PRICE);
    const vaultData = state[c.vault];
...
function readU64(buf: Uint8Array, offset: number): bigint {
  if (offset + 8 > buf.length) throw new Error(`${SLUG}: truncated account data`);
  let v = 0n;
