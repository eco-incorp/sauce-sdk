/**
 * Kipseli venue adapter (SvmRoute ladder fragment, adapter contract v2).
 *
 ...
 * meteora-damm-v1-stable's OWN "Pool" structs carry (see `the consuming app SVM code`'s
 ...
 * future production wiring step), the same way sauce-recipes' `test/svm/the consuming app SVM code`
 ...
 * (`the consuming app SVM code`'s `kipseli.gate`, mirroring `quantum`'s own live-gate pattern) rather than
 ...
 * sauce-recipes' `the consuming app SVM code`'s `kipseli` pin for how this real number is folded into the family's
...
export async function deriveKipseliBanEntry(owner: Address): Promise<Address> {
...
async function deriveAta(owner: Address, mint: Address, tokenProgram: Address): Promise<Address> {
  const encoder = getAddressEncoder();
  const [pda] = await getProgramDerivedAddress({
...
function kipseliConfig(cfg: PoolConfig): KipseliPoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as KipseliPoolConfig;
}

async function fetchDecimals(load: AccountLoader, mint: Address): Promise<number> {
  const data = await load(mint);
  if (data === null) throw new Error(`${SLUG} mint ${mint} not found`);
...
async function fetchPoolConfig(load: AccountLoader, pool: Address): Promise<KipseliPoolConfig> {
  ...
  for (const [role, vault, mint] of [
    ...
    if (vaultData === null) throw new Error(`${SLUG} pool ${pool}: derived ${role} vault ${vault} does not exist`);
...
export const kipseliLadder: SvmVenueLadder = {
  ...
  buildSwapV2(base: PoolConfig, slot: number, user: SwapUser): LadderSwapTemplate {
    const cfg = kipseliConfig(base);
    const [userBaseAta, userQuoteAta] =
      cfg.direction === 'baseToQuote' ? [user.inAta, user.outAta] : [user.outAta, user.inAta];
  ...
  referenceQuote(base: PoolConfig, state: AccountBytesMap, params: readonly bigint[]): (x: bigint) => bigint {
    ...
    if (outData === undefined) throw new Error(`${SLUG} reference is missing vault ${outVault}`);
    const rout = readUintLE(outData, VAULT_AMOUNT_OFFSET, 8);
    const [adjNum, adjDen] = params;
    return (x: bigint): bigint => {
      if (x === 0n || price === 0n) return 0n;
      let raw: bigint;
  ...
  depthReserves(base: PoolConfig, state: AccountBytesMap): { reserveIn: bigint; reserveOut: bigint } {
    ...
    if (baseData === undefined || quoteData === undefined) throw new Error(`${SLUG} depth is missing a vault`);
  ...
  continuousFees(): { gammaPpm: bigint; muPpm: bigint } {
    ...
    return { gammaPpm: 1_000_000n, muPpm: (HAIRCUT_NUM * 1_000_000n) / HAIRCUT_DEN };
  },
};
