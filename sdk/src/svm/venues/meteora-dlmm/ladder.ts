 ...
 */
function emitBinWalkIncremental(p: string, swapForY: boolean, x: string, v: WalkVars, loopVar: string): string[] {
  const maxIn = swapForY ? `((${p}mo << 64) + ${p}pr - 1) / ${p}pr` : `(${p}mo * ${p}pr + ${U64_MAX}) / ${ONE}`;
  const outPartial = swapForY ? `(${p}pr * ${p}ex2) / ${ONE}` : `(${p}ex2 << 64) / ${p}pr`;
  return [
  ...
  buildSwapV2(base: PoolConfig, slot: number, user: SwapUser): LadderSwapTemplate {
    ...
    return {
      ...
      accounts: [
        ...
        roled(
          'bmx',
          cfg.bitmapExtensionExists ? cfg.bitmapExtension : METEORA_DLMM_PROGRAM_ID,
          cfg.bitmapExtensionExists,
        ),
