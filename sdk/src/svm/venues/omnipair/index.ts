...
function omnipairConfig(cfg: PoolConfig): OmnipairPoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as OmnipairPoolConfig;
}
...
async function derivePda(seeds: Uint8Array[]): Promise<Address> {
...
export async function fetchOmnipairPoolConfig(load: AccountLoader, pool: Address): Promise<OmnipairPoolConfig> {
  const data = await load(pool);
  if (data === null) throw new Error(`${SLUG}: pair account ${pool} not found`);
  ...
  const token0Mint = codec.decode(data.subarray(OFF_TOKEN0, OFF_TOKEN0 + 32)) as Address;
  const token1Mint = codec.decode(data.subarray(OFF_TOKEN1, OFF_TOKEN1 + 32)) as Address;
  const rateModel = codec.decode(data.subarray(OFF_RATE_MODEL, OFF_RATE_MODEL + 32)) as Address;
  const swapFeeBps = readUintLE(data, OFF_SWAP_FEE_BPS, 2);
  if (swapFeeBps >= FEE_D) throw new Error(`${SLUG}: pair ${pool} has a degenerate swap_fee_bps ${swapFeeBps}`);

  const tag = data[OFF_OPTION_TAG];
  const reserveBase: 147 | 149 = tag === 0 ? 147 : 149;
  const reserve0 = readUintLE(data, reserveBase, 8);
  const reserve1 = readUintLE(data, reserveBase + 8, 8);
  if (reserve0 === 0n || reserve1 === 0n) throw new Error(`${SLUG}: pair ${pool} is not initialized (zero reserve)`);

  for (const mint of [token0Mint, token1Mint]) {
    const mintData = await load(mint);
    if (mintData === null) throw new Error(`${SLUG}: mint ${mint} of pair ${pool} not found`);
...
interface LiveState {
  ...
  co: bigint;
  fb: bigint;
  /** Capacity-clamped max GROSS input (U64_MAX sentinel when uncapped). */
  icap: bigint;
}

/** reserveIn/reserveOut/cashOut offsets within the Pair account for `direction`. */
function directedOffsets(cfg: OmnipairPoolConfig): { offRin: number; offRout: number; offCout: number } {
 ...
 */
export function omnipairCapacity(reserveIn: bigint, reserveOut: bigint, cashOut: bigint, feeBps: bigint): bigint {
  if (reserveOut <= cashOut) return U64_MAX;
  const kq = reserveIn * reserveOut;
  const target = reserveOut - cashOut;
  let netInMax = kq / target - reserveIn;
  if (netInMax < 0n) netInMax = 0n;
...
function liveState(cfg: OmnipairPoolConfig, state: AccountBytesMap): LiveState {
  const pool = state[cfg.pool];
  if (pool === undefined) throw new Error(`${SLUG} reference is missing account ${cfg.pool}`);
...
export function omnipairQuote(x: bigint, ri: bigint, ro: bigint, fb: bigint, icap: bigint): bigint {
...
export const omnipairLadder = {
  ...
  referenceQuote(base: PoolConfig, state: AccountBytesMap, _params?: readonly bigint[], _now?: bigint): (x: bigint) => bigint {
    ...
    return (x: bigint): bigint => omnipairQuote(x, ri, ro, fb, icap);
  },
  ...
  referenceLadderQuotes(base: PoolConfig, state: AccountBytesMap, _params?: readonly bigint[], _now?: bigint): (grid: readonly bigint[]) => bigint[] {
    const cfg = omnipairConfig(base);
    const { ri, ro, fb, icap } = liveState(cfg, state);
    return (grid: readonly bigint[]): bigint[] => grid.map((x) => omnipairQuote(x, ri, ro, fb, icap));
  },
  ...
  depthReserves(base: PoolConfig, state: AccountBytesMap, _now?: bigint): { reserveIn: bigint; reserveOut: bigint } {
    const cfg = omnipairConfig(base);
    const pool = state[cfg.pool];
    if (pool === undefined) throw new Error(`${SLUG} depth is missing account ${cfg.pool}`);
    const rb = cfg.reserveBase;
  ...
  continuousFees(base: PoolConfig, state: AccountBytesMap, _params?: readonly bigint[]): { gammaPpm: bigint; muPpm: bigint } {
    const cfg = omnipairConfig(base);
    const pool = state[cfg.pool];
    if (pool === undefined) throw new Error(`${SLUG} fees are missing account ${cfg.pool}`);
    const fb = readUintLE(pool, OFF_SWAP_FEE_BPS, 2);
