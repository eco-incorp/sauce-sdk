...
export interface OpenBookV2PoolConfig extends PoolConfig {
  ...
  oracleA?: Address;
  oracleB?: Address;
  windows: { baseIn: OpenBookV2Window; quoteIn: OpenBookV2Window };
}

/** The direction's shipped window (the ladder fragment and the orchestrator gate read through this). */
export function openbookV2WindowFor(cfg: OpenBookV2PoolConfig): OpenBookV2Window {
...
function readPubkeyIsZero(data: Uint8Array, offset: number): boolean {
...
function sortBestFirst(orders: { nodeOffset: number; key: bigint; priceLots: bigint }[], descending: boolean): OpenBookV2Order[] {
  const sorted = [...orders].sort((a, b) => {
    if (a.key === b.key) return 0;
    if (descending) return a.key > b.key ? -1 : 1;
...
function walkFixedTree(data: Uint8Array): { nodeOffset: number; key: bigint; priceLots: bigint }[] {
  const maybeNode = readUintLE(data, 8, 4);
  const leafCount = readUintLE(data, 12, 4);
  const out: { nodeOffset: number; key: bigint; priceLots: bigint }[] = [];
  if (leafCount === 0n) return out;
  const stack: number[] = [Number(maybeNode)];
  let visited = 0;
  const BOUND = 4096; // 1024 slots, generous bound against a malformed/cyclic tree
  while (stack.length > 0) {
    if (++visited > BOUND) throw new Error(`${SLUG}: BookSide tree walk exceeded ${BOUND} visits — refusing to loop`);
    const idx = stack.pop()!;
...
export async function fetchOpenBookV2Config(load: AccountLoader, pool: Address): Promise<OpenBookV2PoolConfig> {
  const market = await load(pool);
  if (market === null) throw new Error(`${SLUG} market ${pool} does not exist`);
...
function decodeAddressFrom(data: Uint8Array, offset: number): Address {
  return address(base58Encode(data.subarray(offset, offset + 32)));
}

function openbookV2Config(cfg: PoolConfig): OpenBookV2PoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as OpenBookV2PoolConfig;
}
...
function emitLevelRead(p: string, slot: number, k: number, params: readonly string[]): string[] {
...
function emitWalk(cfg: OpenBookV2PoolConfig, p: string, slot: number, xExpr: string, outVar: string, decl: string, tag: string, indent: string, isFinal: boolean): string[] {
  ...
  if (baseIn) {
    lines.push(
      `${indent}  let ${p}rm${tag} = ${xExpr} / ${p}bls;`,
      `${indent}  let ${p}out${tag} = 0; let ${p}k${tag} = 0;`,
      `${indent}  for (let ${p}w${tag} = 0; ${p}w${tag} < ${OPENBOOK_V2_MAX_ORDERS} && ${p}rm${tag} > 0 && ${p}k${tag} < ${p}nb; ${p}w${tag}++) {`,
      `${indent}    const ${p}mb${tag} = ${p}rm${tag} < ${p}sz[${p}k${tag}] ? ${p}rm${tag} : ${p}sz[${p}k${tag}];`,
      `${indent}    ${p}out${tag} = ${p}out${tag} + ${p}mb${tag} * ${p}pr[${p}k${tag}];`,
      `${indent}    ${p}rm${tag} = ${p}rm${tag} - ${p}mb${tag};`,
      `${indent}    ${p}k${tag} = ${p}k${tag} + 1;`,
      `${indent}  }`,
      `${indent}  const ${p}qtn${tag} = ${p}out${tag} * ${p}qls;`,
    ...
    lines.push(
      `${indent}  const ${p}nql${tag} = ${xExpr} / ${p}qls;`,
      `${indent}  let ${p}rm${tag} = (${p}nql${tag} * 1000000) / (1000000 + ${p}tfp);`,
      `${indent}  let ${p}ob${tag} = 0; let ${p}oq${tag} = 0; let ${p}k${tag} = 0; let ${p}dn${tag} = 0;`,
      `${indent}  for (let ${p}w${tag} = 0; ${p}w${tag} < ${OPENBOOK_V2_MAX_ORDERS} && ${p}rm${tag} > 0 && ${p}k${tag} < ${p}nb && ${p}dn${tag} === 0; ${p}w${tag}++) {`,
      `${indent}    const ${p}mm${tag} = ${p}rm${tag} / ${p}pr[${p}k${tag}];`,
      `${indent}    if (${p}mm${tag} === 0) { ${p}dn${tag} = 1 }`,
      `${indent}    else {`,
      `${indent}      const ${p}mb${tag} = ${p}mm${tag} < ${p}sz[${p}k${tag}] ? ${p}mm${tag} : ${p}sz[${p}k${tag}];`,
      `${indent}      const ${p}mq${tag} = ${p}mb${tag} * ${p}pr[${p}k${tag}];`,
      `${indent}      ${p}rm${tag} = ${p}rm${tag} - ${p}mq${tag};`,
      `${indent}      ${p}ob${tag} = ${p}ob${tag} + ${p}mb${tag};`,
      `${indent}      ${p}oq${tag} = ${p}oq${tag} + ${p}mq${tag};`,
      `${indent}      ${p}k${tag} = ${p}k${tag} + 1;`,
      `${indent}    }`,
      `${indent}  }`,
      `${indent}  const ${p}qtn${tag} = ${p}oq${tag} * ${p}qls;`,
      `${indent}  const ${p}reb${tag} = (${p}qtn${tag} * ${p}mrp) / 1000000;`,
      `${indent}  ${outVar} = ${p}ob${tag} * ${p}bls;`,
      `${indent}  ${p}lo = ${outVar};`,
      `${indent}  ${p}lx = ${p}qtn${tag} + ${p}reb${tag};`,
    );
  }
  lines.push(`${indent}}`);
  if (isFinal) {
...
export const openbookV2Ladder: SvmVenueLadderV2 = {
  ...
  buildSwapV2(base, slot, user): LadderSwapTemplate {
    const cfg = openbookV2Config(base);
    const baseIn = cfg.direction === 'baseIn';
    ...
    const roled = (r: string, addr: Address, writable?: boolean): VenueAccount =>
      writable ? { ref: ref(slot, r), address: addr, writable: true } : { ref: ref(slot, r), address: addr };
    return {
  ...
  referenceQuote(base, state, params) {
    ...
    if (bookData === undefined) throw new Error(`${SLUG} ladder reference is missing account ${bookAddress}`);
    ...
    const levels: { priceLots: bigint; quantity: bigint }[] = [];
    ...
    return (x: bigint): bigint => {
      ...
      if (baseIn) {
        ...
        const rebate = (quoteTakenNative * makerRebatePpm) / 1_000_000n;
      ...
      for (const level of levels) {
        if (remainingQuoteLots <= 0n) break;
        const maxMatch = remainingQuoteLots / level.priceLots;
        if (maxMatch === 0n) break;
  ...
  depthReserves(base, state) {
    const cfg = openbookV2Config(base);
    const window = openbookV2WindowFor(cfg);
    if (window.orders.length === 0) return { reserveIn: 0n, reserveOut: 0n };
    const bookAddress = cfg.direction === 'baseIn' ? cfg.bidsAccount : cfg.asksAccount;
    const bookData = state[bookAddress];
    let reserveIn = 0n;
    let reserveOut = 0n;
