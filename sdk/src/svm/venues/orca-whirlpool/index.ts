 ...
 */
export function windowStartTicks(tickCurrentIndex: number, tickSpacing: number, aToB: boolean): [number, number, number] {
  const span = TICK_ARRAY_SIZE * tickSpacing;
  const base = floorDiv(tickCurrentIndex, span) * span;
  if (aToB) return [base, base - span, base - 2 * span];
...
async function resolveWindow(
  ...
  const boundaries: WhirlpoolBoundary[] = [];
  ...
  let edge: WhirlpoolWindow['edge'] = null;
 ...
 */
export async function fetchOrcaWhirlpoolConfig(load: AccountLoader, pool: Address): Promise<OrcaWhirlpoolPoolConfig> {
