/**
 * PancakeSwap (Solana CLMM) ladder fragment (adapter contract v2) — the
 * in-VM tick walk + swap CPI template. Byte-for-byte the SAME walk math as
...
import type { PancakeswapClmmPoolConfig, PancakeswapClmmWindow } from './index.js';

export { raydiumSqrtPriceAtTick, PANCAKESWAP_CLMM_MAX_BOUNDARIES };
...
/** sha256('global:swap_v2')[0..8] — verified live against a real PancakeSwap CLMM tx (see ./index.ts). */
const SWAP_V2_DISCRIMINATOR = [43, 4, 237, 11, 26, 201, 30, 98];
/** Walk loop bound: MAX_BOUNDARIES full steps + the edge step + one partial. */
const WALK_BOUND = PANCAKESWAP_CLMM_MAX_BOUNDARIES + 2;
/** cfg words per slot: nb + (meta,hi,lo) per boundary + (tick,hi,lo) edge. */
const PARAM_COUNT = 1 + 3 * PANCAKESWAP_CLMM_MAX_BOUNDARIES + 3;
...
function pcConfig(cfg: PoolConfig): PancakeswapClmmPoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as PancakeswapClmmPoolConfig;
}
...
function liveFromState(cfg: PancakeswapClmmPoolConfig, state: AccountBytesMap): Live {
...
function effectiveWindow(cfg: PancakeswapClmmPoolConfig, state: AccountBytesMap, live: Live, params: readonly bigint[]): EffectiveWindow {
  ...
  for (let k = 0; k < PANCAKESWAP_CLMM_MAX_BOUNDARIES && k < nb; k++) {
    const meta = params[1 + 3 * k];
    const btick = meta & M32;
    if (zeroForOne ? btick > live.bt : btick <= live.bt) continue;
  ...
  const edgeTick = params[1 + 3 * PANCAKESWAP_CLMM_MAX_BOUNDARIES] ?? 0n;
  if (edgeTick !== 0n && (zeroForOne ? edgeTick <= live.bt : edgeTick > live.bt)) {
    result.bsp.push((params[2 + 3 * PANCAKESWAP_CLMM_MAX_BOUNDARIES] << 64n) | params[3 + 3 * PANCAKESWAP_CLMM_MAX_BOUNDARIES]);
    result.bnt.push(0n);
  }
...
function walkStep(cursor: WalkCursor, win: EffectiveWindow, live: Live, zeroForOne: boolean, rm: bigint): bigint {
...
function coldWalk(win: EffectiveWindow, live: Live, zeroForOne: boolean, x: bigint): bigint | null {
...
function coldWalkClamped(win: EffectiveWindow, live: Live, zeroForOne: boolean, x: bigint): { out: bigint; cap: bigint } {
...
// ---------------------------------------------------------------------------
// Fragment emission (the fragment twin of the walk above) — mirrors
// raydium-clmm's emitWalkStep/emitBoundary verbatim.
// ---------------------------------------------------------------------------
interface EmitVars {
  sp: string;
  l: string;
...
function emitWalkStep(p: string, zeroForOne: boolean, v: EmitVars, indent: string): string[] {
  const dIn = (lo: string, hi: string) => `rcD0(${v.l}, ${lo}, ${hi}, 1)`;
  const dInB = (lo: string, hi: string) => `rcD1(${v.l}, ${lo}, ${hi}, 1)`;
...
export const pancakeswapClmmLadder: SvmVenueLadder = {
  ...
  shapeKey(base: PoolConfig): string {
    return `${SLUG}:${pcConfig(base).direction}`;
  },
  ...
  paramsFor(base: PoolConfig): bigint[] {
    const window = windowFor(pcConfig(base));
    const words = [BigInt(window.boundaries.length)];
    for (let k = 0; k < PANCAKESWAP_CLMM_MAX_BOUNDARIES; k++) {
  ...
  emitSetup(base: PoolConfig, slot: number, params: readonly string[], enableVar?: string): string {
    ...
    const lines = [
      ...
      `  let ${p}nbv = 0;`,
      `  let ${p}kmx = 0;`,
      `  const ${p}bsp = new Array(${PANCAKESWAP_CLMM_MAX_BOUNDARIES + 1});`,
      `  const ${p}bnt = new Array(${PANCAKESWAP_CLMM_MAX_BOUNDARIES + 1});`,
      `  const ${p}fxa = new Array(${PANCAKESWAP_CLMM_MAX_BOUNDARIES + 1});`,
      `  const ${p}csn = new Array(${PANCAKESWAP_CLMM_MAX_BOUNDARIES + 1});`,
      `  const ${p}cot = new Array(${PANCAKESWAP_CLMM_MAX_BOUNDARIES + 1});`,
      `  const ${p}lps = new Array(${PANCAKESWAP_CLMM_MAX_BOUNDARIES + 1});`,
      ...
      `  if (${enabled} !== 0) {`,
      ...Array.from({ length: PANCAKESWAP_CLMM_MAX_BOUNDARIES }, (_, k) => emitBoundary(p, slot, k, zeroForOne, params)).flat(),
      `    ${p}t1 = ${params[1 + 3 * PANCAKESWAP_CLMM_MAX_BOUNDARIES]};`,
      `    if (${p}t1 !== 0 && (${edgeKeep})) {`,
      `      ${p}bsp[${p}nbv] = (${params[2 + 3 * PANCAKESWAP_CLMM_MAX_BOUNDARIES]} << 64) | ${params[3 + 3 * PANCAKESWAP_CLMM_MAX_BOUNDARIES]};`,
      `      ${p}bnt[${p}nbv] = 0;`,
      `      ${p}nbv = ${p}nbv + 1;`,
      `    }`,
      `  }`,
  ...
  emitLadderQuote(base: PoolConfig, slot: number, rung: number, x: string, outVar: string): string {
    ...
    const zeroForOne = cfg.direction === '0to1';
    const p = `s${slot}`;
    const v: EmitVars = { sp: `${p}wsp`, l: `${p}wl`, k: `${p}wk`, ex: `${p}wex`, out: `${p}wout`, rm: `${p}rm` };
    return [
  ...
  emitFinalQuote(base: PoolConfig, slot: number, x: string, outVar: string): string {
    ...
    const zeroForOne = cfg.direction === '0to1';
    const p = `s${slot}`;
    const v: EmitVars = { sp: `${p}fsp`, l: `${p}fl`, k: `${p}fk`, ex: `${p}fex`, out: `${p}fo`, rm: `${p}frm` };
    return [
      `  let ${outVar} = 0;`,
      `  if (${p}vld !== 0 && ${x} > 0) {`,
  ...
  referenceQuote(base: PoolConfig, state: AccountBytesMap, params: readonly bigint[]) {
    const cfg = pcConfig(base);
    const zeroForOne = cfg.direction === '0to1';
  ...
  referenceCapacities(base: PoolConfig, state: AccountBytesMap, params: readonly bigint[]) {
    const cfg = pcConfig(base);
    const zeroForOne = cfg.direction === '0to1';
    const live = liveFromState(cfg, state);
    const win = effectiveWindow(cfg, state, live, params);
