...
function detectTokenProgram(mint: Address, data: Uint8Array): Address {
  ...
  while (offset + 4 <= data.length) {
    ...
    if (extensionType === 1) {
      throw new Error(
        `pumpfun-bonding-curve gate: mint ${mint} carries a token-2022 TransferFeeConfig extension (vault/ATA deltas would not match user amounts)`,
      );
    }
...
function parseFeeTiers(data: Uint8Array): { marketCapThreshold: bigint; protocolFeeBps: bigint; creatorFeeBps: bigint }[] {
  const count = Number(readUintLE(data, 65, 4));
  const tiers: { marketCapThreshold: bigint; protocolFeeBps: bigint; creatorFeeBps: bigint }[] = [];
...
function pickFeeTier<T extends { marketCapThreshold: bigint }>(tiers: T[], marketCap: bigint): T {
...
async function loadAccount(load: AccountLoader, addr: Address, what: string): Promise<Uint8Array> {
  ...
  if (data === null) throw new Error(`pumpfun-bonding-curve ${what} ${addr} not found`);
  return data;
}
...
function asCfg(cfg: PoolConfig): PumpfunBondingCurvePoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config for pool ${cfg.pool}`);
  return cfg as PumpfunBondingCurvePoolConfig;
}
...
export const PUMPFUN_BONDING_CURVE_ASSOCIATED_USER_VOLUME_ACCUMULATOR_REF =
  'pumpfun-bonding-curve-associated-user-volume-accumulator';

/** PDA(["user_volume_accumulator", user], PROGRAM) — resolve at execute time with the real wallet. */
export async function pumpfunBondingCurveUserVolumeAccumulatorPda(user: Address): Promise<Address> {
...
export async function pumpfunBondingCurveAssociatedUserVolumeAccumulator(user: Address): Promise<Address> {
...
export const pumpfunBondingCurve = {
   ...
   */
  async fetchPoolConfig(load: AccountLoader, pool: Address, mint: Address): Promise<PumpfunBondingCurvePoolConfig> {
    ...
    if (creator === DEFAULT_PUBKEY) creatorFeeBps = 0n;
...
export const pumpfunBondingCurveLadder: SvmVenueLadderV2 = {
  ...
  continuousFees(base, _state, params) {
    ...
    const [prot, cr] = params as [bigint, bigint];
    if (cfg.direction === 'quoteToBase') {
      return { gammaPpm: (BPS * 1_000_000n) / (BPS + prot + cr), muPpm: 1_000_000n };
    }
    return { gammaPpm: 1_000_000n, muPpm: ((BPS - prot - cr) * 1_000_000n) / BPS };
  },
};
