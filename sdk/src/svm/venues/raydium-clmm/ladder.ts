 ...
 * initialized-tick boundaries (biased tick, array cell, sqrt price) plus the
 * sequence EDGE as per-trade cfg params, and the fragment reads everything
 * VALUE-BEARING live at cook time — sqrt_price_x64 / tick_current / liquidity
 * from the pool, trade_fee_rate from the AmmConfig, each boundary's
 * liquidity_gross (initialized check) + liquidity_net i128 from its tick array.
 *
 * The walk is the classic Uniswap-V3 exact-input loop the Raydium program runs
 * when `get_dynamic_fee_info()` is None (the fetch gate rejects dynamic-fee
 * pools): jump straight to the next initialized tick, fee on the input,
 * static `trade_fee_rate`. It is STRUCTURALLY the same compute_swap loop as
 * whirlpool (fee-floor on amount_calculated, ceil'd fixed deltas, floor'd
 * unfixed deltas, per-step ceil fee, remainder-fee partial step, monotone
 * self-deactivating clamps), differing only in the primitives:
 * - delta_1 (rcD1) and next_sqrt_from_0 (rcNx0) are BIT-IDENTICAL to
 *   whirlpool's wpDB / wpNxA over the valid domain (Raydium uses its own MIN/
 *   MAX_SQRT_PRICE bounds in the clamp);
 * - delta_0 (rcD0) uses Raydium's NESTED rounding
 *   `round(round(L<<64·(b−a)/b)/a)` (Uniswap-V3 getAmount0Delta), NOT
 *   whirlpool's single-division form;
 * - the fee_rate is read LIVE from a separate AmmConfig account (`s<i>:cfg`),
 *   not the pool;
 * - a tick is initialized iff its liquidity_gross (u128) is nonzero;
 * - the crossing applies −liquidity_net going down / +liquidity_net going up
 *   over the raw two's-complement word (add_delta), identical to whirlpool's.
 *
 * Venue-exactness sources (transcribed): github.com/raydium-io/raydium-clmm
 * programs/amm/src/libraries/{swap_math,sqrt_price_math,liquidity_math,
 * tick_math}.rs + instructions/swap.rs `swap_internal`. Every venue abort
 * (delta > u64::MAX, liquidity over/underflow on a cross, amount underflow)
 * maps to the same conservative clamp: the walk marks itself exhausted, the
 * quote past that point is 0, the CPI is never sent into a rejecting state.
 * The SENT sentinel (2^65 > u64::MAX) carries "exceeds max" through helpers.
 */
...
import type { RaydiumClmmPoolConfig } from './index.js';
...
const SENT = 1n << 65n;
const FEE_MUL = 1_000_000n; // FEE_RATE_DENOMINATOR_VALUE
...
/** TS mirror of rcD0. */
export function raydiumDelta0(l: bigint, lo: bigint, hi: bigint, roundUp: boolean): bigint {
  if (hi <= lo || l === 0n) return 0n;
  const num = (l << 64n) * (hi - lo);
  let inner = num / hi;
  if (roundUp && inner * hi < num) inner += 1n;
  let q = inner / lo;
  if (roundUp && q * lo < inner) q += 1n;
  return q > U64_MAX ? SENT : q;
}

/** TS mirror of rcD1 (bit-identical to whirlpool wpDB). */
export function raydiumDelta1(l: bigint, lo: bigint, hi: bigint, roundUp: boolean): bigint {
  if (hi <= lo || l === 0n) return 0n;
  const p = l * (hi - lo);
  let q = p >> 64n;
...
/** TS mirror of rcNx0. */
export function raydiumNextSqrt0(sp: bigint, l: bigint, amt: bigint): bigint {
  if (amt === 0n) return sp;
  if (l === 0n) return 0n;
  const t = l * sp;
  if (t >= G192) return 0n;
  ...
  let q = num / den;
  if (q * den < num) q += 1n;
  if (q < MIN_SQRT_PRICE_X64 || q > MAX_SQRT_PRICE_X64) return 0n;
  return q;
}

// ---------------------------------------------------------------------------
// TS mirror of the setup + walk (transcribes ONE algorithm with the emission).
// ---------------------------------------------------------------------------

interface RaydiumLive {
  l: bigint;
  sp: bigint;
  bt: bigint; // biased live tick
  fr: bigint;
  fn: bigint;
}
...
function rayConfig(cfg: PoolConfig): RaydiumClmmPoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as RaydiumClmmPoolConfig;
}

function liveFromState(cfg: RaydiumClmmPoolConfig, state: AccountBytesMap): RaydiumLive {
  const pool = state[cfg.pool];
  if (pool === undefined) throw new Error(`${SLUG} ladder reference is missing pool ${cfg.pool}`);
  const config = state[cfg.ammConfig];
  if (config === undefined) throw new Error(`${SLUG} ladder reference is missing AmmConfig ${cfg.ammConfig}`);
...
function effectiveWindow(
  ...
  for (let k = 0; k < RAYDIUM_CLMM_MAX_BOUNDARIES && k < nb; k++) {
    ...
    if (zeroForOne ? btick > live.bt : btick <= live.bt) continue; // behind the live tick
    ...
    if (readUintLE(data, cell + OFF_TICK_LIQ_GROSS, 16) === 0n) continue; // removed since prepare
...
function walkStep(cursor: WalkCursor, win: EffectiveWindow, live: RaydiumLive, zeroForOne: boolean, rm: bigint): bigint {
  ...
  let nx = tg;
  if (fx > ca) nx = zeroForOne ? raydiumNextSqrt0(cursor.sp, cursor.l, ca) : cursor.sp + (ca << 64n) / cursor.l;
  if (nx === 0n) {
    cursor.exhausted = true;
    return rm;
  }
...
function coldWalk(win: EffectiveWindow, live: RaydiumLive, zeroForOne: boolean, x: bigint): bigint | null {
...
function emitWalkStep(p: string, zeroForOne: boolean, v: WalkVars, indent: string): string[] {
  const dIn = (lo: string, hi: string): string => `rcD0(${v.l}, ${lo}, ${hi}, 1)`;
  const dInB = (lo: string, hi: string): string => `rcD1(${v.l}, ${lo}, ${hi}, 1)`;
...
function emitBoundary(p: string, slot: number, k: number, zeroForOne: boolean, params: readonly string[]): string[] {
