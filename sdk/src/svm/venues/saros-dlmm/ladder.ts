...
import type { SarosDlmmPoolConfig } from './index.js';
...
function sarosConfig(cfg: PoolConfig): SarosDlmmPoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as SarosDlmmPoolConfig;
}
...
function feeParamsFromCfg(params: readonly bigint[]): FeeParams {
  return {
    baseFee: params[0]!,
    binStep: params[1]!,
    vfc: params[2]!,
...
function liveFromState(cfg: SarosDlmmPoolConfig, state: AccountBytesMap, fee: FeeParams, now: bigint): LiveState {
...
function totalFeeForBin(fee: FeeParams, live: LiveState, bid: bigint): bigint {
...
function effectiveBins(cfg: SarosDlmmPoolConfig, state: AccountBytesMap, live: LiveState, params: readonly bigint[]): KeptBin[] {
  ...
  for (let k = 0; k < SAROS_DLMM_MAX_BINS && k < nb; k++) {
    ...
    if (data === undefined) throw new Error(`${SLUG} ladder reference is missing account ${arrayAddr}`);
...
function mulShr(x: bigint, y: bigint, rounding: 'up' | 'down'): bigint {
  const p = x * y;
  return rounding === 'up' ? (p + ONE - 1n) / ONE : p / ONE;
}
function shlDiv(x: bigint, y: bigint, rounding: 'up' | 'down'): bigint {
  const p = x * ONE;
  return rounding === 'up' ? (p + y - 1n) / y : p / y;
}
const binAmountOut = (amountIn: bigint, price: bigint, swapForY: boolean, rounding: 'up' | 'down'): bigint =>
  swapForY ? mulShr(price, amountIn, rounding) : shlDiv(amountIn, price, rounding);
const binAmountIn = (amountOut: bigint, price: bigint, swapForY: boolean, rounding: 'up' | 'down'): bigint =>
  swapForY ? shlDiv(amountOut, price, rounding) : mulShr(amountOut, price, rounding);

/** Cold bin walk; null when x exceeds the shipped-window capacity. */
function coldWalk(kept: readonly KeptBin[], fee: FeeParams, live: LiveState, swapForY: boolean, x: bigint): bigint | null {
  if (x === 0n) return 0n;
...
function coldWalkClamped(kept: readonly KeptBin[], fee: FeeParams, live: LiveState, swapForY: boolean, x: bigint): { out: bigint; cap: bigint } {
...
function emitBinWalk(p: string, swapForY: boolean, x: string, v: { rm: string; out: string; ex: string }, loopVar: string): string[] {
...
function emitBinUnpack(p: string, slot: number, k: number, swapForY: boolean, params: readonly string[]): string[] {
  ...
  const amountOffset = swapForY ? OFF_BIN_RESERVE_Y : OFF_BIN_RESERVE_X;
  const readArm = (a: 0 | 1): string => `${p}u5 = accountUint(${JSON.stringify(ref(slot, `ba${a}`))}, ${OFF_BA_BINS} + ${p}u3 * ${BIN_LEN} + ${amountOffset}, 8);`;
  return [
...
export const sarosDlmmLadder: SvmVenueLadder = {
  ...
  buildSwapV2(base, slot, user: SwapUser): LadderSwapTemplate {
    ...
    const swapForY = cfg.direction === 'xToY';
    const window = windowFor(cfg);
    const roled = (role: string, addr: Address, writable?: boolean) =>
