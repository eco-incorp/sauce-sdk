 ...
 * "shape" param except the one un-derived additive constant K (registry-keyed
 * — see index.ts's REGISTRY_K and the "residualRisk" note below): every other
 ...
 * per-registry additive constant that isn't structurally derived from the
 * registry bytes (a 40-entry table walk + a ~20-function decision tree this
 * port does not attempt) — it is a value pinned per known registry
 * (index.ts's REGISTRY_K), proven wei-exact for FmxXDS... (84+ fresh checks)
 * and accepted with a disclosed, always-safe (never favourable) <=1-unit
 * residual for QoFvFhDZ.... An unrecognized registry throws at
 * fetchPoolConfig rather than guessing.
 */
