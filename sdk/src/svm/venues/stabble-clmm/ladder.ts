...
import type { StabbleClmmPoolConfig, StabbleClmmWindow } from './index.js';
...
function sbConfig(cfg: PoolConfig): StabbleClmmPoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as StabbleClmmPoolConfig;
}
...
function liveFromState(cfg: StabbleClmmPoolConfig, state: AccountBytesMap): Live {
...
function effectiveWindow(cfg: StabbleClmmPoolConfig, state: AccountBytesMap, live: Live, params: readonly bigint[]): EffectiveWindow {
...
export const stabbleClmmLadder: SvmVenueLadderV2 = {
  slug: SLUG,
