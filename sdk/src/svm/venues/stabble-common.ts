...
export const STABBLE_ONE = 1_000_000_000n; // fixed-point SCALE=9 (both variants' `fixed_math::ONE`)
...
export function mulDivDown(a: bigint, n: bigint, d: bigint): bigint {
  if (d === 0n) return 0n;
  return (a * n) / d;
}
/** ceil(a*n/d), 0 on a zero divisor (engine DIV-rule parity). */
export function mulDivUp(a: bigint, n: bigint, d: bigint): bigint {
  if (d === 0n) return 0n;
  return (a * n + d - 1n) / d;
}
/** ceil(a/d), 0 on a zero divisor (engine DIV-rule parity). */
export function divUpRaw(a: bigint, d: bigint): bigint {
  if (d === 0n) return 0n;
  return (a + d - 1n) / d;
}
...
export function complement(a: bigint): bigint {
  return a >= STABBLE_ONE ? 0n : STABBLE_ONE - a;
}

// ---- token wrapped/unwrapped scaling (Pool::calc_{wrapped,unwrapped,rounded}_amount) ----
...
export function calcWrappedAmount(amount: bigint, token: StabbleTokenScale): bigint {
  if (token.scalingFactor === 1n) return amount;
  return token.scalingUp ? amount * token.scalingFactor : amount / token.scalingFactor;
}
export function calcUnwrappedAmount(amount: bigint, token: StabbleTokenScale): bigint {
  if (token.scalingFactor === 1n) return amount;
  return token.scalingUp ? amount / token.scalingFactor : amount * token.scalingFactor;
}
export function calcRoundedAmount(amount: bigint, token: StabbleTokenScale): bigint {
  if (token.scalingUp) return amount;
  return (amount / token.scalingFactor) * token.scalingFactor;
}

// ---- Pool / Vault account decode ----
...
function decodeTokensVec(data: Uint8Array, offset: number, tokenSize: number, weighted: boolean) {
  const count = data[offset] | (data[offset + 1] << 8) | (data[offset + 2] << 16) | (data[offset + 3] << 24);
  let cursor = offset + 4;
  const pubkey = getAddressDecoder();
  const tokens: Array<StabblePoolTokenBase & { weight?: bigint }> = [];
  for (let i = 0; i < count; i++) {
    ...
    const token: StabblePoolTokenBase & { weight?: bigint } = { mint, decimals, scalingUp, scalingFactor, balance };
    if (weighted) token.weight = readUintLE(data, cursor + 50, 8);
    tokens.push(token);
    cursor += tokenSize;
  }
  return tokens;
...
export function decodeStabbleVault(pool: Address, data: Uint8Array | null): StabbleVaultInfo {
  if (data === null) throw new Error(`stabble vault account (pool ${pool}'s vault) not found`);
  if (data.length < 115) throw new Error(`stabble vault account for pool ${pool} is ${data.length} bytes, expected >= 115`);
  for (let i = 0; i < 8; i++) {
...
export function decodeStabblePoolCommon(
  pool: Address,
  ...
  if (data === null) throw new Error(`${programLabel} pool account ${pool} not found`);
  ...
  if (tokens.length < 2) throw new Error(`${programLabel} pool ${pool} has fewer than 2 tokens`);
...
export function createProgramAddress(seeds: Uint8Array[], programId: Address): Address {
  ...
  h.update(encoder.encode(programId) as Uint8Array);
...
export function deriveStabbleVaultAuthority(vault: Address, authorityBump: number): Address {
  const encoder = getAddressEncoder();
  return createProgramAddress(
    [Buffer.from('vault_authority'), encoder.encode(vault) as Uint8Array, Uint8Array.of(authorityBump)],
...
export async function findStabbleAta(owner: Address, mint: Address): Promise<Address> {
  const encoder = getAddressEncoder();
  const [ata] = await getProgramDerivedAddress({
    programAddress: STABBLE_ATA_PROGRAM_ID,
    seeds: [encoder.encode(owner) as Uint8Array, encoder.encode(STABBLE_TOKEN_PROGRAM_ID) as Uint8Array, encoder.encode(mint) as Uint8Array],
  });
  return ata;
}

function assertU64AmountIn(amountIn: bigint, label: string): void {
  if (amountIn <= 0n) throw new Error(`${label} amountIn must be positive, got ${amountIn}`);
  if (amountIn > U64_MAX) throw new Error(`${label} amountIn must fit u64, got ${amountIn}`);
}
...
export function stableCalcInvariantN(amplification: bigint, balances: readonly bigint[]): bigint {
  const sum0 = balances.reduce((a, b) => a + b, 0n);
  if (sum0 === 0n) return 0n;
  ...
  for (let round = 0; round < 64; round++) {
    let p = invariant;
    for (const bt of balancesTimes) p = mulDivDown(p, invariant, bt);
    const prev = invariant;
    invariant = mulDivDown(
      mulDivDown(ampTimesTotal, sum0, STABLE_AMP_PRECISION) + p * n,
      invariant,
      mulDivDown(ampTimesTotal - STABLE_AMP_PRECISION, invariant, STABLE_AMP_PRECISION) + (n + 1n) * p,
    );
    const diff = invariant > prev ? invariant - prev : prev - invariant;
    if (diff <= STABLE_DEFAULT_INV_THRESHOLD) return invariant;
  }
...
export function stableCalcOutGivenIn(
  amplification: bigint,
  balances: readonly bigint[],
  tokenIndexIn: number,
  ...
  if (finalBalanceOut + 1n >= balanceOut) return 0n; // checked_sub would fail on-chain
...
export function stableGetAmplification(
  ...
  if (currentTs <= rampStartTs) return ai * STABLE_AMP_PRECISION;
  if (currentTs >= rampStopTs) return at * STABLE_AMP_PRECISION;
...
export const WEIGHTED_MAX_IN_RATIO = 300_000_000n; // 30% of balanceIn — a REAL on-chain capacity cap (calc_out_given_in returns None past it)
...
function isqrtFloor(n: bigint): bigint {
  if (n < 0n) throw new Error(`isqrtFloor needs a non-negative value, got ${n}`);
  if (n < 2n) return n;
  let x = n;
  let y = (x + 1n) / 2n;
  while (y < x) {
...
function sqrtInternalCeil(xInternal: bigint): bigint {
  return isqrtCeil(xInternal * ISCALE);
}
function mulInternalCeil(a: bigint, b: bigint): bigint {
  return (a * b + ISCALE - 1n) / ISCALE;
}

/** OVER-estimate of base^exponent (base in (0, ONE], exponent >= 0, both ONE(1e9)-scaled). */
export function weightedPowUp(base: bigint, exponent: bigint): bigint {
  if (exponent === POW_ZERO) return STABBLE_ONE;
  if (exponent === POW_ONE) return base;
  if (exponent === POW_TWO) return mulUp(base, base);
  if (exponent === POW_THREE) return mulUp(mulUp(base, base), base);
  ...
  if (intPart > 0n) {
    ...
    while (n > 0n) {
      if (n & 1n) resultInternal = mulInternalCeil(resultInternal, baseP);
      n >>= 1n;
      if (n > 0n) baseP = mulInternalCeil(baseP, baseP);
    }
  }
  if (frac > 0n) {
...
export function weightedCalcOutGivenIn(
  balanceIn: bigint,
  weightIn: bigint,
  balanceOut: bigint,
  weightOut: bigint,
  ...
  if (amountIn > mulDown(balanceIn, WEIGHTED_MAX_IN_RATIO)) return null;
 ...
 */
export function weightedPowUpSource(weightIn: bigint, weightOut: bigint, baseVar: string, out: string): string[] {
  const exponent = divDown(weightIn, weightOut);
  const mulUpExpr = (a: string, b: string): string => `((${a} * ${b} + 999999999) / 1000000000)`;

  if (exponent === POW_ZERO) return [`const ${out} = 1000000000;`];
  if (exponent === POW_ONE) return [`const ${out} = ${baseVar};`];
  if (exponent === POW_TWO) return [`const ${out} = ${mulUpExpr(baseVar, baseVar)};`];
  ...
  const intPart = exponent / STABBLE_ONE;
  const frac = exponent % STABBLE_ONE;
  const mulInternalCeilExpr = (a: string, b: string): string => `((${a} * ${b} + 999999999999999999) / ${ISCALE})`;
  ...
  if (intPart > 0n) {
    ...
    while (n > 0n) {
      if (n & 1n) lines.push(`${out}r = ${mulInternalCeilExpr(`${out}r`, `${out}bp${idx}`)};`);
      n >>= 1n;
      if (n > 0n) {
 ...
 * collide.
 */
export function weightedCoreQuoteLines(
  tag: string,
  balInVar: string,
  balOutVar: string,
  weightIn: bigint,
