...
export const taurusFiLadder: SvmVenueLadderV2 = {
  slug: SLUG,
  ...
  referenceQuote(base, state: AccountBytesMap) {
    const cfg = taurusFiConfig(base);
    const forward = cfg.direction === '0to1';
    const priceData = state[cfg.priceAccount as unknown as string];
    if (priceData === undefined) throw new Error(`${SLUG} reference is missing price account ${cfg.priceAccount}`);
  ...
  depthReserves(base, state: AccountBytesMap) {
    const cfg = taurusFiConfig(base);
    const v0Data = state[cfg.vault0 as unknown as string];
    const v1Data = state[cfg.vault1 as unknown as string];
    if (v0Data === undefined || v1Data === undefined) throw new Error(`${SLUG} depth is missing a vault`);
  ...
  continuousFees() {
    ...
    return { gammaPpm: (1_000_000n * OUT_DISCOUNT_NUM) / OUT_DISCOUNT_DEN, muPpm: 1_000_000n };
  },
};
