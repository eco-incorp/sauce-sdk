...
async function loadAccount(load: AccountLoader, addr: Address, what: string): Promise<Uint8Array> {
  ...
  if (data === null) throw new Error(`virtuals ${what} ${addr} not found`);
  return data;
}
...
export const virtuals = {
  ...
  async fetchPoolConfig(load: AccountLoader, pool: Address): Promise<VirtualsPoolConfig> {
...
export const virtualsLadder: SvmVenueLadder = {
  ...
  referenceQuote(base, state: AccountBytesMap, params: readonly bigint[]) {
    ...
    if (tokenBytes === undefined) throw new Error(`${SLUG} ladder reference is missing account ${cfg.vpoolTokenAta}`);
    if (virtualsBytes === undefined) throw new Error(`${SLUG} ladder reference is missing account ${cfg.vpoolVirtualsAta}`);
    const rbase = readUintLE(tokenBytes, VAULT_AMOUNT_OFFSET, 8);
    const rvirt = readUintLE(virtualsBytes, VAULT_AMOUNT_OFFSET, 8);
    const [virtualY] = params as [bigint];
  ...
  depthReserves(base, state: AccountBytesMap) {
    ...
    const tokenBytes = state[String(cfg.vpoolTokenAta)];
    const virtualsBytes = state[String(cfg.vpoolVirtualsAta)];
    if (tokenBytes === undefined) throw new Error(`${SLUG} ladder depth is missing account ${cfg.vpoolTokenAta}`);
    if (virtualsBytes === undefined) throw new Error(`${SLUG} ladder depth is missing account ${cfg.vpoolVirtualsAta}`);
    const rbase = readUintLE(tokenBytes, VAULT_AMOUNT_OFFSET, 8);
    const rvirt = readUintLE(virtualsBytes, VAULT_AMOUNT_OFFSET, 8);
  ...
  continuousFees(base, _state, params) {
    ...
    if (cfg.direction === 'quoteToBase') {
      ...
      return { gammaPpm: (BPS_DENOM * 1_000_000n) / (BPS_DENOM + FEE_BPS), muPpm: 1_000_000n };
    }
    // SELL: the fee is deducted from the gross OUTPUT.
    return { gammaPpm: 1_000_000n, muPpm: ((BPS_DENOM - FEE_BPS) * 1_000_000n) / BPS_DENOM };
  },
};
