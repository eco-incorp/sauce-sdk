...
export const VOLTR_PROGRAM_ID: Address<'vVoLTRjQmtFpiYoegx285Ze4gsLJ8ZxgFKVcuvmG1a8'> = address(
  'vVoLTRjQmtFpiYoegx285Ze4gsLJ8ZxgFKVcuvmG1a8',
);
export const SPL_TOKEN_PROGRAM_ID: Address<'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA'> = address(
  'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA',
);
const SYSTEM_PROGRAM_ID: Address<'11111111111111111111111111111111'> = address(
...
let cachedProtocolPda: Promise<Address> | undefined;
function protocolPda(): Promise<Address> {
 ...
 */
export async function fetchVoltrConfig(load: AccountLoader, pool: Address): Promise<VoltrPoolConfig> {
  const raw = await load(pool);
  if (raw === null) throw new Error(`${SLUG}: vault account ${pool} not found`);
  ...
  if (assetMintRaw === null) throw new Error(`${SLUG}: asset mint ${assetMint} not found`);
  if (lpMintRaw === null) throw new Error(`${SLUG}: lp mint ${lpMint} not found`);
...
export const voltrLadder: SvmVenueLadder = {
  ...
  referenceQuote(base: PoolConfig, state: AccountBytesMap, _params: readonly bigint[], now?: bigint): (x: bigint) => bigint {
    ...
    const accumFees =
      readU64LE(vault, OFF_ACCUM_LP_MANAGER_FEES) +
      readU64LE(vault, OFF_ACCUM_LP_ADMIN_FEES) +
      readU64LE(vault, OFF_ACCUM_LP_PROTOCOL_FEES);
    const mgmtFeeBps = BigInt(
      readU16LE(vault, OFF_MANAGER_MGMT_FEE) + readU16LE(vault, OFF_ADMIN_MGMT_FEE) + readU16LE(vault, OFF_PROTOCOL_MGMT_FEE),
    );
    ...
    if (lastMgmtTs !== 0n && totalValue !== 0n && mgmtFeeBps !== 0n) {
      ...
      if (elapsed !== 0n) {
        const feeInAsset = (totalValue * elapsed * mgmtFeeBps) / 10000n / 31_536_000n;
        const lpDen = totalValue - feeInAsset;
        if (lpDen > 0n) unrealisedLpFees = (feeInAsset * lpSupplyInclAccum + lpDen - 1n) / lpDen;
      }
    }
    ...
    if (c.direction === 'lpToAsset') {
      ...
      if (lockedDur !== 0n) {
        ...
        if (duration <= lockedDur) lockedProfit = (lockedAmt * (lockedDur - duration)) / lockedDur;
      }
      ...
      return (x: bigint): bigint => {
        if (x === 0n) return 0n;
        if (lpSupplyInclFees === 0n) return 0n;
    ...
    return (x: bigint): bigint => {
      ...
      if (lpSupplyInclFees === 0n) return (effX * bootstrapMul) / bootstrapDiv;
  ...
  continuousFees(base: PoolConfig, state: AccountBytesMap): { gammaPpm: bigint; muPpm: bigint } {
    ...
    const bytes = (addr: Address): Uint8Array => {
      const data = state[addr as unknown as string];
      if (data === undefined) throw new Error(`${SLUG} ladder continuousFees is missing account ${addr}`);
      return data;
    };
    const vault = bytes(c.pool);
    const feeBps =
      c.direction === 'lpToAsset' ? readU16LE(vault, OFF_REDEMPTION_FEE) : readU16LE(vault, OFF_ISSUANCE_FEE);
    const muPpm = 1_000_000n - BigInt(feeBps) * 100n;
