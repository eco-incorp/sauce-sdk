 ...
 * non-quote -> quote; sellQuote: quote -> non-quote — see index.ts's SCOPE
 * note for why base-to-base is deferred).
 *
 * EVERYTHING VALUE-BEARING IS A LIVE READ except the nine scalar shape params
 * (decimals, coeff, feeRate, maxGamma/maxNotionalSwap/capBal/rangeMin/rangeMax
 * as hi/lo u64 pairs — mirroring obric-v2's bigK split, bound/staleDuration/
 * maximumAge/pythQuoteDecPow as single u64 params): wooracle.price/spread/
 * updatedAt, both Pyth feeds' price/publishTime/verification tag, wooconfig's
 * paused flag, and the capacity vault's live SPL balance are all read via
 * `accountUint` at cook time — "bake the shape, read the level", the same
 * discipline obric-v2/quantum use.
 *
 * PRICE_OUT (`get_price_impl`, swap.rs/get_price.rs, mirrored bit-for-bit):
 *   1. self-drop if either Pyth feed's verification tag isn't `Full` (1) or
 *      either feed's publishTime + maximumAge < now (PriceTooOld);
 *   2. cloPrice = basePythPrice * pythQuoteDecPow / quotePythPrice (ONLY the
 *      quote feed's exponent scales this — the real program's own asymmetry,
 *      mirrored not "fixed");
 *   3. self-drop unless cloPrice != 0 AND now <= updatedAt+staleDuration AND
 *      wooraclePrice is within [cloPrice*(1-bound), cloPrice*(1+bound)] (WAD);
 *   4. self-drop (never attempt — the real require would REVERT the whole
 *      cook) unless wooraclePrice is within [rangeMin, rangeMax];
 *   5. price_out = wooraclePrice (the keeper price, NOT cloPrice).
 *
 * CAPACITY (capacityInputVar/referenceCapacities — SATURATING, never
 * collapsing, the same class of fix as obric-v2/meteora-damm-v2): every cap
 * below is a bound on the TRUE (indirection-adjusted) delivered amount that
 * is proven SAFE via a strictly-looser closed-form over-estimate (the real
 * `(1e18-gamma-spread)/1e18` factor is always <= 1, so dropping it only
 * OVER-estimates the delivered amount — capping the over-estimate is always
 * at least as strict as capping the truth):
 *   sellBase (x = base units): capNotional/capGamma bound the intermediate
 *     calcA = floor(x*priceOut/priceDec), inverted back to an x-bound;
 *     capBal bounds x directly (same units); capVault bounds the delivered
 *     quote amount via calcA*quoteDec/baseDec <= quoteVault.amount, inverted;
 *     minSwapAmount is a floor (0 on every live pool today).
 *   sellQuote (x = quote units, already the traded "quoteAmount" up to the
 *     tiny rounded-up fee, so quoteAmount <= x always): capNotional/capGamma
 *     bound quoteAmount directly (== bound x, safe since quoteAmount<=x);
 *     capBal/capVault bound the delivered BASE amount via
 *     x*baseDec*priceDec/(priceOut*quoteDec) <= bound, inverted to an x-bound
 *     (same one-sided-safe drop of the <=1e18 factor); minSwapAmount is
 *     UNAPPLIED here (unit mismatch — see index.ts; inert today regardless).
 *
 * FEE (BOTH directions, once, ROUNDING UP — unlike the EVM port's floor):
 *   swapFee = ceilDiv(quoteAmount * feeRate, 1e5); quoteAmount -= swapFee.
 *   The fee-vault sufficiency require (`quote reserve/vault >= swapFee`) is
 *   NOT separately modeled: swapFee is a small fraction of quoteAmount, and
 *   quoteAmount is already bounded under the SAME quote vault's balance by
 *   the capVault clamp above (sellBase) — so it is dominated, never binding,
 *   whenever the (weaker) fee condition would actually matter.
 */
...
import type { WoofiPoolConfig } from './index.js';
...
function woofiConfig(cfg: PoolConfig): WoofiPoolConfig {
  if (cfg.venue !== SLUG) throw new Error(`${SLUG} ladder adapter got a '${cfg.venue}' pool config`);
  return cfg as WoofiPoolConfig;
}

const ref = (slot: number, role: string): string => `s${slot}:${role}`;

// ---------------------------------------------------------------------------
// Pure math (the TS mirror AND the source the fragment below transcribes).
// ---------------------------------------------------------------------------
...
export function woofiPriceOut(i: PriceOutInputs): bigint {
  if (i.paused) return 0n;
  if (!i.basePythVerified || !i.quotePythVerified) return 0n;
  if (i.basePythPublishTime + i.maximumAge < i.now) return 0n;
  if (i.quotePythPublishTime + i.maximumAge < i.now) return 0n;
  if (i.quotePythPrice <= 0n) return 0n;
  const cloPrice = (i.basePythPrice * i.pythQuoteDecPow) / i.quotePythPrice;
  if (cloPrice === 0n) return 0n;
  if (i.now > i.updatedAt + i.staleDuration) return 0n;
  const lo = (cloPrice * (ONE_E18 - i.bound)) / ONE_E18;
  const hi = (cloPrice * (ONE_E18 + i.bound)) / ONE_E18;
  if (i.wooraclePrice < lo || i.wooraclePrice > hi) return 0n;
  if (i.wooraclePrice < i.rangeMin || i.wooraclePrice > i.rangeMax) return 0n;
  return i.wooraclePrice;
}

/** `calc_quote_amount_sell_base` (swap_math.rs), mirrored bit-for-bit. 0 on any of its own requires failing. */
export function woofiCalcQuoteAmountSellBase(
  x: bigint,
  priceOut: bigint,
  coeff: bigint,
  spread: bigint,
...
export function woofiCalcBaseAmountSellQuote(
  q: bigint,
  priceOut: bigint,
  coeff: bigint,
  spread: bigint,
...
export function woofiSellBaseCapacity(
  priceOut: bigint,
  priceDec: bigint,
  quoteDec: bigint,
  baseDec: bigint,
  ...
  if (calcACapVault < calcACap) calcACap = calcACapVault;
  let cap = (calcACap * priceDec) / priceOut;
  if (capBal < cap) cap = capBal;
  if (minSwapAmount > 0n && cap < minSwapAmount) cap = 0n;
  return cap < 0n ? 0n : cap;
}

/** sellQuote capacity: the largest x (quote units) that keeps every closed-form cap satisfied. */
export function woofiSellQuoteCapacity(
  priceOut: bigint,
  priceDec: bigint,
  quoteDec: bigint,
  baseDec: bigint,
  ...
  if (capBalX < cap) cap = capBalX;
  if (capVaultX < cap) cap = capVaultX;
  return cap < 0n ? 0n : cap;
}
...
function liveState(cfg: WoofiPoolConfig, state: AccountBytesMap, now: bigint): LiveState {
...
function referenceQuoteOf(cfg: WoofiPoolConfig, x: bigint, live: LiveState): bigint {
  if (x <= 0n || live.priceOut <= 0n) return 0n;
  const cx = x > live.capacity ? live.capacity : x;
  if (cx <= 0n) return 0n;
  if (cfg.direction === 'sellBase') {
    const quoteAmount = woofiCalcQuoteAmountSellBase(
      cx,
      live.priceOut,
      cfg.coeff,
      live.spread,
  ...
  return woofiCalcBaseAmountSellQuote(
    net,
    live.priceOut,
    cfg.coeff,
    live.spread,
...
interface ParamExprs {
  priceDec: string; quoteDec: string; baseDec: string; coeff: string; feeRate: string;
  bound: string; staleDuration: string; maximumAge: string; pythQuoteDecPow: string;
  maxGammaHi: string; maxGammaLo: string; maxNotionalHi: string; maxNotionalLo: string;
  capBalHi: string; capBalLo: string; rangeMinHi: string; rangeMinLo: string; rangeMaxHi: string; rangeMaxLo: string;
}

function paramExprs(params: readonly string[]): ParamExprs {
  const [
...
export const woofiLadder: SvmVenueLadder = {
  ...
  referenceQuote(base: PoolConfig, state: AccountBytesMap, params: readonly bigint[], now?: bigint): (x: bigint) => bigint {
    ...
    return (x: bigint): bigint => referenceQuoteOf(cfg, x, live);
  },
  ...
  referenceLadderQuotes(base: PoolConfig, state: AccountBytesMap, params: readonly bigint[], now?: bigint): (grid: readonly bigint[]) => bigint[] {
    const cfg = woofiConfig(base);
    const live = liveState(cfg, state, now ?? 0n);
    return (grid: readonly bigint[]): bigint[] => grid.map((x) => referenceQuoteOf(cfg, x, live));
  },

  referenceCapacities(base: PoolConfig, state: AccountBytesMap, params: readonly bigint[], now?: bigint): (grid: readonly bigint[]) => bigint[] {
    const cfg = woofiConfig(base);
    const live = liveState(cfg, state, now ?? 0n);
    return (grid: readonly bigint[]): bigint[] => grid.map((g) => (g > live.capacity ? live.capacity : g));
  },
  ...
  depthReserves(base: PoolConfig, state: AccountBytesMap): { reserveIn: bigint; reserveOut: bigint } {
    ...
    if (inData === undefined || outData === undefined) throw new Error(`${SLUG} depth is missing a vault`);
 ...
 */
function quoteBody(c: WoofiPoolConfig, p: string, cxVar: string, outVar: string): string {
  const sellBase = c.direction === 'sellBase';
  const lines: string[] = [];
