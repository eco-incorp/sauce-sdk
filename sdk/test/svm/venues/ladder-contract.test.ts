...
const FAMILIES: Family[] = [
  ...
  {
    slug: 'humidifi',
    ladder: humidifiLadder,
    async variants() {
      const POOL = address('8sKQHfjNhvmAw94PhfvfMcytmqW6jmxvwieYyzXCCPu');
      const fixtures = fixturesFor('humidifi');
      const cfg = await fetchHumidifiConfig(fixtureLoader(fixtures), POOL);
      return [{ label: 'default', cfg, state: fixtureBytesMap(fixtures) }];
    },
  },
  {
    slug: 'gatorswap',
    ladder: gatorswapLadder,
    async variants() {
      const POOL = address('8RmCninssSiRZs1Do1Ug13t4toeT57hJVnvZc4EMZXMb');
      const fixtures = fixturesFor('gatorswap');
      const cfg = await gatorswap.fetchPoolConfig(fixtureLoader(fixtures), POOL);
      return [{ label: 'default', cfg, state: fixtureBytesMap(fixtures) }];
    },
  },
  {
    slug: 'kipseli',
    ladder: kipseliLadder,
    async variants() {
      const POOL = address('89dvMturcS87kYgrwyeWqqsaFZUi5dQQpBHs6pvfUu3v');
      const fixtures = fixturesFor('kipseli');
      const cfg = await kipseli.fetchPoolConfig(fixtureLoader(fixtures), POOL);
      return [{ label: 'default', cfg, state: fixtureBytesMap(fixtures) }];
    },
  },
  {
    slug: 'binaryfi',
    ladder: binaryfiLadder,
    async variants() {
      const POOL = address('GWqUcqoZtjq8K6H48YvcHJMEGq8CahNDGMuz3iqU4ESA');
      const fixtures = fixturesFor('binaryfi');
      const cfg = await binaryfi.fetchPoolConfig(fixtureLoader(fixtures), POOL);
      return [{ label: 'default', cfg, state: fixtureBytesMap(fixtures) }];
    },
  },
  {
